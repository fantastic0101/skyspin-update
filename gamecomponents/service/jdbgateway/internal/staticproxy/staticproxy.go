...
func StartProxy(addr string) {
	go func() {
		...
		httpmux.HandleFunc("GET /env/gcp1688.json", Gcp1688)
		httpmux.HandleFunc("GET /env/loggerConfig.json", loggerConfig)
		httpmux.HandleFunc("/frontendAPI.do", frontendAPI)
		httpmux.HandleFunc("/api/logs/batchLog", batchLog)
		httpmux.HandleFunc("/detail", Details)
		httpmux.HandleFunc("/getToken", GetToken)
		httpmux.HandleFunc("/", static_assets)
		httpmux.HandleFunc("/websocket", OnWebsocket)
