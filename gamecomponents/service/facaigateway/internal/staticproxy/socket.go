...
func (cm *ConnectionManager) UpdateConnMType(conn *websocket.Conn, pid int64, mType string) {
	cm.mutex.Lock()
	defer cm.mutex.Unlock()
	...
	u.OnMessage(func(c *websocket.Conn, messageType websocket.MessageType, data []byte) {
		...
		if ok {
			...
			if action == 1 {
				...
				token, _ := pp.GetString("token")
				pid, gameid, err := jwtutil.ParseTokenData(token)
				if err != nil {
					slog.Error("ParseTokenData", err)
					return
				}
				userName, _ := p.GetString("un")

				so := slotsmongo.C0A1PFC(pid, userName)
				bytes, err := so.ToBinary()
				if err != nil {
					slog.Error("ClientLogin Err", "db.GetDocPlayer err", err)
					return
				...
				GlobalConnManager.AddConn(c, pid)
				GlobalConnManager.UpdateConnMType(c, pid, gameid)
				// base64写死的
				so2 := slotsmongo.A4C0FACAI()
				bytes2, err := so2.ToBinary()
				if err != nil {
					slog.Error("ClientLogin Err", "db.GetDocPlayer err", err)
					return
				}
				c.WriteMessage(messageType, bytes2)
				//初始化相关（不知道做什么）
				so3 := slotsmongo.C1A13FACaiLobbyInfo(userName)
				bytes3, err := so3.ToBinary()
				if err != nil {
					slog.Error("ClientLogin Err", "db.GetDocPlayer err", err)
					return
				}
				c.WriteMessage(messageType, bytes3)
				//走游戏info
				...
				subj := fmt.Sprintf("%s.%s", gameid, H5Init)
				...
				soInit := slotsmongo.C1A13EV_SC_GAME_INIT(resp.Data)
				fmt.Println(so)
				initBytes, err := soInit.ToBinary()
				if err != nil {
					slog.Error("ClientLogin Err", "db.GetDocPlayer err", err)
					return
				}
				c.WriteMessage(messageType, initBytes)
				soA1000 := slotsmongo.A1000C0FACAI()
				A1000Bytes, err := soA1000.ToBinary()
				if err != nil {
					slog.Error("ClientLogin Err", "db.GetDocPlayer err", err)
					return
				}
				c.WriteMessage(messageType, A1000Bytes)
			}
			if action == 13 {
				if pc == GameLogin {
					//pp, _ := p.GetSFSObject("p")
				...
				if pc == EV_CS_SPIN {
					...
					subj := fmt.Sprintf("%s.%s", info.MachineType, EV_CS_SPIN)
					resp, err := mq.NC().RequestMsg(&nats.Msg{
						Subject: subj,
						Data:    payload,
					}, time.Second*60)
					...
					so := slotsmongo.C1A13h5FaCaiSpinResponse(resp.Data)
					bytes, err := so.ToBinary()
					if err != nil {
						slog.Error("ClientLogin Err", "db.GetDocPlayer err", err)
						return
