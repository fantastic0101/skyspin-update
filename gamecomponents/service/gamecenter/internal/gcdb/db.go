...
var (
	...
	DBAdmin        = mongodb.NewDB("GameAdmin")
	CollGameConfig = DBAdmin.Collection("GameConfig")
	...
	CollAdminOperator = DBAdmin.Collection("AdminOperator") // 运营商
	CollSystemConfig  = DBAdmin.Collection("SystemConfig")  // 系统配置
...
func ConnectMongoByConfig() {
	...
	err = DBAdmin.Connect(mongoAddr)

	if err != nil {
		logger.Info("连接DBAdmin mongodb失败", err)
	} else {
		logger.Info("连接DBAdmin mongodb成功")
		db.SetupClient(DBAdmin.Client)
	}
}
