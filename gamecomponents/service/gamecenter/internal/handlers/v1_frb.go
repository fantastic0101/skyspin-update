...
func v1_frb_add(app *operator.MemApp, ps v1FRBAddPs, ret *v1FRBAddRet) (err error) {
	...
	if len(ps.PlayerList) == 0 {
		err = define.NewErrCode("PlayerList is empty", 1020)
		return
	}
	...
	if ps.ExpirationDate < now.Unix() {
		err = define.NewErrCode("ExpirationDate is invalid", 1021)
		return
	}

	if ps.Rounds < 1 {
		err = define.NewErrCode("Rounds is invalid", 1022)
		return
	}
	if ps.TotalBet <= 0 {
		err = define.NewErrCode("TotalBet is invalid", 1023)
		return
	}
	if ps.BonusCode == "" {
		err = define.NewErrCode("BonusCode is invalid", 1024)
		return
	}
	if !isPPGame(ps.GameID) {
		err = define.NewErrCode("GameID is invalid", 1025)
		return
	}
