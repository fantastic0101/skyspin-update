...
func (s *service) RngRtp(state int, drbMathI interface{}) (rtpState int, rtpId string, rtpBullets uint64) {
	drbMath := reflect.ValueOf(drbMathI).Interface().(*PSF_ON_00002_1.DrbMath)
	rtpGroupId := -1
	...
	case 10:
		rtpId = drbMath.RTPGroup.RTP10.ID

		rtpBullets = s.rngBullets(
			drbMath.RTPGroup.RTP10.MinBullet,
			drbMath.RTPGroup.RTP10.MaxBullet,
		)

	case 11:
		rtpId = drbMath.RTPGroup.RTP11.ID

		rtpBullets = s.rngBullets(
			drbMath.RTPGroup.RTP11.MinBullet,
			drbMath.RTPGroup.RTP11.MaxBullet,
		)
