...
var MagicShell = &magicShell{
	bonusMode: strings.Contains(os.Getenv("ENABLED_BONUS"), "300"),
}

type magicShell struct {
	bonusMode bool
}

type BsMagicShell struct {
	...
	RTP1           struct{ HitBsMagicShell } `json:"RTP1"`
	RTP2           struct{ HitBsMagicShell } `json:"RTP2"`
	RTP3           struct{ HitBsMagicShell } `json:"RTP3"`
	RTP4           struct{ HitBsMagicShell } `json:"RTP4"`
	RTP5           struct{ HitBsMagicShell } `json:"RTP5"`
	RTP6           struct{ HitBsMagicShell } `json:"RTP6"`
	RTP7           struct{ HitBsMagicShell } `json:"RTP7"`
	RTP8           struct{ HitBsMagicShell } `json:"RTP8"`
	RTP9           struct{ HitBsMagicShell } `json:"RTP9"`
}

type HitBsMagicShell struct {
	ID            string `json:"ID"`
...
func (p *magicShell) Hit(rtpId string, math *BsMagicShell) (iconPay, triggerIconId, bonusTypeId, multiplier int) {
	iconPay = 0
	triggerIconId = -1
	bonusTypeId = -1
	multiplier = 0
...
func (p *magicShell) pick(math *BsMagicShell) int {
...
func (p *magicShell) HitFs(math *FsFishMath, payIndex int) (iconPays int) {
