...
func (s *starfish) Hit(rtpId string, math *BsFishMath) (iconPay, triggerIconId, bonusTypeId int) {
	...
	switch rtpId {
	case math.RTP1.ID:
		return s.rtpHit(math.IconPays[0], &math.RTP1)

	case math.RTP2.ID:
		return s.rtpHit(math.IconPays[0], &math.RTP2)

	case math.RTP3.ID:
		return s.rtpHit(math.IconPays[0], &math.RTP3)

	case math.RTP4.ID:
		return s.rtpHit(math.IconPays[0], &math.RTP4)

	case math.RTP5.ID:
		return s.rtpHit(math.IconPays[0], &math.RTP5)

	case math.RTP6.ID:
		return s.rtpHit(math.IconPays[0], &math.RTP6)

	case math.RTP7.ID:
		return s.rtpHit(math.IconPays[0], &math.RTP7)

	case math.RTP8.ID:
		return s.rtpHit(math.IconPays[0], &math.RTP8)

	case math.RTP9.ID:
		return s.rtpHit(math.IconPays[0], &math.RTP9)
