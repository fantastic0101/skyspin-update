...
func psf_on_00001_HitBullet(action *flux.Action, am *AccountingManager) {
	if am.guestEnable {
		return
	}
	...
	if hitResult.TriggerIconId != 28 && hitResult.TriggerIconId != 29 && hitResult.TriggerIconId != 31 && hitResult.TriggerIconId > 0 {
	...
	if hitBullet.TypeId != 26 {
		// 26 had only one UUID, so needed generate UUID as PK when insert table.
	...
	if hitBullet.TypeId == 26 || hitBullet.TypeId == 27 {
		gameData = strconv.Itoa(int(bet))
		bet = 0
	}

	// Regular Fish
	// Todo Levi psf_on_00002 hotpot need * hitResult.Multiplier
...
func psf_on_00001_Bonus(action *flux.Action, am *AccountingManager) {
	if am.guestEnable {
		return
	}
	...
	case *slot.Slot:
		...
		if hitResult.TriggerIconId == 29 || hitBullet.TypeId == 26 || hitBullet.TypeId == 27 {
			bet = 0
		}
...
func psf_on_00001_RefundBonusBullet(action *flux.Action, am *AccountingManager) {
	if am.guestEnable {
		return
	}
...
func psf_on_00001_CheckBulletTypeId(bulletId, restBullets int32,
...
func psf_on_00001_BulletHit(bulletId int32,
...
func psf_on_00001_BulletHit_TriggerId(triggerIconId int,
	am *AccountingManager,
	hitBullet *bullet.Bullet,
...
func psf_on_00001_BulletBonus(bonusBulletId int32, bonusBullet *bullet.Bullet, machineGunUuid map[string]map[uint64]int32, bonusBulletShooted map[string]map[string]map[uint64]int32) {
