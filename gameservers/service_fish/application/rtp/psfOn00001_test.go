...
const (
	gameId00001 = models.PSF_ON_00001
)

var rtpList00001 = map[int][]string{
	0: {"0", "20", "40", "60"},
	1: {"80", "100", "150", "200", "300"},
}
var mathModuleId00001 string

func TestService00001(t *testing.T) {
	// Process Input Value
	...
	// 數學代號
	mathModuleId00001 = chooseMath(gameId00001, os.Args[1])
	if mathModuleId00001 == "" {
		fmt.Println("數學代號不存在")
		return
	}
	if mathModuleId00001 == " " {
	...
	if os.Args[3] == "-1" {
		selectFishList = fishList[gameId00001]
	} else {
		inputFishIdList := strings.Split(os.Args[3], ",")
		for _, inputFishId := range inputFishIdList {
			if !isContain(fishList[gameId00001], inputFishId) {
				fmt.Println("魚種不存在")
				return
			}
		}
	...
	for i := 1; i <= runTimes*10000; i++ {
		...
		_, result, rtpState, rtpId, _, rtpBudget, _, _, _ := lottery.Service.MathProcess(
			secWebSocketKey, gameId00001, mathModuleId00001, "", "", "", "",
			-1, fishId, -1,
		...
		hit, win, envelope, _, _, _, _, _, _,
			...
			_, _,
			drillTime, drillHit, _ := resultProcess(gameId00001, mathModuleId00001, fishId, result, uint64(bet), weaponFish)
	...
	showString := ""
	for _, oneFish := range fishList[gameId00001] {
		showString = fmt.Sprint(oneFish)
		// Rtp State Times
		showString = getShowData(showString, oneFish, rtpStateTimes, rtpList00001)

		// Rtp Bullet Count
		showString = getShowData(showString, oneFish, rtpBudgets, rtpList00001)

		// Fish Times
		showString = getShowData(showString, oneFish, resultTimes, rtpList00001)

		// Hit Times
		showString = getShowData(showString, oneFish, resultHits, rtpList00001)
		...
		// RedEnvelop Hits
		showString = getShowData(showString, oneFish, resultHitEnvelope, rtpList00001)
