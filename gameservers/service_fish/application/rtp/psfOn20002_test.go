...
const gameId20002 = models.PSF_ON_20002

var mathModuleId20002 string

var rtpList20002 = map[int][]string{
	0: {"0", "20", "40", "60"},
	1: {"80", "100", "150", "200", "300"},
}

func TestService20002(t *testing.T) {
	// Process Input Value
	...
	// 數學代號
	mathModuleId20002 = chooseMath(gameId20002, os.Args[1])
	if mathModuleId20002 == "" {
		fmt.Println("數學代號不存在")
		return
	}
	if mathModuleId20002 == " " {
	...
	if os.Args[3] == "-1" {
		selectFishList = fishList[gameId20002]
	} else {
		inputFishIdList := strings.Split(os.Args[3], ",")
		for _, inputFishId := range inputFishIdList {
			if !isContain(fishList[gameId20002], inputFishId) {
				fmt.Println("魚種不存在")
				return
			}
		}
	...
	for i := 1; i < runTimes*10000; i++ {
		...
		_, result, rtpState, rtpId, _, rtpBudget, _, _, _ := lottery.Service.MathProcess(
			secWebSocketKey, gameId20002, mathModuleId20002, "", "", "", "",
			-1, fishId, -1,
		...
		/*
			hit, win, envelope, _, _, _, _, _, _,
				...
				_, _,
				drillTime, drillHit := resultProcess(gameId20002, mathModuleId20002, fishId, result, uint64(bet))
		*/
		hit, win, envelope, _, _, _, _, _, _,
			machineGunTime, machineGunHit,
			_, _,
			drillTime, drillHit, _ := resultProcess(gameId20002, mathModuleId20002, fishId, result, uint64(bet), weaponFish)
	...
	for _, oneFish := range fishList[gameId20002] {
		showString = fmt.Sprint(oneFish)
		// Rtp State Times
		showString = getShowData(showString, oneFish, rtpStateTimes, rtpList20002)

		// Rtp Bullet Count
		showString = getShowData(showString, oneFish, rtpBudgets, rtpList20002)

		// Fish Times
		showString = getShowData(showString, oneFish, resultTimes, rtpList20002)

		// Hit Times
		showString = getShowData(showString, oneFish, resultHits, rtpList20002)
		...
		// RedEnvelop Hits
		showString = getShowData(showString, oneFish, resultHitEnvelope, rtpList20002)
