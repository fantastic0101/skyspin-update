...
const gameId00005 = models.PSF_ON_00005

var mathModuleId00005 string
var rtpList00005 = map[int][]string{
	0: {"0", "20", "40", "60"},
	1: {"80", "100", "150", "200", "300"},
}

func TestService00005(t *testing.T) {
	// Process Input Value
	...
	// 數學代號
	mathModuleId00005 = chooseMath(gameId00005, os.Args[1])
	if mathModuleId00005 == "" {
		fmt.Println("數學代號不存在")
		return
	}
	if mathModuleId00005 == " " {
	...
	if os.Args[3] == "-1" {
		selectFishList = fishList[gameId00005]
	} else {
		inputFishIdList := strings.Split(os.Args[3], ",")
		for _, inputFishId := range inputFishIdList {
			if !isContain(fishList[gameId00005], inputFishId) {
				fmt.Println("魚種不存在")
				return
			}
		}
	...
	for i := 1; i <= runTimes*10000; i++ {
		...
		_, result, rtpState, rtpId, _, rtpBudget, _, _, _ := lottery.Service.MathProcess(
			secWebSocketKey, gameId00005, mathModuleId00005, "", "", "", "",
			-1, fishId, -1,
		...
		hit, win, envelope, _, _, _, _, _, _,
			...
			superMachineGunTime, superMachineGunHit, _, _, _ := resultProcess(gameId00005, mathModuleId00005, fishId, result, uint64(bet), weaponFish)
	...
	for _, oneFish := range fishList[gameId00005] {
		showString = fmt.Sprint(oneFish)

		// Rtp State Times
		showString = getShowData(showString, oneFish, rtpStateTimes, rtpList00005)

		// Rtp Bullet Count
		showString = getShowData(showString, oneFish, rtpBudgets, rtpList00005)

		// Fish Times
		showString = getShowData(showString, oneFish, resultTimes, rtpList00005)

		// Hit Times
		showString = getShowData(showString, oneFish, resultHits, rtpList00005)
		...
		// RedEnvelop Hits
		showString = getShowData(showString, oneFish, resultHitEnvelope, rtpList00005)
