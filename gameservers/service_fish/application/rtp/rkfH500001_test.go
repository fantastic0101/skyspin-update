...
const gameIdRkfH500001 = models.RKF_H5_00001

var mathModuleIdRkfH500001 string
var rtpListRkfH500001 = map[int][]string{
	0: {"0", "20", "40", "60"},
	1: {"80", "100", "150", "200", "300"},
}

func TestServiceH500001(t *testing.T) {
	// Process Input Value
	...
	// 數學代號
	mathModuleIdRkfH500001 = chooseMath(gameIdRkfH500001, os.Args[1])
	if mathModuleIdRkfH500001 == "" {
		fmt.Println("數學代號不存在")
		return
	}
	if mathModuleIdRkfH500001 == " " {
	...
	if os.Args[3] == "-1" {
		selectFishList = fishList[gameIdRkfH500001]
	} else {
		inputFishIdList := strings.Split(os.Args[3], ",")
		for _, inputFishId := range inputFishIdList {
			if !isContain(fishList[gameIdRkfH500001], inputFishId) {
				fmt.Println("魚種不存在")
				return
			}
		}
	...
	for i := 1; i <= runTimes*10000; i++ {
		...
		_, result, rtpState, rtpId, _, rtpBudget, _, _, _ := lottery.Service.MathProcess(
			secWebSocketKey, gameIdRkfH500001, mathModuleIdRkfH500001, "", "", "", "",
			-1, fishId, -1,
		...
		hit, win, envelope, _, _, _, _, _, _,
			...
			superMachineGunTime, superMachineGunHit, _, _, _ := resultProcess(gameIdRkfH500001, mathModuleIdRkfH500001, fishId, result, uint64(bet), weaponFish)

		totalTimes++
		totalPay += win + envelope
	...
	for _, oneFish := range fishList[gameIdRkfH500001] {
		showString = fmt.Sprint(oneFish)

		// Rtp State Times
		showString = getShowData(showString, oneFish, rtpStateTimes, rtpListRkfH500001)

		// Rtp Bullet Count
		showString = getShowData(showString, oneFish, rtpBudgets, rtpListRkfH500001)

		// Fish Times
		showString = getShowData(showString, oneFish, resultTimes, rtpListRkfH500001)

		// Hit Times
		showString = getShowData(showString, oneFish, resultHits, rtpListRkfH500001)
		...
		// RedEnvelop Hits
		showString = getShowData(showString, oneFish, resultHitEnvelope, rtpListRkfH500001)
	...
	fmt.Println(getRate(totalPay, totalTimes))
}
