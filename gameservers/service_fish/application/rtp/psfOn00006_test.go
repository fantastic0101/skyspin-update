...
const gameId00006 = models.PSF_ON_00006

var mathModuleId00006 string
var rtpList00006 = map[int][]string{
	0: {"0", "20", "40", "60"},
	1: {"80", "100", "150", "200", "300"},
}

func TestService00006(t *testing.T) {
	// Process Input Value
	...
	// 數學代號
	mathModuleId00006 = chooseMath(gameId00006, os.Args[1])
	if mathModuleId00006 == "" {
		fmt.Println("數學代號不存在")
		return
	}
	if mathModuleId00006 == " " {
	...
	if os.Args[3] == "-1" {
		selectFishList = fishList[gameId00006]
	} else {
		inputFishIdList := strings.Split(os.Args[3], ",")
		for _, inputFishId := range inputFishIdList {
			if !isContain(fishList[gameId00006], inputFishId) {
				fmt.Println("魚種不存在")
				return
			}
		}
	...
	for i := 1; i <= runTimes*10000; i++ {
		...
		_, result, rtpState, rtpId, _, rtpBudget, _, _, _ := lottery.Service.MathProcess(
			secWebSocketKey, gameId00006, mathModuleId00006, "", "", "", "",
			-1, fishId, -1,
		...
		hit, win, envelope, _, _, _, _, _, _,
			...
			superMachineGunTime, superMachineGunHit, _, _, _ := resultProcess(gameId00006, mathModuleId00006, fishId, result, uint64(bet), weaponFish)
	...
	for _, oneFish := range fishList[gameId00006] {
		showString = fmt.Sprint(oneFish)

		// Rtp State Times
		showString = getShowData(showString, oneFish, rtpStateTimes, rtpList00006)

		// Rtp Bullet Count
		showString = getShowData(showString, oneFish, rtpBudgets, rtpList00006)

		// Fish Times
		showString = getShowData(showString, oneFish, resultTimes, rtpList00006)

		// Hit Times
		showString = getShowData(showString, oneFish, resultHits, rtpList00006)
		...
		// RedEnvelop Hits
		showString = getShowData(showString, oneFish, resultHitEnvelope, rtpList00006)
