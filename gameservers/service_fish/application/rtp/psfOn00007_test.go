...
const gameId00007 = models.PSF_ON_00007

var mathModuleId00007 string
var rtpList00007 = map[int][]string{
	0: {"0", "20", "40", "60"},
	1: {"80", "100", "150", "200", "300"},
}

func TestService00007(t *testing.T) {
	// Process Input Value
	...
	// 數學代號
	mathModuleId00007 = chooseMath(gameId00007, os.Args[1])
	if mathModuleId00007 == "" {
		fmt.Println("數學代號不存在")
		return
	}
	if mathModuleId00007 == " " {
	...
	if os.Args[3] == "-1" {
		selectFishList = fishList[gameId00007]
	} else {
		inputFishIdList := strings.Split(os.Args[3], ",")
		for _, inputFishId := range inputFishIdList {
			if !isContain(fishList[gameId00007], inputFishId) {
				fmt.Println("魚種不存在")
				return
			}
		}
	...
	for i := 1; i <= runTimes*10000; i++ {
		...
		_, result, rtpState, rtpId, _, rtpBudget, _, _, _ := lottery.Service.MathProcess(
			secWebSocketKey, gameId00007, mathModuleId00007, "", "", "", "",
			-1, fishId, -1,
		...
		hit, win, envelope, _, _, _, _, _, _,
			...
			superMachineGunTime, superMachineGunHit, _, _, _ := resultProcess(gameId00007, mathModuleId00007, fishId, result, uint64(bet), weaponFish)
		...
		if envelope > 0 {
			...
			if result.TriggerIconId == 201 || result.TriggerIconId == 202 {
	...
	for _, oneFish := range fishList[gameId00007] {
		showString = fmt.Sprint(oneFish)

		// Rtp State Times
		showString = getShowData(showString, oneFish, rtpStateTimes, rtpList00007)

		// Rtp Bullet Count
		showString = getShowData(showString, oneFish, rtpBudgets, rtpList00007)

		// Fish Times
		showString = getShowData(showString, oneFish, resultTimes, rtpList00007)

		// Hit Times
		showString = getShowData(showString, oneFish, resultHits, rtpList00007)
		...
		// RedEnvelop Hits
		showString = getShowData(showString, oneFish, resultHitEnvelope, rtpList00007)
