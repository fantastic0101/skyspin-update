...
func SendTelegramChat(msg string) {
	sendTelegramChat(msg)
}
...
func BigRewardDeal(ps *BigRewardDealParams) {
	...
	msg.WriteString("中奖金额：" + define.ThousandSeparator(fmt.Sprintf("%.2f\n", ps.OriginWin)))
	msg.WriteString(fmt.Sprintf("币种：%s\n", ps.CurrencyKey))
	msg.WriteString("实际金额：" + define.ThousandSeparator(fmt.Sprintf("%.2f\n", ps.RealWin)))
	msg.WriteString("中奖时净盈亏：" + define.ThousandSeparator(fmt.Sprintf("%.2f\n", float64(ps.WinLose)/10000)))
