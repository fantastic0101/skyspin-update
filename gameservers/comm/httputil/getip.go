...
// copy from https://github.com/Ferluci/fasthttp-realip/blob/master/realip.go
...
var xAliCdnRealIp = http.CanonicalHeaderKey("Ali-Cdn-Real-Ip")
...
func GetIPFromRequest(ctx *http.Request) string {
	aliCdnRealIp := ctx.Header.Get(xAliCdnRealIp)
	if aliCdnRealIp != "" {
		return aliCdnRealIp
	}

	xClientIP := ctx.Header.Get(xClientIPHeader)
	if xClientIP != "" {
		return string(xClientIP)
	}

	xOriginalForwardedFor := ctx.Header.Get(xOriginalForwardedForHeader)
	...
	xForwardedFor := ctx.Header.Get(xForwardedForHeader)
	if xForwardedFor != "" {
...
func fromSpecialHeaders(ctx *http.Request) (string, error) {
	...
	for _, iplHeader := range ipHeaders {
		if clientIP := ctx.Header.Get(iplHeader); clientIP != "" {
