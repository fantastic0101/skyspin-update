...
import (
	...
	"serve/comm/db"
	"serve/service/pg_34/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_34")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_34").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("mubanyy%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "34"))
