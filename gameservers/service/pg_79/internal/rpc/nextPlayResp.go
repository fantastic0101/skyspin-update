...
func init() {
	...
	fmt.Println("bigRewardBucketId is:", bigRewardBucketId)
}

func nextPlayResp(player *models.Player, bet int64, selfPoolGold int64, isBuy bool, App *redisx.AppStore) (playResp *gendata.SimulateData, hitBigAward, forcedKill bool, buyKill bool, err error) {
	if false {
	...
	if isBuy {
		...
		fmt.Println("player.minAwardPercent:", minAwardPercent)
		if minAwardPercent == 0 {
			minAwardPercent = App.BuyMinAwardPercent
			fmt.Println("App.minAwardPercent:", minAwardPercent)
		}
