...
import (
	...
	"serve/comm"
	"serve/service/pg_1340277/internal/gendata"
	...
	"serve/comm/ut"
	"serve/service/pg_1340277/internal/models"
...
func init() {
	mux.RegRpc("/game-api/1340277/v2/Spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
	mux.RegRpc("/game-api/1340277/v2/spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
}
...
func spin(plr *models.Player, ps define.PGParams, ret *json.RawMessage) (err error) {
	...
	if isEnd {
		...
		if err != nil {
			// err = lang.Error(plr.Language, "金币不足")
			// return err
	...
	/*if len(plr.BdRecords) > 0 {
		...
		if ps.Header["jump_log"] == nil {
			detailsId = slotsmongo.InsertBetHistory(plr.PID, 1340277, plr.BdRecords, ids, plr.GetCurrencyOrTHB())
	...
	if ps.Header["jump_log"] == nil {
		slotsmongo.InsertBetHistoryEvery(plr.BetHistLastID, plr.PID, 1340277, num, data, betId, plr.GetCurrencyOrTHB(), isEnd)
