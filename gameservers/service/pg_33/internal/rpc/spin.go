...
import (
	...
	"serve/service/pg_33/internal/models"
...
func init() {
	mux.RegRpc("/game-api/33/v2/Spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
	mux.RegRpc("/game-api/33/v2/spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
}
...
func spin(plr *models.Player, ps define.PGParams, ret *json.RawMessage) (err error) {
	...
	type RS struct {
		Ewp interface{} `json:"ewp"`
		BF  interface{} `json:"bf"`
	}
	...
	if ps.Header["jump_log"] == nil {
		detailsId = slotsmongo.InsertBetHistoryEvery(plr.BetHistLastID, plr.PID, 33, num, data, betId, plr.GetCurrencyOrTHB(), isEnd)
	}
