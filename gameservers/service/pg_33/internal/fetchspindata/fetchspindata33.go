...
import (
	...
	"serve/comm/db"
	"serve/service/pg_33/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_33")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_33").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("33zw%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "33"))
