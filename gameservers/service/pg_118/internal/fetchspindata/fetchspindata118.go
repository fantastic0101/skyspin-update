...
import (
	...
	"serve/comm/db"
	"serve/service/pg_118/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_118")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_118").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("118normal_SJ%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "118"))
