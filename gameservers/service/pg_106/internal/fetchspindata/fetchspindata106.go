...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_106")
...
func normal(i int) {
	uid := fmt.Sprintf("testusery106yy%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "106"))
	...
	for ; ; time.Sleep(time.Second) {
		...
		if sid == bsondoc.Lookup("psid").StringValue() && len(bsonDocs) > 0 {
			...
			// 判断是否是从初始盘开始拉去的
			if firstDoc.Lookup("psid").StringValue() == firstDoc.Lookup("sid").StringValue() && len(bsonDocs) > 1 {
...
func game(i int) {
	uid := fmt.Sprintf("testyygame106yy%d", i)
