...
import (
	...
	"serve/comm/db"
	"serve/service/pg_115/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_115")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_115").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("115sj%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "115"))
