...
import (
	...
	"serve/comm/ut"
	"serve/service/pg_115/internal/gendata"
	"serve/service/pg_115/internal/models"
...
func init() {
	mux.RegRpc("/game-api/115/v2/Spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
	mux.RegRpc("/game-api/115/v2/spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
}
...
func spin(plr *models.Player, ps define.PGParams, ret *json.RawMessage) (err error) {
	...
	if ps.Header["jump_log"] == nil {
		detailsId = slotsmongo.InsertBetHistoryEvery(plr.BetHistLastID, plr.PID, 115, num, data, betId, plr.GetCurrencyOrTHB(), isEnd)
	}
