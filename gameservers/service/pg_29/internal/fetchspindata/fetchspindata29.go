...
import (
	...
	"serve/comm/db"
	"serve/service/pg_29/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_29")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_29").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("29tjh%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "29"))
