...
import (
	...
	"serve/comm/db"
	"serve/comm/plats/pg"
	"serve/service/pg_1381200/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_1381200")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_1381200").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("1381200tjh%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "1381200"))
...
func game(i int) {
	uid := fmt.Sprintf("1381200tjhgame%d", i)
