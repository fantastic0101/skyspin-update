...
import (
	...
	"serve/comm/ut"
	"serve/service/pg_44/internal/gendata"
	"serve/service/pg_44/internal/models"
...
func init() {
	mux.RegRpc("/game-api/44/v2/Spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
	mux.RegRpc("/game-api/44/v2/spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
}
...
func spin(plr *models.Player, ps define.PGParams, ret *json.RawMessage) (err error) {
	...
	data := doc.Deal2(num, ut.Gold2Money(balance), plr.Cs, plr.Ml, gendata.Line, gendata.BuyMul, 0.9, plr.IsBuy)
	...
	//detailsId := slotsmongo.InsertBetHistoryEvery(plr.BetHistLastID, plr.PID, 44, num, data, betId, plr.GetCurrencyOrTHB(), isEnd)
