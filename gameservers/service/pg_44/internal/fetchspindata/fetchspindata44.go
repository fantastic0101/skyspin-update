...
import (
	...
	"serve/comm/db"
	"serve/service/pg_44/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_44")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_44").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("44normal_SJ%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "44"))
