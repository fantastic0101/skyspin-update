...
import (
	...
	"serve/comm/db"
	"serve/comm/plats/pg"
	"serve/service/pg_1338274/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_1338274")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_1338274").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("1338274sj%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "1338274"))
...
func game(i int) {
	uid := fmt.Sprintf("1338274yygame%d", i)
