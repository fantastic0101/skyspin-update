...
import (
	...
	"serve/comm/db"
	"serve/service/pg_1368367/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_1368367")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_1368367").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("1368367normal_SJ2%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "1368367"))
...
func game(i int) {
	uid := fmt.Sprintf("1368367yygame%d", i)
