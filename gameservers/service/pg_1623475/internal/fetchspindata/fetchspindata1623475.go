...
import (
	...
	"log/slog"
	"net/url"
	"serve/service/pg_1623475/internal/dealData"
	"strings"
	"sync"
	"time"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_1623475")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_1623475").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("1623475yy%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "1623475"))
...
func game(i int) {
	uid := fmt.Sprintf("1623475yygame%d", i)
