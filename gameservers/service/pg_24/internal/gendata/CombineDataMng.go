...
type CombineDataMng struct {
	...
	buckets3     map[int][][]primitive.ObjectID
	...
	combineDatas3    map[string]*CombineData
...
func (mng *CombineDataMng) GetBigReward2_5(gameType db.BoundType, witchBucket int) (playResp *SimulateData, err error) {
	...
	if gameType == GameTypeNormal {
		ids = GetBucketIds(10, 20, GameTypeNormal)
	...
	} else if gameType == GameTypeGamex2 {
		ids = GetBucketIds(20, 40, GameTypeGamex2)
		ids = lo.Filter(ids, func(item int, index int) bool {
		...
		id := lo.Sample(data)
		return findFromSimulate(id)
	} else if gameType == GameTypeGamex3 {
		ids = GetBucketIds(30, 60, GameTypeGamex3)
		ids = lo.Filter(ids, func(item int, index int) bool {
			return len(mng.buckets3[witchBucket][item]) != 0
		})
		lo.Must0(len(ids) != 0)
		bucketid := lo.Sample(ids)
		data := mng.buckets3[witchBucket][bucketid]
		id := lo.Sample(data)
		return findFromSimulate(id)
	}
...
func LoadCombineData() (err error) {
	...
	type bucketGroups struct {
		...
		third  [][]primitive.ObjectID
	}

	bucketConfig := map[int]*bucketGroups{
		BucketHeartBeat:      {makeSlice(), makeSlice(), makeSlice()},
		BucketWave:           {makeSlice(), makeSlice(), makeSlice()},
		BucketGov:            {makeSlice(), makeSlice(), makeSlice()},
		BucketMix:            {makeSlice(), makeSlice(), makeSlice()},
		BucketStable:         {makeSlice(), makeSlice(), makeSlice()},
		BucketHighAward:      {makeSlice(), makeSlice(), makeSlice()},
		BucketSuperHighAward: {makeSlice(), makeSlice(), makeSlice()},
	}
	...
	for _, doc := range docs {
		...
		handleBucket := func(bucketType int, config *bucketGroups, isOff bool) {
			if isOff {
				...
				if doc.Type == GameTypeGamex2 {
					config.double[doc.BucketID] = append(config.double[doc.BucketID], doc.ID)
				}
				if doc.Type == GameTypeGamex3 {
					config.third[doc.BucketID] = append(config.third[doc.BucketID], doc.ID)
				}
			}
		}
	...
	GCombineDataMng = &CombineDataMng{
		...
		combineDatas3:    make(map[string]*CombineData),
...
func (mng *CombineDataMng) get3(bet string) (data *CombineData) {
	mng.mtx.Lock()
	defer mng.mtx.Unlock()
	data = mng.combineDatas3[bet]
	if data == nil {
		data = &CombineData{
			combine: mng.combine,
			buckets: mng.buckets3,
		}
		mng.combineDatas3[bet] = data
	}
	return
}
...
func (mng *CombineDataMng) getCombineUsedCount(bucketid int, ty db.BoundType, witchBucket int) int {
	// data := mng.get(0)
	data := mng.get("")
	if ty == GameTypeGamex2 {
		data = mng.get2("")
	} else if ty == GameTypeGamex3 {
		data = mng.get3("")
	}
	...
	if nextData != nil {
		...
		if len(ids) == 0 {
			if gameType == GameTypeNormal {
				...
				for id, bucket := range mng.buckets3[witchBucket] {
					if len(bucket) > 0 {
						ids = append(ids, id)
					}
				}
