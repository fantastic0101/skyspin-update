		...
		if pan["sid"].(string) == pan["psid"].(string) && len(pans) > 0 {
			if pans[0]["sid"].(string) == pans[0]["psid"].(string) { // 第一盘是否有效
				...
				data := gendata.SimulateData{
					...
					HasGame:  hasGame,
					Times:    times,
					BucketId: gendata.GBuckets.GetBucket(times, gendata.GameTypeNormal),
					Type:     int(gendata.GameTypeNormal),
					Selected: true,
				}
				lo.Must(newColl.InsertOne(context.TODO(), data))
				fmt.Println("DealNormal success", objectId.Hex())
			}
...
func DealGamex2(collName string, gameType db.BoundType) {
		...
		if pan["sid"].(string) == pan["psid"].(string) && len(pans) > 0 {
			if pans[0]["sid"].(string) == pans[0]["psid"].(string) { // 第一盘是否有效
				...
				data := gendata.SimulateData{
					...
					HasGame:  true,
					Times:    times,
					BucketId: gendata.GBuckets.GetBucket(times, gameType),
					Type:     int(gameType),
					Selected: true,
				}
				lo.Must(newColl.InsertOne(context.TODO(), data))
				fmt.Println("DealGamex2 success", objectId.Hex())

				fmt.Println("gameType = ", gameType)
				fmt.Println("db.BoundType(gameType) = ", db.BoundType(gameType))
...
func DealGamex3(collName string, gameType db.BoundType) {
		...
		if pan["sid"].(string) == pan["psid"].(string) && len(pans) > 0 {
			if pans[0]["sid"].(string) == pans[0]["psid"].(string) { // 第一盘是否有效
				...
				data := gendata.SimulateData{
					...
					HasGame:  true,
					Times:    times,
					BucketId: gendata.GBuckets.GetBucket(times, db.BoundType(gameType)),
					Type:     int(gameType),
					Selected: true,
				}
				lo.Must(newColl.InsertOne(context.TODO(), data))
				fmt.Println("DealGamex3 success", objectId.Hex())
