...
func TestSimulate(t *testing.T) {
	...
	for i := 0; i < len(collNames); i++ {
		if strings.HasPrefix(collNames[i], "pgSpinData_") {
			name := collNames[i]
			go DealNormal(name)
		} else if strings.HasPrefix(collNames[i], "pgSpinDataGamex2_") {
			name := collNames[i]
			go DealGamex2(name, gendata.GameTypeGamex2)
		} else if strings.HasPrefix(collNames[i], "pgSpinDataGamex3_") {
			name := collNames[i]
			go DealGamex3(name, gendata.GameTypeGamex3)
		}
	}
	select {}
}
