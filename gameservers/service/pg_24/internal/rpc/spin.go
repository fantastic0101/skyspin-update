...
func init() {
	mux.RegRpc("/game-api/24/v2/Spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
	mux.RegRpc("/game-api/24/v2/spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
}
...
func spin(plr *models.Player, ps define.PGParams, ret *json.RawMessage) (err error) {
	...
	tmp, err := strconv.Atoi(bn)
	if err != nil {
		panic("Oops! bn 倍数转换失败！")
	}
	boundType := db.BoundType(tmp - 1)
	fmt.Println("=> trans bn tmp", boundType)
	buyMul := int64(tmp)
	...
	if id == "0" {
		...
		fmt.Println("(id != plr.LastSid) = ", id != plr.LastSid)
		fmt.Println("id = ", id)
		fmt.Println("plr.LastSid = ", plr.LastSid)
	...
	data := doc.Deal(num, ut.Gold2Money(balance), plr.Cs, plr.Ml, gendata.Line, float64(buyMul), false)
	...
	if ps.Header["jump_log"] == nil {
		detailsId = slotsmongo.InsertBetHistoryEvery(plr.BetHistLastID, plr.PID, 24, num, data, betId, plr.GetCurrencyOrTHB(), isEnd)
	}
