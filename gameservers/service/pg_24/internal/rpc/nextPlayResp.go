...
func nextPlayResp(player *models.Player, bet int64, selfPoolGold int64, boundType db.BoundType, App *redisx.AppStore) (playResp *gendata.SimulateData, hitBigAward, forcedKill bool, err error) {
	if false {
	...
	if player.NoAwardPercent != 0 {
		...
		if selfPoolGold < 0 && rand.IntN(1000) < player.NoAwardPercent {
			bucketId := 0
			if boundType == gendata.GameTypeGamex2 {
				bucketId = 9
			} else if boundType == gendata.GameTypeGamex3 {
				bucketId = 18
			}
			playResp, err = gendata.GCombineDataMng.SampleSimulate(bucketId, App.GamePatten, boundType)
			forcedKill = true
			return
		}
	} else { //走平台设置
	...
	if rand.IntN(1000) < gendata.HitBigAwardPercent {
		...
		if bet*10 < selfPoolGold {
			...
			if avg*10 < selfPoolGold {
				//兜底，极限情况只有一条数据
				for i := 0; i < 5; i++ {
					playResp, err = gendata.GCombineDataMng.GetBigReward(boundType, App.GamePatten)
	...
	for {
		playResp, err = gendata.GCombineDataMng.Next(fmt.Sprintf("%d", bet), boundType, App.GamePatten)
		//保底直接返回
