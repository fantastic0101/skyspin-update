...
func init() {
	...
	mux.RegRpc("/game-rtp-api/24/game/spinstat", "spinstat", "game-rtp-api", db.WrapRpcRtpPlayer(playStatistics), nil)
	mux.RegRpc("/game-rtp-api/24/game/querysimulate", "spinstat", "query-simulate", db.WrapRpcRtpPlayer(QuerySimulate), nil)
...
func QuerySimulate(plr *models.Player, ps define.PGParams, ret *json.RawMessage) (err error) {
	...
	filter := db.D("selected", true, "bucketid", bson.D{
		{"$gte", 0},
		{"$lte", 8},
	})
...
func playStatistics(plr *models.Player, ps define.PGParams, ret *json.RawMessage) (err error) {
	...
	postFrom.Bet = ps.Form.Get("Bet")
	fmt.Println("====>from get", postFrom.Bet)
