...
func normal(i int) {
	uid := fmt.Sprintf("1372643_tjh_%d", i)

	pinfo, err := pg.GetPlayerToken(uid, "1372643")
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	for ; ; time.Sleep(time.Second) {
		...
		if err := pg.InvokePGService(spinurl, ps, &spinret); err != nil {
			...
			fmt.Println("开始重试。。。")
			pinfo = lo.Must(pg.GetPlayerToken(uid, "1372643"))
			bsonDocs = []interface{}{}
			ps.Set("id", pinfo.Sid)
			ps.Set("atk", pinfo.Tk)
			continue
...
func game(i int) {
	uid := fmt.Sprintf("1372643_tjhGame_%d", i)
