...
func init() {
	...
	mux.RegRpc("/game-rtp-api/1372643/game/spinstat", "spinstat", "game-rtp-api", db.WrapRpcRtpPlayer(playStatistics), nil)
	mux.RegRpc("/game-rtp-api/1372643/game/querysimulate", "spinstat", "query-simulate", db.WrapRpcRtpPlayer(QuerySimulate), nil)
...
func GoRTP(ps ut.SpinStatPs, ret *ut.SpinStatRet, postFrom PostFrom) {
	...
	for i := 0; i < numThreads; i++ {
		...
		go func(threadID int) {
			...
			spinMsg.Form.Set("fb", postFrom.Fb)
...
func QuerySimulate(plr *models.Player, ps define.PGParams, ret *json.RawMessage) (err error) {
	...
	filterBuy := db.D("selected", true, "type", gendata.GameTypeGame)
	countbuy, err := coll.CountDocuments(context.TODO(), filterBuy)
	if err != nil {
		log.Fatal(err)
	}
