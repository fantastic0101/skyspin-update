...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_126")
...
func normal(i int) {
	uid := fmt.Sprintf("testccgg126_%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "126"))
...
func game(i int) {
	uid := fmt.Sprintf("testcggame126_%d", i)
	...
	for ; ; time.Sleep(time.Second) {
		...
		if err := pg.InvokePGService(spinurl, ps, &spinret); err != nil {
			slog.Error("spin buy game error", "error", err)
			continue
		}
