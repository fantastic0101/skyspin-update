...
import (
	...
	"serve/comm/db"
	"serve/service/pg_58/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_58")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_58").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("58hxy%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "58"))
