...
import (
	...
	"serve/comm/db"
	"serve/service/pg_105/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_105")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_105").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("105hxy%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "105"))
...
func game(i int) {
	uid := fmt.Sprintf("105hxygame%d", i)
