...
import (
	...
	"serve/comm/db"
	"serve/service/pg_59/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_59")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_59").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("mubanwc3%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "59"))
