...
import (
	...
	"serve/comm/ut"
	"serve/service/pg_1671262/internal/gendata"
...
func init() {
	mux.RegRpc("/game-api/1671262/v2/Spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
}
...
func spin(plr *models.Player, ps define.PGParams, ret *json.RawMessage) (err error) {
	...
	/*if len(plr.BdRecords) > 0 {
		...
		if ps.Header["jump_log"] == nil {
			detailsId = slotsmongo.InsertBetHistory(plr.PID, 1671262, plr.BdRecords, ids, plr.GetCurrencyOrTHB())
	...
	if ps.Header["jump_log"] == nil {
		slotsmongo.InsertBetHistoryEvery(plr.BetHistLastID, plr.PID, 1671262, num, data, betId, plr.GetCurrencyOrTHB(), isEnd)
