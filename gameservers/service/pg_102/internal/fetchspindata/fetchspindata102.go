...
import (
	...
	"serve/comm/db"
	"serve/service/pg_102/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_102")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_102").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("testwc11%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "102"))
