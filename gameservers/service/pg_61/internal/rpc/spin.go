...
import (
	...
	"serve/comm/ut"
	"serve/service/pg_61/internal/gendata"
	"serve/service/pg_61/internal/models"
...
func init() {
	mux.RegRpc("/game-api/61/v2/Spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
	mux.RegRpc("/game-api/61/v2/spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
}

// var spinLock sync.Mutex
//var fps_map map[int64][]string = map[int64][]string{}
...
func spin(plr *models.Player, ps define.PGParams, ret *json.RawMessage) (err error) {
	...
	idArr := strings.Split(id, "_")
	if len(idArr) == 3 && idArr[1] == idArr[2] {
		plr.FpChoose = []string{}
	}

	if fp != "" {
		plr.FpChoose = append(plr.FpChoose, fp)
	}
	...
	if isEnd {
		...
		//doc, hitBigAward, forcedKill, err = nextPlayResp(plr, bet, selfPoolGold, isBuy, App)
	...
	if ps.Header["jump_log"] == nil {
		detailsId = slotsmongo.InsertBetHistoryEvery(plr.BetHistLastID, plr.PID, 61, num, data, betId, plr.GetCurrencyOrTHB(), isEnd)
...
func hasGame(data map[string]interface{}) bool {
	fs := data["fs"]
	return fs != nil
}

func changePosition(data map[string]interface{}, plr *models.Player) map[string]interface{} {
	fs := data["fs"]
	if fs != nil {
		fsm := fs.(bson.M)
		fs_bf := fsm["bf"]
		if fs_bf != nil {
			fs_bf_arr := fs_bf.(primitive.A)
			//for k, v := range fps_map[pid] {
			for k, v := range plr.FpChoose {
				if k >= len(fs_bf_arr) {
					break
				}
				arr_item := fs_bf_arr[k]
				arr_item_m := arr_item.(bson.M)
				pos, _ := strconv.Atoi(v)
				arr_item_m["fp"] = pos
				fs_bf_arr[k] = arr_item_m
			}
			fsm["bf"] = fs_bf_arr
		}
	}
