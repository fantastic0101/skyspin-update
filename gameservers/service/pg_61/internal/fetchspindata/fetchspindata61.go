...
import (
	...
	"serve/comm/db"
	"serve/comm/ut"
	"serve/service/pg_61/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_61")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_61").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("61ccgg%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "61"))
	...
	for ; ; time.Sleep(time.Second) {
		...
		numOf_1 := getNoSelectPositionNum(bsondoc)
		if numOf_1 == 0 {
			ps.Del("fp")
		} else {
			ps.Set("fp", fmt.Sprintf("%v", numOf_1-1))
		}
