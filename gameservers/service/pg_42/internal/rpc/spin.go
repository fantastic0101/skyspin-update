...
import (
	...
	"serve/comm/ut"
	"serve/service/pg_42/internal/gendata"
	"serve/service/pg_42/internal/models"
...
func init() {
	mux.RegRpc("/game-api/42/v2/Spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
	mux.RegRpc("/game-api/42/v2/spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
}
...
func spin(plr *models.Player, ps define.PGParams, ret *json.RawMessage) (err error) {
	...
	/*if len(plr.BdRecords) > 0 {
		...
		//detailsId := slotsmongo.InsertBetHistory(plr.PID, 42, plr.BdRecords, ids, plr.GetCurrencyOrTHB())
	...
	//slotsmongo.InsertBetHistoryEvery(plr.BetHistLastID, plr.PID, 42, num, data, betId, plr.GetCurrencyOrTHB(), isEnd)
	if ps.Header["jump_log"] == nil {
		slotsmongo.InsertBetHistoryEvery(plr.BetHistLastID, plr.PID, 42, num, data, betId, plr.GetCurrencyOrTHB(), isEnd)
	}
