...
import (
	...
	"serve/comm/ut"
	"serve/service/pg_37/internal/gendata"
	"serve/service/pg_37/internal/models"
...
func init() {
	mux.RegRpc("/game-api/37/v2/Spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
	mux.RegRpc("/game-api/37/v2/spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
}
...
func spin(plr *models.Player, ps define.PGParams, ret *json.RawMessage) (err error) {
	...
	data := doc.Deal2(num, ut.Gold2Money(balance), plr.Cs, plr.Ml, gendata.Line, gendata.BuyMul, 0.9, false)
	...
	if ps.Header["jump_log"] == nil {
		detailsId = slotsmongo.InsertBetHistoryEvery(plr.BetHistLastID, plr.PID, 37, num, data, betId, plr.GetCurrencyOrTHB(), isEnd)
	}
