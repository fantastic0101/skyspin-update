...
import (
	...
	"serve/comm/db"
	"serve/service/pg_37/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_37")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_37").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("37tjh%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "37"))
