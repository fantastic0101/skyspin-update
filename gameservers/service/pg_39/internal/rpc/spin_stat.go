...
func init() {
	...
	mux.RegRpc("/game-rtp-api/39/game/spinstat", "spinstat", "game-rtp-api", db.WrapRpcRtpPlayer(playStatistics), nil)
	mux.RegRpc("/game-rtp-api/39/game/querysimulate", "spinstat", "query-simulate", db.WrapRpcRtpPlayer(QuerySimulate), nil)
...
func GoRTP(ps ut.SpinStatPs, ret *ut.SpinStatRet, postFrom PostFrom) {
	...
	for i := 0; i < numThreads; i++ {
		...
		go func(threadID int) {
			...
			spinMsg.Pid = pid
			err = gameinfo(player, spinMsg, &define.M{})
			if err != nil {
				slog.Error("game_info_err", "gameinfo", err)
			}
