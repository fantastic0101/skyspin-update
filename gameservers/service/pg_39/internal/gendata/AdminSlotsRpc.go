...
func init() {
	mux.RegRpc("/pg_39/AdminSlotsRpc/GetPoolStatus", "原始生成的数据", "gendata", GetPoolStatus, nil).SetOnlyDev()
}
...
func GetPoolStatus(_ mux.EmptyParams, ret *SlotsPoolStatusResp) (err error) {
	...
	for i, bound := range bounds {
		...
		u := RateDistributionStatus{
			...
			MinRate: bound.Min,
			MaxRate: bound.Max,
			// ExpectCount:  bound.Need,
			CurrentCount: statRet.List[i].Count,
		}
		if s := selectedStat[i]; s != nil {
			u.DstCount = s.Count
			// u.NotUseCombineCount = u.DstCount - u.UseCombineCount
			// if bound.Type == BoundTypeNormal {
			u.UseCombineCount = GCombineDataMng.getCombineUsedCount(i, 1)
			// }
			u.DstTimes = s.Times
