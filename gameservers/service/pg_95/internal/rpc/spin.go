...
import (
	...
	"serve/comm/ut"
	"serve/service/pg_95/internal/gendata"
...
func init() {
	mux.RegRpc("/game-api/95/v2/Spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
	mux.RegRpc("/game-api/95/v2/spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
}
...
func spin(plr *models.Player, ps define.PGParams, ret *json.RawMessage) (err error) {
	...
	//处理历史记录中rwsp数据，老数据格式不对string：float 替换为string：[]float
	if data["rwsp"] != nil {
		marshal, _ := json.Marshal(data["rwsp"])
		fmt.Println(string(marshal))
		oldMap := make(map[string]float64)
		json.Unmarshal(marshal, &oldMap)
	...
	if ps.Header["jump_log"] == nil {
		detailsId = slotsmongo.InsertBetHistoryEvery(plr.BetHistLastID, plr.PID, 95, num, data, betId, plr.GetCurrencyOrTHB(), isEnd)
