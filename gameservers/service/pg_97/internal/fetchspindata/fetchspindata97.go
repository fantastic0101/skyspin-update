...
import (
	...
	"serve/comm/db"
	"serve/comm/plats/pg"
	"serve/service/pg_97/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_97")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_97").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("97zw%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "97"))
