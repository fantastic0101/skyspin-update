...
import (
	...
	"serve/comm/db"
	"serve/comm/plats/pg"
	"serve/service/pg_88/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_88")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_88").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("88sj%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "88"))
