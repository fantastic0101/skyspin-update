...
import (
	...
	"serve/comm/db"
	"serve/comm/plats/pg"
	"serve/service/pg_121/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_121")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_121").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("121yy%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "121"))
