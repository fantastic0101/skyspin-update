...
import (
	...
	"serve/comm/ut"
	"serve/service/pg_121/internal/gendata"
	"serve/service/pg_121/internal/models"
...
func init() {
	mux.RegRpc("/game-api/121/v2/Spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
	mux.RegRpc("/game-api/121/v2/spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
}
...
func spin(plr *models.Player, ps define.PGParams, ret *json.RawMessage) (err error) {
	...
	if isEnd {
		...
		allWin := ut.GetFloat(lastPan["aw"]) * cs * ml * gendata.Line / 1.2
	...
	data := doc.Deal2(num, ut.Gold2Money(balance), plr.Cs, plr.Ml, gendata.Line, 1, 1.2, false)
	...
	if ps.Header["jump_log"] == nil {
		detailsId = slotsmongo.InsertBetHistoryEvery(plr.BetHistLastID, plr.PID, 121, num, data, betId, plr.GetCurrencyOrTHB(), isEnd)
	}
