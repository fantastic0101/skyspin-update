...
import (
	...
	"serve/comm/ut"
	"serve/service/pg_69/internal/gendata"
	"serve/service/pg_69/internal/models"
...
func init() {
	mux.RegRpc("/game-api/69/v2/Spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
	mux.RegRpc("/game-api/69/v2/spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
}
...
func spin(plr *models.Player, ps define.PGParams, ret *json.RawMessage) (err error) {
	...
	data := doc.Deal(num, ut.Gold2Money(balance), plr.Cs, plr.Ml, gendata.Line, 75, false)
	...
	/*if len(plr.BdRecords) > 0 {
		...
		if ps.Header["jump_log"] == nil {
			detailsId = slotsmongo.InsertBetHistory(plr.PID, 69, plr.BdRecords, ids, plr.GetCurrencyOrTHB())
	...
	detailsId := primitive.ObjectID{}
	if ps.Header["jump_log"] == nil {
		detailsId = slotsmongo.InsertBetHistoryEvery(plr.BetHistLastID, plr.PID, 69, num, data, betId, plr.GetCurrencyOrTHB(), isEnd)
