...
import (
	...
	"serve/comm/db"
	"serve/service/pg_38/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_38")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_38").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("38normal_SJ%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "38"))
