// CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o bin/pg_129 ./service/pg_129
package main
...
import (
	...
	"serve/service/pg_129/internal/config"
	"serve/service/pg_129/internal/gendata"
	_ "serve/service/pg_129/internal/rpc"
...
// 	httpAddr, _ := lazy.RouteFile.Get("pg_129.http.test")
// 	if len(httpAddr) != 0 {
...
func main() {
	...
	config.LoadConfig(func() {
		...
		if len(os.Args) > 1 && os.Args[1] == "cache" {
			slotsmongo.InitData("simulate", "pg_129")
			lo.Must0(gendata.LoadCombineData())
			os.Exit(0)
		}
		lo.Must0(gendata.LoadCombineData())
