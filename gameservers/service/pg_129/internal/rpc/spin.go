...
import (
	...
	"serve/comm/ut"
	"serve/service/pg_129/internal/gendata"
...
func init() {
	mux.RegRpc("/game-api/129/v2/Spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
	mux.RegRpc("/game-api/129/v2/spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
}
...
func spin(plr *models.Player, ps define.PGParams, ret *json.RawMessage) (err error) {
	...
	/*if len(plr.BdRecords) > 0 {
		...
		if ps.Header["jump_log"] == nil {
			detailsId = slotsmongo.InsertBetHistory(plr.PID, 129, plr.BdRecords, ids, plr.GetCurrencyOrTHB())
		}
	...
	detailsId := primitive.ObjectID{}
	if ps.Header["jump_log"] == nil {
		detailsId = slotsmongo.InsertBetHistoryEvery(plr.BetHistLastID, plr.PID, 129, num, data, betId, plr.GetCurrencyOrTHB(), isEnd)
	}
	plr.BetHistLastID = detailsId
