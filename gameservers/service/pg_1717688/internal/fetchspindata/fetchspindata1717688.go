...
import (
	...
	"serve/comm/db"
	"serve/comm/plats/pg"
	"serve/service/pg_1717688/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_1717688")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_1717688").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("1717688ttk%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "1717688"))
...
func game(i int) {
	uid := fmt.Sprintf("1717688ttkgame%d", i)
