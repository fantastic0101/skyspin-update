...
func init() {
	mux.RegRpc("/game-api/1717688/v2/Spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
}
...
func spin(plr *models.Player, ps define.PGParams, ret *json.RawMessage) (err error) {
	...
	data := doc.Deal3(num, ut.Gold2Money(balance), plr.Cs, plr.Ml, gendata.Line, gendata.BuyMul, 0.4, plr.IsBuy)
	...
	if ps.Header["jump_log"] == nil {
		slotsmongo.InsertBetHistoryEvery(plr.BetHistLastID, plr.PID, 1717688, num, data, betId, plr.GetCurrencyOrTHB(), isEnd)
