...
import (
	...
	"serve/comm/ut"
	"serve/service/pg_3/internal/gendata"
	"serve/service/pg_3/internal/models"
...
func init() {
	mux.RegRpc("/game-api/3/v2/spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
	mux.RegRpc("/game-api/3/v2/Spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
}
...
func spin(plr *models.Player, ps define.PGParams, ret *json.RawMessage) (err error) {
	...
	if isEnd {
		...
		//doc, hitBigAward, forcedKill, err = nextPlayResp(plr, bet, selfPoolGold, App)
		...
		allWin := ut.GetFloat(lastPan["aw"]) * cs * ml * gendata.Line / 0.9
		...
		plr.OnSpinFinish(bet, ut.Money2Gold(allWin), false, false, 1)
		// plr.SpinCount++
		// plr.SpinCountOfThisEnter++
		// plr.BetHistory = append(plr.BetHistory, bet)
		// plr.BetHistory = ut.MaxLen(plr.BetHistory, 200)
	} else {
		objectId := params[0]
		num, _ = strconv.Atoi(params[1])
	...
	data := doc.Deal2(num, ut.Gold2Money(balance), plr.Cs, plr.Ml, gendata.Line, 0, 0.9, false)
	...
	// 处理老数据 上线的游戏才需要这段代码
	/*if len(plr.BdRecords) > 0 {
		obet := ut.Money2Gold(plr.Ml * plr.Cs * gendata.Line)

		obig := plr.BigReward
		ids := make([]primitive.ObjectID, len(plr.BdRecords))
		for i := range plr.BdRecords {
			ids[i] = primitive.NewObjectID()
		}
		detailsId := primitive.ObjectID{}
		if ps.Header["jump_log"] == nil {
			detailsId = slotsmongo.InsertBetHistory(plr.PID, 3, plr.BdRecords, ids, plr.GetCurrencyOrTHB())
		}
		...
		for i := range plr.BdRecords {
			...
			tw := ut.Money2Gold(plr.BdRecords[i]["tw"].(float64))
			bl := ut.Money2Gold(plr.BdRecords[i]["bl"].(float64))
		...
		CurrencyKey:  plr.CurrencyKey,
				...
				Win:          tw,
				Balance:      bl,
				RoundID:      plr.BdRecords[i]["psid"].(string),
				Completed:    false,
				TotalWinLoss: 0,
				IsBuy:        false,
	...
	detailsId := primitive.ObjectID{}
	if ps.Header["jump_log"] == nil {
		detailsId = slotsmongo.InsertBetHistoryEvery(plr.BetHistLastID, plr.PID, 3, num, data, betId, plr.GetCurrencyOrTHB(), isEnd)
	}
	plr.BetHistLastID = detailsId
