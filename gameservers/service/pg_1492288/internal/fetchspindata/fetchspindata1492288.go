...
import (
	...
	"log/slog"
	"net/url"
	"serve/service/pg_1492288/internal/dealData"
	"strings"
	"sync"
	"time"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_1492288")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_1492288").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("1492288yy%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "1492288"))
...
func game(i int) {
	uid := fmt.Sprintf("1492288yygame%d", i)
