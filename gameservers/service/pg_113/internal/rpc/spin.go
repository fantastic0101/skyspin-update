...
import (
	...
	"serve/comm/ut"
	"serve/service/pg_113/internal/gendata"
	"serve/service/pg_113/internal/models"
...
func init() {
	mux.RegRpc("/game-api/113/v2/Spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
	mux.RegRpc("/game-api/113/v2/spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
}
...
func spin(plr *models.Player, ps define.PGParams, ret *json.RawMessage) (err error) {
	...
	if ps.Header["jump_log"] == nil {
		detailsId = slotsmongo.InsertBetHistoryEvery(plr.BetHistLastID, plr.PID, 113, num, data, betId, plr.GetCurrencyOrTHB(), isEnd)
	}
