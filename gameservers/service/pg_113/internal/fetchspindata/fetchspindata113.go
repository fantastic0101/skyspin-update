...
import (
	...
	"serve/comm/db"
	"serve/service/pg_113/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_113")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_113").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("113zw%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "113"))
...
func game(i int) {
	uid := fmt.Sprintf("113hxygame%d", i)
