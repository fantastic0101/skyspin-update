...
import (
	...
	"serve/comm/db"
	"serve/comm/plats/pg"
	"serve/service/pg_1648578/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_1648578")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_1648578").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("1648578ttk%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "1648578"))
...
func game(i int) {
	uid := fmt.Sprintf("1648578ttkgame%d", i)
