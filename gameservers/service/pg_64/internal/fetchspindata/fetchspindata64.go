...
import (
	...
	"serve/comm/db"
	"serve/comm/plats/pg"
	"serve/service/pg_64/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_64")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_64").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("64yy%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "64"))
