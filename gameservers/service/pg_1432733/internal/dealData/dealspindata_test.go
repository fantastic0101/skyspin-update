...
func TestSimulate(t *testing.T) {
	...
	db.DialToMongo(mongoaddr, "pg_1432733")

	collNames, _ := db.Client().Database("pg_1432733").ListCollectionNames(context.TODO(), bson.M{})
		...
		if pan["sid"].(string) == pan["psid"].(string) && len(pans) > 0 {
			if pans[0]["sid"].(string) == pans[0]["psid"].(string) { // 第一盘是否有效
				...
				data := gendata.SimulateData{
					...
					HasGame:  true,
					Times:    times,
					BucketId: gendata.GBuckets.GetBucket(times, true, db.BoundType(gendata.GameTypeGame)),
