...
import (
	...
	"serve/comm/db"
	"serve/comm/plats/pg"
	"serve/service/pg_63/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_63")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_63").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("wc%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "63"))
...
func normal2(i int) {
	uid := fmt.Sprintf("wcgame%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "63"))

	coll := db.Collection(fmt.Sprintf("pgSpinData2_%d", i))
	ps := url.Values{}
	ps.Add("cs", "1")
	ps.Add("ml", "1")
	ps.Add("pf", "1")
