...
import (
	...
	"serve/comm/ut"
	"serve/service/pg_63/internal/gendata"
	"serve/service/pg_63/internal/models"
...
func init() {
	mux.RegRpc("/game-api/63/v2/Spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
	mux.RegRpc("/game-api/63/v2/spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
}
...
func spin(plr *models.Player, ps define.PGParams, ret *json.RawMessage) (err error) {
	...
	if isEnd {
		...
		//_, err = slotsmongo.ModifyGold(plr.PID, -bet, "下注")
		...
		if err != nil {
			//slotsmongo.ModifyGold(plr.PID, bet, "下注-退回, err:"+err.Error())
	...
	if ps.Header["jump_log"] == nil {
		detailsId = slotsmongo.InsertBetHistoryEvery(plr.BetHistLastID, plr.PID, 63, num, newData, betId, plr.GetCurrencyOrTHB(), isEnd)
...
func dealTwiceData(pan map[string]any, gpt int, cs, ml, line float64) map[string]any {
	bet := cs * ml * line
	pan["gpt"] = gpt
	mrl := pan["mrl"]
	if mrl != nil {
