...
func init() {
	mux.RegRpc("/pg_63/AdminSlotsRpc/GetPoolStatus", "原始生成的数据", "gendata", GetPoolStatus, nil).SetOnlyDev()
}
...
func GetPoolStatus(_ mux.EmptyParams, ret *SlotsPoolStatusResp) (err error) {
	...
	for i, bound := range bounds {
		...
		if s := selectedStat[i]; s != nil {
			u.DstCount = s.Count
			u.UseCombineCount = GCombineDataMng.getCombineUsedCount(i, bound.Type, 1)
