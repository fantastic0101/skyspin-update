...
import (
	...
	"serve/comm/db"
	"serve/service/pg_114/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_114")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_114").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("mubanwc1%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "114"))
