...
func init() {
	mux.RegRpc("/game-api/50/v2/Spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
	mux.RegRpc("/game-api/50/v2/spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
}
...
func spin(plr *models.Player, ps define.PGParams, ret *json.RawMessage) (err error) {
	...
	if isEnd {
		...
		//_, err = slotsmongo.ModifyGold(plr.PID, -bet*buyMul, "下注")
		...
		if err != nil {
			//slotsmongo.ModifyGold(plr.PID, bet*buyMul, "下注-退回, err:"+err.Error())
	...
	if ps.Header["jump_log"] == nil {
		detailsId = slotsmongo.InsertBetHistoryEvery(plr.BetHistLastID, plr.PID, 50, num, data, betId, plr.GetCurrencyOrTHB(), isEnd)
	}
