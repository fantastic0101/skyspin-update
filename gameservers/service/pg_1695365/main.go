// CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o bin/pg_1695365 ./service/pg_1695365
package main
...
import (
	...
	"serve/service/pg_1695365/internal/config"
	"serve/service/pg_1695365/internal/gendata"
	_ "serve/service/pg_1695365/internal/rpc"
...
// 	httpAddr, _ := lazy.RouteFile.Get("pg_1695365.http.test")
// 	if len(httpAddr) != 0 {
...
func main() {
	...
	httpAddr, _ := lazy.RouteFile.Get("pg_1695365.http.test")
	fmt.Println("pg_1695365 api listen httpAddr:", httpAddr)
	if len(httpAddr) != 0 {
		mux.StartHttpServer(httpAddr)
	}
