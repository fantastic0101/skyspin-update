...
import (
	...
	"serve/comm/ut"
	"serve/service/pg_1695365/internal/gendata"
	"serve/service/pg_1695365/internal/models"
...
func init() {
	mux.RegRpc("/game-api/1695365/v2/Spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
}
...
func spin(plr *models.Player, ps define.PGParams, ret *json.RawMessage) (err error) {
	...
	data := gendata.DealMF(doc.Deal(num, ut.Gold2Money(balance), plr.Cs, plr.Ml, gendata.Line, gendata.BuyMul, plr.IsBuy))
	...
	/*if len(plr.BdRecords) > 0 {
		...
		if ps.Header["jump_log"] == nil {
			detailsId = slotsmongo.InsertBetHistory(plr.PID, 1695365, plr.BdRecords, ids, plr.GetCurrencyOrTHB())
	...
	if ps.Header["jump_log"] == nil {
		slotsmongo.InsertBetHistoryEvery(plr.BetHistLastID, plr.PID, 1695365, num, data, betId, plr.GetCurrencyOrTHB(), isEnd)
