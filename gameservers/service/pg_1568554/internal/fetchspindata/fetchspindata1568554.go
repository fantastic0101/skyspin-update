...
import (
	...
	"serve/comm/db"
	"serve/comm/plats/pg"
	"serve/service/pg_1568554/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_1568554")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_1568554").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("1568554normal_SJ%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "1568554"))
...
func game(i int) {
	uid := fmt.Sprintf("1568554game_SJ33%d", i)
