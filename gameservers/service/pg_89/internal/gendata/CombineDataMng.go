...
func findFromSimulate(id primitive.ObjectID) (playResp *SimulateData, err error) {
	one := &SimulateData{}
	err = db.Collection("simulate").FindOne(context.TODO(), db.ID(id), options.FindOne().SetProjection(bson.M{"droppan": 1, "times": 1})).Decode(&one)
	if err != nil {
		return
	}
	playResp = one
...
func (mng *CombineDataMng) GetBuyMinData() (playResp *SimulateData, err error) {
