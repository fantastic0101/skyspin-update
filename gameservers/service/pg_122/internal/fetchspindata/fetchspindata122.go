...
import (
	...
	"serve/comm/db"
	"serve/comm/plats/pg"
	"serve/service/pg_122/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_122")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_122").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("122yy%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "122"))
