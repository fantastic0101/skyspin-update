...
import (
	...
	"serve/comm/db"
	"serve/service/pg_108/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_108")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_108").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("108normal_SJ%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "108"))
...
func game(i int) {
	uid := fmt.Sprintf("108game_SJ%d", i)
