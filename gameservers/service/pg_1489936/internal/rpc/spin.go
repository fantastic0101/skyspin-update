...
import (
	...
	"serve/comm/ut"
	"serve/service/pg_1489936/internal/gendata"
...
func init() {
	mux.RegRpc("/game-api/1489936/v2/Spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
	mux.RegRpc("/game-api/1489936/v2/spin", "spin", "game-api", db.WrapRpcPlayer(spin), nil)
}
...
func spin(plr *models.Player, ps define.PGParams, ret *json.RawMessage) (err error) {
	...
	if isEnd {
		...
		if err != nil {
			...
			balance, _ = slotsmongo.GetBalance(plr.PID)
			money := ut.Gold2Money(balance)
			si["bl"] = money
			si["blb"] = money
			si["blab"] = money
			si["cs"] = cs
	...
	/*if len(plr.BdRecords) > 0 {
		...
		if ps.Header["jump_log"] == nil {
			detailsId = slotsmongo.InsertBetHistory(plr.PID, 1489936, plr.BdRecords, ids, plr.GetCurrencyOrTHB())
	...
	if ps.Header["jump_log"] == nil {
		slotsmongo.InsertBetHistoryEvery(plr.BetHistLastID, plr.PID, 1489936, num, data, betId, plr.GetCurrencyOrTHB(), isEnd)
