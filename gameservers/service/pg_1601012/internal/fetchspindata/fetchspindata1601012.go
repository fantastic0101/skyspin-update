...
import (
	...
	"serve/comm/db"
	"serve/comm/plats/pg"
	"serve/service/pg_1601012/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_1601012")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_1601012").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("1601012TJH%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "1601012"))
...
func game(i int) {
	uid := fmt.Sprintf("1601012TJHgame%d", i)
