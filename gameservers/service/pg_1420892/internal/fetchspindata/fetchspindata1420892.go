...
import (
	...
	"serve/comm/db"
	"serve/service/pg_1420892/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_1420892")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_1420892").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("1420892hxy111%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "1420892"))
...
func game(i int) {
	uid := fmt.Sprintf("1420892hxygame%d", i)
