...
import (
	...
	"serve/comm/db"
	"serve/comm/plats/pg"
	"serve/service/pg_1529867/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_1529867")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_1529867").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("1529867normal_SJ%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "1529867"))
...
func game(i int) {
	uid := fmt.Sprintf("1529867game_SJ%d", i)
