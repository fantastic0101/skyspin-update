...
import (
	...
	"serve/comm/db"
	"serve/comm/plats/pg"
	"serve/service/pg_1635221/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_1635221")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_1635221").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("1635221ttk%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "1635221"))
...
func game(i int) {
	uid := fmt.Sprintf("1635221ttkgame%d", i)
