...
import (
	...
	"serve/service/pg_1448762/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_1448762")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_1448762").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("1448762yy%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "1448762"))
...
func game(i int) {
	uid := fmt.Sprintf("1448762yygame%d", i)
