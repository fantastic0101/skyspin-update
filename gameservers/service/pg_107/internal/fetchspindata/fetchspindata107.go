...
import (
	...
	"serve/comm/db"
	"serve/service/pg_107/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_107")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_107").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("107hxy%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "107"))
...
func game(i int) {
	uid := fmt.Sprintf("107hxygame%d", i)
