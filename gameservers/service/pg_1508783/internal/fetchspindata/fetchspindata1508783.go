...
import (
	...
	"serve/comm/db"
	"serve/comm/plats/pg"
	"serve/service/pg_1508783/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_1508783")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_1508783").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("1508783tjh%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "1508783"))
...
func game(i int) {
	uid := fmt.Sprintf("1508783tjhgame%d", i)
