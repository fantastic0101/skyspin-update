...
func nextPlayResp(player *models.Player, bet int64, selfPoolGold int64, App *redisx.AppStore) (playResp *gendata.SimulateData, hitBigAward, forcedKill bool, err error) {
	if false {
		...
		cursor, err := coll.Aggregate(context.TODO(), bson.A{
			bson.D{{"$match", match}},                // 执行了筛选文档的操作 match代表筛选条件
			bson.D{{"$sample", bson.D{{"size", 1}}}}, // 执行了随机选择的操作 ，size = 1代表随机一个
		})
