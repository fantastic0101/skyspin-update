...
import (
	...
	"serve/comm/db"
	"serve/comm/plats/pg"
	"serve/service/pg_86/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_86")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_86").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("86normal_SJ%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "86"))
