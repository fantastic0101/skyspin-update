...
import (
	...
	"serve/comm/db"
	"serve/service/pg_36/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_36")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_36").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("mubanwc7%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "36"))
