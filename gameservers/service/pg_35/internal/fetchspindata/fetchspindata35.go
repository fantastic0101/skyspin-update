...
import (
	...
	"serve/comm/db"
	"serve/service/pg_35/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_35")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_35").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("35hxy%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "35"))
