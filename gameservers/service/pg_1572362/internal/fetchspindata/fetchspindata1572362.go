...
import (
	...
	"serve/comm/db"
	"serve/comm/plats/pg"
	"serve/service/pg_1572362/internal/dealData"
...
func main() {
	...
	db.DialToMongo(mongoaddr, "pg_1572362")
	...
	wg := sync.WaitGroup{}
	collNames, _ := db.Client().Database("pg_1572362").ListCollectionNames(context.TODO(), bson.M{})
...
func normal(i int) {
	uid := fmt.Sprintf("1572362yy%d", i)

	pinfo := lo.Must(pg.GetPlayerToken(uid, "1572362"))
...
func game(i int) {
	uid := fmt.Sprintf("1572362yygame%d", i)
