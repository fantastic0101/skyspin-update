...
import (
	...
	"fmt"
	"os"
	main2 "serve/servicepp/pp_vs50dmdcascade/internal"
	"testing"
	"time"
...
func TestFetchSpin(t *testing.T) {
	...
	db.DialToMongo(mongoaddr, main2.GameID)
...
func normal() {
	fetcher := ppcomm.NewFetcher(main2.GameID, "testusertjh_01")
	r := &robot.Robot{}
...
func game() {
	fetcher := ppcomm.NewFetcher(main2.GameID, "testusertjhgame_01")
	r := &robot.Robot{}
	ppR := ppcomm.NewPPRobot(r, fetcher, &ppcomm.FetchDataParam{
		Game: main2.GameID,
		ArrC: map[float64]int{ // 游戏下注挡位，k：下注值，v:权重
