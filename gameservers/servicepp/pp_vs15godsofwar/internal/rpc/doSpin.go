...
func doSpin(msg *nats.Msg) (ret []byte, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *ppcomm.Player) error {
		...
		god := internal.GetWitchGod()[ps.Int("ind")]
		...
		if isEnd {
			...
			if plr.IsBuy && internal.BuyBetMulti != 0 {
				mul = internal.GetFreeMultiply()[ps.Int("pur")]
			}
			...
			doc, hitBigAward, forcedKill, buyKill, err = ppcomm.NextPlayResp2(&ppcomm.NextPlayRespParam2{
				Player:                 plr,
				Bet:                    bet,
				AppStore:               App,
				...
				OptionsSlots:           god,                                     //选的是哪个神
				OptionsSlots2:          ps.Int("pur"),
			...
			plr.OnSpinFinish(costGold, ut.Money2Gold(allWin), plr.IsBuy, false, int(internal.GetFreeMultiply()[ps.Int("pur")]))
