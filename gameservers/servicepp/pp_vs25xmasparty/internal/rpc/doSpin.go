...
import (
	...
	"serve/servicepp/pp_vs25xmasparty/internal/gamedata"
	"serve/servicepp/pp_vs25xmasparty/internal/gendata"
	...
	err = db.CallWithPlayer(pid, func(plr *ppcomm.Player) error {
		...
		if isEnd {
			...
			if isBuy {
				...
				//}
				if isDouble {
					toSelfAwardPool += int64(float32(bet) * 1)
				}
			}
		...
		if doc.DropPan[0].Str("g") != "" {
			...
			if count == 1 {
				...
				if len(match) > 1 {
					...
					doc.DropPan[0].SetStr("trail", wins_mask)
				}
			}
		}
...
func winsMaskMaker(ind int, trail string) string {
	trailList := strings.Split(trail[2:], ",")
	needTrail := trailList[ind]
	trailList[ind] = trailList[0]
	trailList[0] = needTrail
	winsMask := trail[:2] + strings.Join(trailList, ",")
	return winsMask
}
