...
import (
	...
	"serve/comm/ut"
	"serve/servicepp/pp_vs20forge/internal"
	"serve/servicepp/pp_vs20forge/internal/gamedata"
	"serve/servicepp/pp_vs20forge/internal/gendata"
	...
	err = db.CallWithPlayer(pid, func(plr *ppcomm.Player) error {
		...
		if isEnd {
			...
			if pur == 1 {
				...
				minBuyFunc = gendata.GCombineDataMng.GetSuperBuyMinData3
				nextBuyFunc = gendata.GCombineDataMng.NextSuperBuy3
			} else if pur == 3 {
				minBuyFunc = gendata.GCombineDataMng.GetSuperBuyMinData4
				nextBuyFunc = gendata.GCombineDataMng.NextSuperBuy4
			}
		...
		if trail != "" {
			// 正则表达式匹配 tmb_win 后的值
			r := regexp.MustCompile(`(tmb_win~)(\d+\.\d{2})`)
			// 获取当前的小数值
			match := r.FindStringSubmatch(trail)
			currentValue := ""
			if len(match) > 2 {
				currentValue = match[2] // 获取原小数值
				fmt.Println("当前 tmb_win 的值:", currentValue)
				ff, _ := strconv.ParseFloat(currentValue, 64)
				multiply := c / originC
				// 替换逻辑
				trail = r.ReplaceAllString(trail, fmt.Sprintf("tmb_win~%.2f", ff*multiply))
