...
func init() {
	...
	mux.RegRpc("/game-rtp-api/pp_vs20forge/game/spinstat", "spinstat", "game-rtp-api", db.WrapRpcRtpPlayer(playStatistics), nil)
	mux.RegRpc("/game-rtp-api/pp_vs20forge/game/querysimulate", "spinstat", "query-simulate", db.WrapRpcRtpPlayer(QuerySimulate), nil)
	...
	for i := 0; i < numThreads; i++ {
		...
		go func(threadID int) {
			...
			if ps.BuyBonus {
				...
				if ps.BuyInt == 2 {
					params = fmt.Sprintf("%s&pur=%v", params, 2)
				}
				if ps.BuyInt == 3 {
					params = fmt.Sprintf("%s&pur=%v", params, 3)
				}
			}
			if postFrom.BuyEX == 1 {
				fmt.Println("buyEx==========", 1)
