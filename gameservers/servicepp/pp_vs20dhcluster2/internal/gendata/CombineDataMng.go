...
type CombineDataMng struct {
	...
	superBuyBucketMap1 [][]primitive.ObjectID
	superBuyBuckets1   []primitive.ObjectID
	superBuyBucketMap2 [][]primitive.ObjectID
	superBuyBuckets2   []primitive.ObjectID
	combine            ppcomm.Combine

	combineDatas            map[string]*ppcomm.CombineData
	combineDatas_buy        map[string]*ppcomm.CombineDataBuy
	combineDatas_super_buy1 map[string]*ppcomm.CombineDataBuy
	combineDatas_super_buy2 map[string]*ppcomm.CombineDataBuy
...
func (mng *CombineDataMng) NextSuperBuy1(bet string) (playResp *ppcomm.SimulateData, err error) {
	mng.mtx.Lock()
	data := mng.combineDatas_super_buy1[bet]
	if data == nil {
		data = &ppcomm.CombineDataBuy{
			RawSeries: mng.superBuyBuckets1,
		}
		mng.combineDatas_super_buy1[bet] = data
	}
	mng.mtx.Unlock()
...
func (mng *CombineDataMng) NextSuperBuy2(bet string) (playResp *ppcomm.SimulateData, err error) {
	mng.mtx.Lock()
	data := mng.combineDatas_super_buy2[bet]
	if data == nil {
		data = &ppcomm.CombineDataBuy{
			RawSeries: mng.superBuyBuckets2,
		}
		mng.combineDatas_super_buy2[bet] = data
	}
	mng.mtx.Unlock()
...
func (mng *CombineDataMng) GetSuperBuyMinData1() (playResp *ppcomm.SimulateData, err error) {
	bucketId := GetSuperBuyMinBucketId1()
	data := mng.superBuyBucketMap1[bucketId]
	id := lo.Sample(data)
	if id == primitive.NilObjectID {
		return nil, errors.New("not found")
	}
...
func (mng *CombineDataMng) GetSuperBuyMinData2() (playResp *ppcomm.SimulateData, err error) {
	bucketId := GetSuperBuyMinBucketId2()
	data := mng.superBuyBucketMap2[bucketId]
	id := lo.Sample(data)
	if id == primitive.NilObjectID {
		return nil, errors.New("not found")
	}
