...
import (
	...
	"serve/comm/ut"
	"serve/servicepp/pp_vs20dhcluster2/internal"
	"serve/servicepp/pp_vs20dhcluster2/internal/gamedata"
	"serve/servicepp/pp_vs20dhcluster2/internal/gendata"
...
func doSpin(msg *nats.Msg) (ret []byte, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *ppcomm.Player) error {
		...
		if isEnd {
			...
			// 当 pur 为 1，超级购买
			if pur == 1 {
				minBuyFunc = gendata.GCombineDataMng.GetSuperBuyMinData1
				nextBuyFunc = gendata.GCombineDataMng.NextSuperBuy1
			}
			if pur == 2 {
				minBuyFunc = gendata.GCombineDataMng.GetSuperBuyMinData2
				nextBuyFunc = gendata.GCombineDataMng.NextSuperBuy2
			}
