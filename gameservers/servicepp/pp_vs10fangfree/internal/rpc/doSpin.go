...
import (
	...
	"serve/comm/ut"
	"serve/servicepp/pp_vs10fangfree/internal"
	"serve/servicepp/pp_vs10fangfree/internal/gamedata"
	"serve/servicepp/pp_vs10fangfree/internal/gendata"
...
func doSpin(msg *nats.Msg) (ret []byte, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *ppcomm.Player) error {
		...
		//multiply := (int)(plr.C*internal.Line) / internal.MinBet // 倍数，所有与钱相关的都在入库的时候乘以了100转换为整数
		//multiply := plr.C * internal.Line / internal.MinBet
