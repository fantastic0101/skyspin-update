...
import (
	...
	"serve/comm/ut"
	"serve/servicepp/pp_vs20procount/internal"
	"serve/servicepp/pp_vs20procount/internal/gamedata"
	"serve/servicepp/pp_vs20procount/internal/gendata"
	...
	err = db.CallWithPlayer(pid, func(plr *ppcomm.Player) error {
		...
		if trail != "" {
			...
			if len(match) > 2 {
				...
				// 替换逻辑
				trail = r.ReplaceAllString(trail, fmt.Sprintf("tmb_win~%.2f", ff*plr.C*internal.Line))
