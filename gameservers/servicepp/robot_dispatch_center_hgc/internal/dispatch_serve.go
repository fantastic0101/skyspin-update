...
func RobotsDispatch() {
	...
	for ; ; time.Sleep(5 * time.Minute) {
		for _, mission := range GetMissionsHGC() {
			...
			go func() {
				...
				dispatchRobotsHGC(game, gameWeight)
				//if os.Getenv("GO_ENV") != "" {
...
func TasksDispatchHGC() {
	ticker := time.NewTicker(time.Minute * 5)
	for {
		<-ticker.C

		taskCur := db.Collection2("robot_hgc", "game")
		count, err := taskCur.CountDocuments(context.TODO(), bson.M{"status": 1, "isEnd": false})
		if err != nil {
			fmt.Println(err)
			return
