...
func FetchData(ps *FetchDataParam) {
	...
	for ; ; time.Sleep(30 * time.Second) {
		//数量够了就停止
		if robot.CheckGameEndHGC(ps.Game) || CheckGameEnoughHGC(ps.Game, ps.Ty) {
			break
		}
		robots := robot.GetRobotHGC(ps.Game)
		...
		for i := range robots {
			...
			fetcher := NewFetcherHGC(ps.Game, ps.ApiVersion, ps.HuiDuSerial, ps.bonusTy)
			if fetcher == nil {
				robotLock.Lock()
...
func (f *Fetcher) Do(ps ppcomm.Variables) ppcomm.Variables {
	...
	if f.game == "pp_vs40bigjuan" && f.lastResp.Int("counter")+1 == 2 && f.ty == ppcomm.GameTypeGame {
		ps.SetInt("pur", 0)
	} else if f.ty == ppcomm.GameTypeGame && f.game != "pp_vs40bigjuan" {
		ps.SetInt("pur", 0)
	...
	//slog.Info("robot_dispatch_center_hgc.DO", "ps", ps.Encode(),
	//	"user", f.user,
	//	"na", vars["na"],
