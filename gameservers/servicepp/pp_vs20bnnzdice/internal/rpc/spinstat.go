...
func init() {
	...
	mux.RegRpc("/game-rtp-api/pp_vs20bnnzdice/game/spinstat", "spinstat", "game-rtp-api", db.WrapRpcRtpPlayer(playStatistics), nil)
	mux.RegRpc("/game-rtp-api/pp_vs20bnnzdice/game/querysimulate", "spinstat", "query-simulate", db.WrapRpcRtpPlayer(QuerySimulate), nil)
...
func GoRTP(ps ut.SpinStatPs, ret *ut.SpinStatRet, postFrom PostFrom) {
	...
	for i := 0; i < numThreads; i++ {
		...
		go func(threadID int) {
			...
			if ps.BuyBonus {
				if ps.BuyInt == 0 {
					params = fmt.Sprintf("%s&fsp=%v", params, 0)
				}
				if ps.BuyInt == 1 {
					params = fmt.Sprintf("%s&fsp=%v", params, 1)
				}
			}
			if postFrom.BuyEX == 1 {
				fmt.Println("buyEx==========", 1)
