...
func init() {
	...
	mux.RegRpc("/game-rtp-api/pp_vs5aztecgems/game/spinstat", "spinstat", "game-rtp-api", db.WrapRpcRtpPlayer(playStatistics), nil)
	mux.RegRpc("/game-rtp-api/pp_vs5aztecgems/game/querysimulate", "spinstat", "query-simulate", db.WrapRpcRtpPlayer(QuerySimulate), nil)
...
func GoRTP(ps ut.SpinStatPs, ret *ut.SpinStatRet, postFrom PostFrom) {
	...
	wg.Wait()
	fmt.Println("END", time.Since(NewNow))
