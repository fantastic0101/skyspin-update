...
import (
	...
	"serve/servicepp/pp_vs9aztecgemsdx/internal/gamedata"
	"serve/servicepp/pp_vs9aztecgemsdx/internal/gendata"
...
func doSpin(msg *nats.Msg) (ret []byte, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *ppcomm.Player) error {
		...
		if isEnd {
			...
			findOptions.SetProjection(db.D("droppan", bson.M{"$slice": []int{0, num + 1}}))
			...
			oc := doc.DropPan[0].Float("c")
			doc.DropPan = doc.DropPan[len(doc.DropPan)-1:]
			doc.DropPan[0].SetFloat("c", oc)
		...
		//sampleDoSpin := ppcomm.ParseVariables(s)372*0.1/0.05
		...
		if msg.Header != nil && msg.Header.Get("isstat") == "1" && (data.Get("na") == "c" || data.Get("na") == "cb") {
