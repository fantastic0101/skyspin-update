...
import (
	...
	"serve/servicepp/pp_vs10bbdoubled/internal/gamedata"
...
func doSpin(msg *nats.Msg) (ret []byte, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *ppcomm.Player) error {
		...
		plrCs, err := redisx.GetPlayerCs(plr.AppID, plr.PID, plr.IsEnd)
		if err != nil {
		...
		// 倍数，所有与钱相关的都在入库的时候乘以了100转换为整数
		// 初始化使用上一轮下注金额，除非重新开始新的一轮。否则不信任前端的下注金额
		// 取数据库里面的值，触发免费的时候，需要使用数据库里面的值作为计算依据
		//multiply := (int)(plr.C*100) / internal.MinBet
		...
		if isEnd {
			...
			if plr.IsBuy && internal.BuyBetMulti != 0 {
				mul = float64(internal.GetFreeMultiply()[pur])
			}
			...
			////选取满足要求的RTP结果集
			//boundType := ppcomm.GameTypeNormal
			//if isBuy {
			//	boundType = ppcomm.GameTypeGame
			//} else if isDouble { //是否是双倍下注
			//	boundType = ppcomm.DoubleGame
			//}
			//if pur == 1 {
			//	boundType = ppcomm.GameTypeSuperGame1
			//}
			//if pur == 2 {
			//	boundType = ppcomm.GameTypeSuperGame2
			//}
			//presetRtp, ok := db.GRtpDataMgr.GetPresetRtp(plr.CurrentRtp, boundType)
			//targetMng := gendata.GCombineDataMng
			//if ok {
			//	if mng := gendata.GetMng(presetRtp, boundType); mng != nil {
			//		targetMng = mng
			//	}
			//}
			//
			////定义强杀函数
			//minNextFunc := gendata.GCombineDataMng.GetBuyMinData
			...
			//currentWinLose := float64(ut.Money2Gold(allWin)-costGold) / 10000
			//winMaxLimit := slotspool.GetMaxWin(plr.AppID, int64(float64(plr.GetWinLose())/10000), curItem.Multi)
			//if winMaxLimit != nil && currentWinLose > float64(winMaxLimit.MaxWin)*curItem.Multi {
			//	doc, err = gendata.GCombineDataMng.GetBuyMinData(boundType)
			//	if err != nil {
			//		//回滚下注信息
			//		_, _ = slotsmongo.ModifyGold(&slotsmongo.ModifyGoldPs{
			//			Pid:     plr.PID,
			...
			if isBuy {
				...
				//else {
				//	poolCost := ppcomm.GBuckets.GetPoolCost(doc.BucketId)
				//	if poolCost > 0 {
		...
		if len(doc.DropPan[0]) == 0 || doc.DropPan == nil {
			fmt.Println(doc.Id)
		}
		...
		if gid != "" {
			...
			if lastRound && plr.LastData.Str("na") != "c" { // 表示是最后一盘 "c" 还回执行collect操作
				isCompleted = true
			}
		}
