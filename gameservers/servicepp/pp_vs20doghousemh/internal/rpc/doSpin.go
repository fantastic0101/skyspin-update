...
import (
	...
	"serve/servicepp/pp_vs20doghousemh/internal/gamedata"
	"serve/servicepp/pp_vs20doghousemh/internal/gendata"
...
func doSpin(msg *nats.Msg) (ret []byte, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *ppcomm.Player) error {
		...
		if g != "" {
			// 编译正则表达式
			r := regexp.MustCompile(`(l\d\s*:\s*"\d+~)(\d+\.\d{1,2})`)
			if r.MatchString(g) {
				multiply := c / originC
				//fmt.Println(multiply)
				g = r.ReplaceAllStringFunc(g, func(match string) string {
					submatches := r.FindStringSubmatch(match)
					if len(submatches) > 2 {
						v := submatches[2] // 捕获组中的值
						if result, err := strconv.ParseFloat(v, 64); err == nil && result != 0.00 {
							replacement := fmt.Sprintf("%.2f", result*multiply)
