...
type PPRobot struct {
	...
	*Fetcher
	...
	bigJuanLast string //官方试玩 使用记录 na /s /c
}
...
func dispatchRobots(gameId string, weights int64) error {
	...
	robotCur := db.Collection2("robot", "robot")
...
func (ppr *PPRobot) Run() {
	...
	for {
		...
		default:
			...
			fmt.Println(ppr.lastResp.Str("frozen"))
			if ppr.lastResp.Str("frozen") == "An+error+has+occurred.+Please+restart+the+game." {
				continue
			}
		...
		if ppr.DealData() {
			...
			//}
			//数据够了结束
			if robot.CheckGameEnd(ppr.RpGameId) || CheckGameEnough(ppr.RpGameId, ppr.ty) {
				robotLock.Lock()
				delete(robotMap, ppr.ID.Hex())
				robotLock.Unlock()
				break
