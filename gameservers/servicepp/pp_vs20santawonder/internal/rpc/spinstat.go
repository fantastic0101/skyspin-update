...
func init() {
	...
	mux.RegRpc("/game-rtp-api/pp_vs20santawonder/game/spinstat", "spinstat", "game-rtp-api", db.WrapRpcRtpPlayer(playStatistics), nil)
	mux.RegRpc("/game-rtp-api/pp_vs20santawonder/game/querysimulate", "spinstat", "query-simulate", db.WrapRpcRtpPlayer(QuerySimulate), nil)
		...
		go func(threadID int) {
			...
			for i := 0; i < spinNum; i++ {
				...
				for {
					...
					//1-x doCollect 结束，下一轮
					if len(lastIds) > 1 && lastIds[1] == lastIds[2] {
