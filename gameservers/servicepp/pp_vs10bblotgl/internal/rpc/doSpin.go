...
import (
	...
	"serve/servicepp/pp_vs10bblotgl/internal/gamedata"
	"serve/servicepp/pp_vs10bblotgl/internal/gendata"
...
func doSpin(msg *nats.Msg) (ret []byte, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *ppcomm.Player) error {
		...
		//multiply := plr.C * internal.Line / internal.MinBet // 倍数，所有与钱相关的都在入库的时候乘以了100转换为整数
		...
		if isEnd {
			...
			//兼容各种超级购买,target不需要为了强杀，而特地初始化最小回报率购买集合
			minBuyFunc := gendata.GCombineDataMng.GetBuyMinData // 默认使用 GetBuyMinData
			nextBuyFunc := gendata.GCombineDataMng.NextBuy      // 默认使用 NextBuy
			// 当 pur 为 1，超级购买
			if pur == 1 {
				minBuyFunc = gendata.GCombineDataMng.GetSuperBuyMinData
				nextBuyFunc = gendata.GCombineDataMng.NextSuperBuy
			}
			...
			doc, hitBigAward, forcedKill, buyKill, err = ppcomm.NextPlayResp(&ppcomm.NextPlayRespParam{
				...
				MinBuyFunc:             minBuyFunc,                                    //杀！
				NextBuyFunc:            nextBuyFunc,                                   //购买游戏轮的次获取，包括首轮和后续轮次
			...
			// 金额数
			multiply := c * internal.Line / internal.MinBet
			allWin := ut.GetFloat(lastPan.Float("tw")) * float64(multiply)
			...
			if isBuy {
				...
				//}
				if isDouble {
					toSelfAwardPool += int64(float32(bet) * 0.50)
				}
			}
		...
		data := doc.Deal(plr.C, internal.Line, ut.Gold2Money(balance))
		if doc.DropPan[0].Str("g") != "" {
			g := doc.DropPan[0].Str("g")
			count := strings.Count(g, "ch_h")
			if count == 1 {
				// 正则表达式匹配 ch_h 的值
				re := regexp.MustCompile(`ch_h:"([^"]*?)"`)
				re2 := regexp.MustCompile(`status:"([^"]*?)"`)
				re3 := regexp.MustCompile(`wins_mask:"([^"]*?)"`)
				// 提取值
				match := re.FindStringSubmatch(g)
				match2 := re2.FindStringSubmatch(g)
				match3 := re3.FindStringSubmatch(g)
				trail := doc.DropPan[0].Get("trail")
				wins_mask := winsMaskMaker(ind, trail)
				g = strings.ReplaceAll(g, match3[0], fmt.Sprintf(`wins_mask:"%v"`, wins_mask))
				if len(match) > 1 {
					str := match[0]
					g = strings.ReplaceAll(g, str, fmt.Sprintf(`ch_h:"0~%v"`, ind))
					strList := make([]string, 12)
					for i := range strList {
						if i == ind {
							strList[i] = "1"
						} else {
					...
					g = strings.ReplaceAll(g, match2[0], fmt.Sprintf(`status:"%v"`, join))
					doc.DropPan[0].SetStr("g", g)
				}
			}
		}
...
func winsMaskMaker(ind int, trail string) string {
	winsMask := ""
	wins_masktempList := make([]string, 11)
	flag := "gl"
	if strings.Contains(trail, "0") { //鱼
		flag = "f"
		for i := 0; i < 11; i++ {
			if i < 2 {
				wins_masktempList[i] = "gl"
			} else {
				wins_masktempList[i] = "f"
			}
		}
	} else { // 人
		for i := 0; i < 11; i++ {
	...
	rand.Shuffle(len(wins_masktempList), func(i, j int) {
		wins_masktempList[i], wins_masktempList[j] = wins_masktempList[j], wins_masktempList[i] // 交换元素
	})
	wins_maskList := make([]string, 12)
	wins_maskList[ind] = flag
	num := 0
	for k := range wins_masktempList {
		if wins_maskList[num] == "" {
			wins_maskList[num] = wins_masktempList[k]
		} else {
			num++
			wins_maskList[num] = wins_masktempList[k]
		}
	...
	winsMask = strings.Join(wins_maskList, ",")
