...
import (
	...
	"serve/comm/ut"
	"serve/servicepp/pp_vswayschilheat/internal"
...
var (
	sampleCollect = ppcomm.ParseVariables(`balance=999,967.95&index=5&balance_cash=999,967.95&balance_bonus=0.00&na=s&stime=1726280350909&sver=5&counter=10&ntp=-1.60`)
)
...
func doCollect(msg *nats.Msg) (ret []byte, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *ppcomm.Player) error {
		...
		tw := ut.GetFloat(plr.LastData.Float("tw"))
		win := ut.Money2Gold(tw)
		...
		frbdata := plr.FRBData
		ppcomm.FRBOnRoundFinish(plr, info, tw)
		//sf := ut.NewSnowflake()
		//betId := strconv.Itoa(int(sf.NextID()))
		if msg.Header != nil && msg.Header.Get("jump_log") != "1" {
			ppcomm.InsertBetHistoryEvery(plr, 0, false, frbdata != nil, ps, info, curItem.Key, curItem.Symbol, 0, primitive.ObjectID{}, sampleDoInit.Encode(), "")
		}
		bonusCode := ""
		if frbdata != nil {
