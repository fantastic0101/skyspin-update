...
func init() {
	//mux.RegRpc("/pp_vswayschilheat/game/spinstat", "spin stat", "stat", spinstat, nil).SetOnlyDev()

	mux.RegRpc("/game-rtp-api/pp_vswayschilheat/game/spinstat", "spinstat", "game-rtp-api", db.WrapRpcRtpPlayer(playStatistics), nil)
	mux.RegRpc("/game-rtp-api/pp_vswayschilheat/game/querysimulate", "spinstat", "query-simulate", db.WrapRpcRtpPlayer(QuerySimulate), nil)
...
//	token := lo.Must(jwtutil.NewToken(ps.Pid, time.Now().Add(time.Hour*12)))
//	params := fmt.Sprintf("mgckey=%s&c=%v", token, ps.Bet)
//	if ps.BuyBonus {
//		params = fmt.Sprintf("%s&pur=%v", params, 1)
//	}
//	spinMsg := &nats.Msg{
...
//			spinMsg.Header.Set("stat_bet", "")
//			spinMsg.Header.Set("stat_win", "")
//			lastIds := strings.Split(lastId, "_")
//			if len(lastIds) != 3 || lastIds[1] == lastIds[2] {
//				break
//			}
//		}
//	}
