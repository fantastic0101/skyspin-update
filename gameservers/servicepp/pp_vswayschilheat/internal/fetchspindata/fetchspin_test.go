...
import (
	...
	"serve/servicepp/pp_vswayschilheat/internal/gendata"
...
func normal() {
	fetcher := ppcomm.NewFetcher(internal.GameID, "testuseryy_01")
	r := &robot.Robot{}
	...
	ppR.ID = primitive.NewObjectID()
	ppR.Run()
}

func game() {
	fetcher := ppcomm.NewFetcher(internal.GameID, "testuseryyg_01")
	r := &robot.Robot{}
	ppR := ppcomm.NewPPRobot(r, fetcher, &ppcomm.FetchDataParam{
		Game: internal.GameID,
		ArrC: map[float64]int{ // 游戏下注挡位，k：下注值，v:权重
