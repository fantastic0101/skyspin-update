...
import (
	...
	"serve/servicepp/pp_vs25pandatemple/internal/gamedata"
	"serve/servicepp/pp_vs25pandatemple/internal/gendata"
...
func doSpin(msg *nats.Msg) (ret []byte, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *ppcomm.Player) error {
		...
		gsf_a := doc.DropPan[0].Get("gsf_a")
		wp := doc.DropPan[0].Get("wp")
		data := doc.Deal2(plr.C, internal.Line, ut.Gold2Money(balance))
		data.Set("gsf_a", gsf_a)
		data.Set("wp", wp)
		data.Set("coef", strconv.FormatFloat(c*internal.Line, 'f', 2, 64))
