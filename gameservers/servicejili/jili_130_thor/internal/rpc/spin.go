...
func spin(pid int64, data []byte, ps *nats.Msg) (ret protoreflect.ProtoMessage, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		// 游戏内部数据
		thorData := doc.ThorData
		for _, roundInfo := range thorData.GetRoundQueue() {
			ut.FloatPtrMul(roundInfo.RoundWin, bet)
			ut.FloatPtrDiv(roundInfo.RoundWin, internal.BaseBet)
			for _, comboStageData := range roundInfo.ComboStageData {
				ut.FloatPtrMul(comboStageData.ComboStageWin, bet)
				ut.FloatPtrDiv(comboStageData.ComboStageWin, internal.BaseBet)
		...
		// for _, bi := range thorData.GetBonusInfo() {
		// 	for _, ar := range bi.Array {
		// 		ut.FloatPtrMul(ar.Win, bet)
		// 		ut.FloatPtrDiv(ar.Win, internal.BaseBet)

		// 	}
		// }
		// for _, bt := range thorData.BonusTotalWin {
		// 	ut.FloatPtrMul(&bt, bet)
		// 	ut.FloatPtrDiv(&bt, internal.BaseBet)
		// }
		ut.FloatPtrMul(thorData.TotalWin, bet)
		ut.FloatPtrDiv(thorData.TotalWin, internal.BaseBet)
		ut.FloatPtrMul(thorData.FreeTotalWin, bet)
		ut.FloatPtrDiv(thorData.FreeTotalWin, internal.BaseBet)

		//
		for i := 0; i < len(thorData.GetBonusInfo()); i++ {
			ar := thorData.GetBonusInfo()[i].Array
			thorData.GetBonusTotalWin()[i] = 0
			for j := 0; j < len(ar); j++ {
				ut.FloatPtrMul(ar[j].Win, bet)
				ut.FloatPtrDiv(ar[j].Win, internal.BaseBet)
				thorData.GetBonusTotalWin()[i] += ar[j].GetWin()
			}
		}
		//
		...
		// thorData.NowMoney = proto.Float64(ut.Gold2Money(balanceGold))
		jiliUtData.PostMoney = proto.Float64(ut.Gold2Money(balanceGold))
		jiliUtData.Data, _ = proto.Marshal(thorData)
		...
		his.PostMoney = ut.Ftoa(jiliUtData.GetPostMoney())
