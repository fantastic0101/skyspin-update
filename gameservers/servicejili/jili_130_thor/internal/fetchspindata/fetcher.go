...
import (
	...
	"serve/servicejili/jili_130_thor/internal/message"
...
func newFetcher(username string, coll *mongo.Collection, buy bool) *Fetcher {
	...
	launchUrl := lo.Must(jiliut.LaunchGame(username, "130", "en-US"))
	...
	f.gameReq = &jiliUtMessage.Server_Request{
		...
		Req: jiliut.ProtoEncode(&jiliUtMessage.Server_SpinReq{
			...
			MallBet:       lo.Ternary(buy, proto.Float64(97.5), nil),
		}),
	}
...
func (f *Fetcher) run() {
	...
	for ; ; time.Sleep(time.Second) {
		...
		var thorData message.Thor_SpinAck
		lo.Must0(proto.Unmarshal(spinResponse.Data, &thorData))
		...
		doc := &models.RawSpin{
			...
			Times:    totalWin / internal.BaseBet, // bet is 1.0 for csh
			Type:     lo.Ternary(f.buy, gendata.GameTypeGame, gendata.GameTypeNormal),
			HasGame:  len(thorData.GetBonusTotalWin()) > 0,
			ThorData: &thorData,
			UtData:   &spinResponse,
			Selected: true,
		}
