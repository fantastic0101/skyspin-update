...
func newFetcher(username string, coll *mongo.Collection, ty int) *Fetcher {
	...
	launchUrl := lo.Must(jiliut.LaunchGame(username, "223", "en-US"))
...
func (f *Fetcher) run() bool {
	...
	for ; ; time.Sleep(time.Second) {
		...
		var fgpData message.Fgp_SpinAck
		lo.Must0(proto.Unmarshal(spinResponse.Data, &fgpData))
		spinResponse.Data = []byte{}

		if fgpData.GetAckType() != 0 {
			slog.Error("pull spin data error", "acktype", fgpData.GetAckType(), "user", f.username)
			break
		}
