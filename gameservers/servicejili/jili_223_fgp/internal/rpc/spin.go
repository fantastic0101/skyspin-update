...
func spin(pid int64, data []byte, ps *nats.Msg) (ret protoreflect.ProtoMessage, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		// 游戏内部数据
		fgpData := doc.FgpData
		for _, roundInfo := range fgpData.GetAwardDataVec() {
			ut.FloatPtrMul(roundInfo.Win, bet)
		}
		ut.FloatPtrMul(fgpData.TotalWin, bet)
		ut.FloatPtrMul(fgpData.WheelWin, bet)
		// ut.FloatPtrMul(fgpData.WheelOdd, bet)
		ut.FloatPtrMul(fgpData.LineWin, bet)
		// ut.FloatPtrMul(fgpData.Bet, bet)
		// ut.FloatPtrMul(fgpData.WheelMult, bet)
		// ut.FloatPtrMul(fgpData.LineMult, bet)
		// 通用数据
		jiliUtData := doc.UtData
		ut.FloatPtrMul(jiliUtData.TotalWin, bet)

		//lo.Must0(ut.FloatEQ(spinAck.GetTotalWin(), totalWin))
		totalWin := fgpData.GetTotalWin()
		...
		fgpData.NowMoney = proto.Float64(ut.Gold2Money(balanceGold))
		jiliUtData.PostMoney = fgpData.NowMoney
		jiliUtData.Data, _ = proto.Marshal(fgpData)
		...
		his.PostMoney = ut.Ftoa(fgpData.GetNowMoney())
