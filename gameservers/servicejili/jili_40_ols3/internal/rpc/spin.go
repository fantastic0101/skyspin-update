...
func spin(ps *nats.Msg) (ret []byte, err error) {
	...
	var spinReq message.Ols3_SpinReq
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		for _, pd := range spinAck.AckQueue {
			ut.FloatPtrMul(pd.BothWin, bet)
			ut.FloatPtrDiv(pd.BothWin, internal.BaseBet)
			for _, ad := range pd.AwardDataVec {
				ut.FloatPtrMul(ad.Win, bet)
				ut.FloatPtrDiv(ad.Win, internal.BaseBet)
			}
			for _, pw := range pd.PlateWin {
				ut.FloatPtrMul(&pw, bet)
				ut.FloatPtrDiv(&pw, internal.BaseBet)
			}
		}
		...
		var spinAll = message.Ols3_SpinAllData{
			Info: []*message.Ols3_SpinAck{
				spinAck,
			}}
		encode, _ := proto.Marshal(&spinAll)

		var resData = message.Ols3_ResData{
			Type:  proto.Int32(AckType["spin"]),
			Token: proto.String(token),
			Data: []*message.Ols3_InfoData{
				{
					Encode: encode,
				},
			},
		...
		for j, sm := range doc.SingleRoundLogSummaries {
			if j == 0 {
