...
func newFetcher(username string, coll *mongo.Collection) *Fetcher {
	...
	launchUrl := lo.Must(jiliut.LaunchGame(username, "100", "en-US"))
...
func (f *Fetcher) run() bool {
	...
	for ; ; time.Sleep(time.Second) {
		...
		var srData tmp.Sr_SpinAck
		lo.Must0(proto.Unmarshal(spinResponse.Data, &srData))
		spinResponse.Data = []byte{}

		if srData.GetAckType() != 0 {
			slog.Error("pull spin data error", "acktype", srData.GetAckType(), "user", f.username, "buy", f.buy)
			break
		}
