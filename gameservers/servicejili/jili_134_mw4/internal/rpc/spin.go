...
import (
	...
	"regexp"
	"serve/comm/redisx"
	"serve/servicejili/jili_134_mw4/internal/message"
	"strconv"
	"time"
	...
	"serve/comm/define"
	"serve/comm/lazy"
	"serve/servicejili/jili_134_mw4/internal/gamedata"

	"serve/comm/db"
	"serve/comm/lang"
	"serve/comm/slotsmongo"
	"serve/comm/ut"
	"serve/servicejili/jili_134_mw4/internal"
	"serve/servicejili/jili_134_mw4/internal/models"
...
func spin(pid int64, data []byte, ps *nats.Msg) (ret protoreflect.ProtoMessage, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		doc, hitBigAward, forcedKill, buyKill, err = nextPlayResp(plr, gold, selfPoolGold, ty, App)
		if err != nil {
		...
		// 游戏内部数据
		mw4Data := doc.Mw4Data
		for _, roundInfo := range mw4Data.GetRoundQueue() {
			ut.FloatPtrMul(roundInfo.RoundWin, bet)
			for _, comboInfo := range roundInfo.GetComboStageData() {
				ut.FloatPtrMul(comboInfo.ComboStageWin, bet)

				for _, awardInfo := range comboInfo.GetAwardDataVec() {
					ut.FloatPtrMul(awardInfo.Win, bet)
				}
			}
		}
		//ut.FloatPtrMul(mw4Data.FreeTotalWin, bet)
		ut.FloatPtrMul(mw4Data.TotalWin, bet)
		...
		mw4Data.NowMoney = proto.Float64(ut.Gold2Money(balanceGold))
		jiliUtData.PostMoney = mw4Data.NowMoney
		jiliUtData.Data, _ = proto.Marshal(mw4Data)
		...
		his.PostMoney = ut.Ftoa(mw4Data.GetNowMoney())
		...
		for i, sm := range doc.SingleRoundLogSummaries {
			roundInfos := mw4Data.RoundQueue[i]

			plateInfos := hdoc.LogPlateInfos[sm.LogIndex]
			if len(plateInfos) != len(roundInfos.ComboStageData) {
				fmt.Println("doc.id", doc.ID.Hex())
				continue
			}
...
func getSymbolLength(stageInfo *message.Mw4_ComboStageInfo) (symbolLength string) {
