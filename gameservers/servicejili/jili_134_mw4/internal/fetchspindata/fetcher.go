...
import (
	...
	"serve/servicejili/jili_134_mw4/internal/gendata"
...
func newFetcher(username string, coll *mongo.Collection, ty int) *Fetcher {
	...
	launchUrl := lo.Must(jiliut.LaunchGame(username, "134", "en-US"))
	...
	f.gameReq = &jiliUtMessage.Server_Request{
		...
		Req: jiliut.ProtoEncode(&jiliUtMessage.Server_SpinReq{
			...
			MallBet: lo.Ternary(ty == internal.GameTypeGame, proto.Float64(46), nil),
		}),
	}
...
func (f *Fetcher) run() bool {
	...
	for ; ; time.Sleep(time.Second) {
		...
		var mw4Data message.Mw4_SpinAck
		lo.Must0(proto.Unmarshal(spinResponse.Data, &mw4Data))
		spinResponse.Data = []byte{}

		if mw4Data.GetAckType() != 0 {
			slog.Error("pull spin data error", "acktype", mw4Data.GetAckType(), "user", f.username)
			break
		}
		...
		doc := &models.RawSpin{
			...
			Type:     f.ty,
			HasGame:  len(mw4Data.GetRoundQueue()) > 1,
			Mw4Data:  &mw4Data,
			UtData:   &spinResponse,
			Selected: true,
		}
