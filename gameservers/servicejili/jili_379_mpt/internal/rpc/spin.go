...
func spin(pid int64, data []byte, ps *nats.Msg) (ret protoreflect.ProtoMessage, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		if extra == 1 {
			//baseBet = 1.5
			costGold = ut.Money2Gold(bet * 1.5)
			ty = internal.GameTypeExtra
		}
		if extra == 2 {
			//baseBet = 3
			costGold = ut.Money2Gold(bet * 3.0)
		...
		jiliUtData.Data, _ = proto.Marshal(mptData)
		...
		his.ExtraBet = ut.Ftoa(costMoney - bet)
