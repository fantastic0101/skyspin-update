...
import (
	...
	"math"
	"serve/comm/redisx"
	"serve/servicejili/jili_379_mpt/internal"
...
type CombineDataMng struct {
	...
	bucketExtrasPlus      map[int][][]primitive.ObjectID
	combineExtraPlusDatas map[string]*CombineData
}
...
func LoadCombineData() (err error) {
	...
	type bucketGroups struct {
		...
		ExtraPlus [][]primitive.ObjectID
	}

	bucketConfig := map[int]*bucketGroups{
		ppcomm.BucketHeartBeat:      {makeSlice(), makeSlice(), makeSlice()},
		ppcomm.BucketWave:           {makeSlice(), makeSlice(), makeSlice()},
		ppcomm.BucketGov:            {makeSlice(), makeSlice(), makeSlice()},
		ppcomm.BucketMix:            {makeSlice(), makeSlice(), makeSlice()},
		ppcomm.BucketStable:         {makeSlice(), makeSlice(), makeSlice()},
		ppcomm.BucketHighAward:      {makeSlice(), makeSlice(), makeSlice()},
		ppcomm.BucketSuperHighAward: {makeSlice(), makeSlice(), makeSlice()},
	}
	var (
	...
	for _, doc := range docs {
		...
		if doc.Type == ppcomm.GameTypeNormal || doc.Type == internal.GameTypeExtra || doc.Type == internal.GameTypeExtraPlus {
			handleBucket := func(bucketType int, config *bucketGroups, isOff bool) {
				if isOff {
					...
					if doc.Type == internal.GameTypeExtraPlus {
						config.ExtraPlus[doc.BucketID] = append(config.ExtraPlus[doc.BucketID], doc.ID)
					}
				}
			}
	...
	bucketExtraPlus := make(map[int][][]primitive.ObjectID, len(bucketConfig))
	for k, v := range bucketConfig {
		buckets[k] = v.normal
		bucketExtras[k] = v.Extra
		bucketExtraPlus[k] = v.ExtraPlus
	}
	...
	GCombineDataMng = &CombineDataMng{
		...
		bucketExtrasPlus:      bucketExtraPlus,
		combineExtraDatas:     map[string]*CombineData{},
		combineDatas_buy:      map[string]*CombineDataBuy{},
		combineExtraPlusDatas: map[string]*CombineData{},
	}
...
func (mng *CombineDataMng) Next(bet string, ty, WitchBucket int) (playResp *models.RawSpin, err error) {
	...
	if ty == internal.GameTypeExtraPlus {
		data = mng.getExtraPlus(bet)
	}

	id := data.next(ty, WitchBucket)
	//id, _ := primitive.ObjectIDFromHex("67f9ca2c2876867276b59886")
...
func (mng *CombineDataMng) getCombineUsedCount(bucketid int, ty int) int {
	var data *CombineData
	if ty == internal.GameTypeNormal {
		data = mng.get("")
...
func (mng *CombineDataMng) GetBigReward(ty, witchBucket int) (playResp *models.RawSpin, err error) {
	...
	if ty == internal.GameTypeExtraPlus {
		ids = append(ids, GetBucketIds(10, 20, true, ty)...) //todo 记得还原
		ids = lo.Filter(ids, func(item int, index int) bool {
			return len(mng.bucketExtrasPlus[witchBucket][item]) != 0
		})
		lo.Must0(len(ids) != 0)
		bucketid := lo.Sample(ids)
		data := mng.bucketExtrasPlus[witchBucket][bucketid]
		id = lo.Sample(data)
	}
...
func (mng *CombineDataMng) SampleForceSimulate(ty, witchBucket int) (playResp *models.RawSpin, err error) {
	...
	if ty == internal.GameTypeExtraPlus {
		data := mng.bucketExtrasPlus[witchBucket][47]
		id = lo.Sample(data)
	}
	return findFromSimulate(id)
}
...
func (mng *CombineDataMng) getExtraPlus(bet string) (data *CombineData) {
	mng.mtx.Lock()
	defer mng.mtx.Unlock()
	data = mng.combineExtraPlusDatas[bet]
	if data == nil {
		data = &CombineData{
			combine: mng.combine,
			buckets: mng.bucketExtrasPlus,
		}
		mng.combineExtraPlusDatas[bet] = data
	}
	return
}
...
func (mng *CombineDataMng) ControlNextData(nextData *redisx.NextMulti, gameType int, witchBucket int) (playResp *models.RawSpin, err error) {
	...
	if nextData != nil {
		...
		for attempt := 0; attempt < 20; attempt++ {
			...
			} else if gameType == GameTypeExtraPlus {
				tempIds := filterBucket(minMulti, maxMulti, gameType)
				ids = lo.Filter(tempIds, func(item int, index int) bool {
					return len(mng.bucketExtrasPlus[witchBucket][item]) != 0
				})
		...
		if len(ids) == 0 {
			if gameType == GameTypeNormal {
				...
				for id, bucket := range mng.bucketExtrasPlus[witchBucket] {
					if len(bucket) > 0 {
						ids = append(ids, id)
					}
				}
