...
func newFetcher(username string, coll *mongo.Collection, buy bool) *Fetcher {
	...
	launchUrl := lo.Must(jiliut.LaunchGame(username, "208", "en-US"))
...
func (f *Fetcher) run() bool {
	...
	for ; ; time.Sleep(time.Second) {
		...
		var phoenix_SpinAck tmp.Phoenix_SpinAck
		lo.Must0(proto.Unmarshal(spinResponse.Data, &phoenix_SpinAck))
		spinResponse.Data = []byte{}

		if phoenix_SpinAck.GetAckType() != 0 {
			slog.Error("pull spin data error", "acktype", phoenix_SpinAck.GetAckType(), "user", f.username, "buy", f.buy)
			break
		}
		...
		doc := &models.RawSpin{
			...
			HasGame:         len(phoenix_SpinAck.GetRoundQueue()) > 1,
			Phoenix_SpinAck: &phoenix_SpinAck,
			UtData:          &spinResponse,
			Selected:        true,
		}
