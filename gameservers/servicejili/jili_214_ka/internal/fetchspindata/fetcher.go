...
import (
	...
	"serve/servicejili/jili_214_ka/internal/message"
...
func newFetcher(username string, coll *mongo.Collection, ty int) *Fetcher {
	...
	launchUrl := lo.Must(jiliut.LaunchGame(username, "214", "en-US"))
...
func (f *Fetcher) run() bool {
	...
	for ; ; time.Sleep(time.Second) {
		...
		var kaData message.Ka_SpinAck
		lo.Must0(proto.Unmarshal(spinResponse.Data, &kaData))
		spinResponse.Data = []byte{}

		if kaData.GetAckType() != 0 {
			slog.Error("pull spin data error", "acktype", kaData.GetAckType(), "user", f.username)
			break
		}
		...
		doc := &models.RawSpin{
			...
			Type:  f.ty,
			//HasGame:  len(kaData.GetRoundQueue()) > 1,
			HasGame:  false,
			KaData:   &kaData,
			UtData:   &spinResponse,
			Selected: true,
