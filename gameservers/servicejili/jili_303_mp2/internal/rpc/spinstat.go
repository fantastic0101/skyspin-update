	...
	for i := 0; i < numThreads; i++ {
		...
		go func(threadID int) {
			...
			gameReq := &message.Mp2_GameReqData{
				Token: proto.String(lo.Must(jwtutil.NewToken(pid, time.Now().Add(time.Hour*12)))),
				Encode: jiliut.ProtoEncode(&message.Mp2_SpinReq{
					...
					Extra:   proto.Int32(int32(ps.Extra)),
				}),
			}
...
func spinstat(ps ut.SpinStatPs, ret *ut.SpinStatRet) (err error) {
	...
	gameReq := &message.Mp2_GameReqData{
		Token: proto.String(lo.Must(jwtutil.NewToken(ps.Pid, time.Now().Add(time.Hour*12)))),
		Encode: jiliut.ProtoEncode(&message.Mp2_SpinReq{
			// Bet:     proto.Float64(1),
			Bet:     proto.Float64(cmp.Or(ps.Bet, 1.0)),
			MallBet: proto.Float64(lo.Ternary(ps.BuyBonus, 1.0, 0.0)),
