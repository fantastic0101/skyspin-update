...
func gameinfo(ps *nats.Msg) (ret []byte, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		// aid := int32(pid)
		// var ack = message.Mp2_LoginDataAck{
		// 	Aid: &aid,
		// }
		...
		var ack = message.Mp2_GameInfoAck{
			ArcadeNo:      proto.Int32(1517216),
			CanIntoArcade: proto.Bool(true),
			Mul:           []float64{1, 1.5},
			Prefer:        []*message.Mp2_PreferRoundShow{},
			ShowExtra:     proto.Bool(true),
			Test:          proto.Bool(true),
			WalletInfo: []*message.Mp2_WalletInfo{
				{
