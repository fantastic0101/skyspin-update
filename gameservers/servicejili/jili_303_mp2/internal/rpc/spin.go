...
func spin(ps *nats.Msg) (ret []byte, err error) {
	var gameReq message.Mp2_GameReqData
	...
	var spinReq message.Mp2_SpinReq
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		spinAck := doc.Data
		// 处理数据
		for _, round := range spinAck.AwardDataVec {
			ut.FloatPtrMul(round.Win, baseBet)
		}
		...
		var spinAll = message.Mp2_SpinAllData{
			Info: []*message.Mp2_SpinAck{
				spinAck,
			}}
		encode, _ := proto.Marshal(&spinAll)
		var resData = message.Mp2_ResData{
			Type:  proto.Int32(AckType["spin"]),
			Token: proto.String(token),
			Data: []*message.Mp2_InfoData{
				{
					Encode: encode,
				},
			},
