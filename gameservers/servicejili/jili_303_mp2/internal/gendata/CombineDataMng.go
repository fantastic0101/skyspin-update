...
func (mng *CombineDataMng) GetBigReward(ty, witchBucket int) (playResp *models.RawSpin, err error) {
	ids := []int{}
	//ids = append(ids, GetBucketIds(30, 50, true, ty)...) //todo 记得还原
	...
	if ty == internal.GameTypeExtra {
		ids = lo.Filter(ids, func(item int, index int) bool {
			return len(mng.bucketExtra[witchBucket][item]) != 0
		})
		lo.Must0(len(ids) != 0)
		bucketid := lo.Sample(ids)
		data := mng.bucketExtra[witchBucket][bucketid]
		id = lo.Sample(data)
	}
	return findFromSimulate(id)
}
...
func (mng *CombineDataMng) SampleForceSimulate(ty, witchBucket int) (playResp *models.RawSpin, err error) {
	...
	if ty == GameTypeExtra {
		data := mng.bucketExtra[witchBucket][18]
		id = lo.Sample(data)
	}
	return findFromSimulate(id)
}
	...
	if nextData != nil {
		...
		if len(ids) == 0 {
			if gameType == GameTypeNormal {
				...
				for id, bucket := range mng.bucketExtra[witchBucket] {
					if len(bucket) > 0 {
						ids = append(ids, id)
					}
				}
