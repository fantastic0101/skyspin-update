...
func spin(pid int64, data []byte, ps *nats.Msg) (ret protoreflect.ProtoMessage, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		isBuy := mallBet != 0
		mul := []float64{40, 57, 78.5}
		if isBuy {
			ty = internal.GameTypeGame
			if spinReq.Mall.Index == 1 {
				ty = internal.GameTypeGamePlus
			} else if spinReq.Mall.Index == 2 {
				ty = internal.GameTypeGamePPlus
			}
			costGold = ut.Money2Gold(bet * mul[spinReq.Mall.Index])
		}
		...
		for _, roundInfo := range tlpData.GetRoundQueue() {
			ut.FloatPtrMul(&roundInfo.RoundWin, betMul)
			for _, freePlate := range roundInfo.GetFreePlateSymbol() {
				for _, ccolumn := range freePlate.GetCol() {
					if ccolumn.Number != 0 {
						ut.FloatPtrMul(&ccolumn.Number, betMul)
					}
				}
			}
			for _, freePlateLog := range roundInfo.GetFreePlateSymbolLog() {
				for _, ccolumnLog := range freePlateLog.GetCol() {
					if ccolumnLog.Number != 0 {
						ut.FloatPtrMul(&ccolumnLog.Number, betMul)
					}
				}
			}
			for _, blue := range roundInfo.GetBlueData() {
				ut.FloatPtrMul(&blue.Number, betMul)
			}
			for _, green := range roundInfo.GetGreenData() {
				ut.FloatPtrMul(&green.Number, betMul)
			}
			for _, red := range roundInfo.GetRedData() {
				ut.FloatPtrMul(&red.Number, betMul)
			}
			for _, awardInfo := range roundInfo.GetAwardDataVec() {
				ut.FloatPtrMul(&awardInfo.Win, betMul)
			}
		}
		ut.FloatPtrMul(&tlpData.TotalWin, betMul)
		ut.FloatPtrMul(&tlpData.FreeTotalWin, betMul)
		ut.FloatPtrMul(&jiliUtData.TotalWin, betMul)

		//lo.Must0(ut.FloatEQ(spinAck.GetTotalWin(), totalWin))
		totalWin := tlpData.GetTotalWin()
		...
		jiliUtData.Data, _ = proto.Marshal(tlpData)
		...
		jiliUtData.RealBet = ut.Gold2Money(costGold)
		...
		for _, pinfos := range doc.LogPlateInfos {
			for _, pinfo := range pinfos {
				...
				for _, v := range pinfo.PlateNumList {
					ut.FloatPtrMul(&v.Number, betMul)
				}
