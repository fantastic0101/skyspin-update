...
func newFetcher(username string, coll *mongo.Collection, buy bool) *Fetcher {
	...
	launchUrl := lo.Must(jiliut.LaunchGame(username, "191", "en-US"))
	...
	f.gameReq = &jiliUtMessage.Server_Request{
		...
		Req: jiliut.ProtoEncode(&jiliUtMessage.Server_SpinReq{
			...
			Bet:           proto.Float64(1),
			MallBet:       lo.Ternary(buy, proto.Float64(43.5), nil),
		}),
	}
...
func (f *Fetcher) run() {
	...
	for ; ; time.Sleep(time.Second) {
		...
		var petraData message.Petra_SpinAck
		lo.Must0(proto.Unmarshal(spinResponse.Data, &petraData))
		spinResponse.Data = []byte{}

		if petraData.GetAckType() != 0 {
			slog.Error("pull spin data error", "acktype", petraData.GetAckType(), "user", f.username, "buy", f.buy)
			break
		}
		...
		doc := &models.RawSpin{
			...
			HasGame:   len(petraData.GetRoundQueue()) > 1,
			PetraData: &petraData,
			UtData:    &spinResponse,
			Selected:  true,
		}
