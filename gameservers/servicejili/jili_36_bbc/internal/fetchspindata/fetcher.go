...
import (
	...
	"serve/servicejili/jili_36_bbc/internal/gendata"
...
type Fetcher struct {
	...
	launchUrl    *url.URL
	spinUrl      *url.URL
	gameReq      *message.Bbc_GameReqData
	historyToken string
}
...
func newFetcher(username string, coll *mongo.Collection, buy bool) *Fetcher {
	...
	var loginDataReq = message.Bbc_LoginDataReq{
	...
	var gameloginResData message.Bbc_ResData
	...
	f.gameReq = &message.Bbc_GameReqData{
		Token: proto.String(token),
		Aid:   proto.Int32(tokenST.Profile.Aid),
		Apiid: proto.Int32(tokenST.Profile.ApiId),
		// Encode: lo.Must(proto.Marshal(&spinReq)),
		Encode: jiliut.ProtoEncode(&message.Bbc_SpinReq{
			Bet:     proto.Float64(1),
			MallBet: lo.Ternary(buy, proto.Float64(46.2), nil),
...
func (f *Fetcher) run() {
	...
	for {
		...
		var spinResData message.Bbc_ResData
		...
		hasGame := len(spinAck.PlateInfo[0].FreeGamePackage[0].FreeGamePlateInfo) > 0
		doc := &models.RawSpin{
			ID:    primitive.NewObjectID(),
			Times: totalWin / internal.BaseBet,
			Type:  lo.Ternary(f.buy, gendata.GameTypeGame, gendata.GameTypeNormal),
			//todo
			//HasGame:  len(spinAck.BonusInfo) > 1,
			HasGame:  hasGame,
			Data:     spinAck,
			Selected: true,
		}
