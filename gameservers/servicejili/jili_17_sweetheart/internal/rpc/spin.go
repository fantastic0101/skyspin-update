...
func spin(pid int64, data []byte, ps *nats.Msg) (ret protoreflect.ProtoMessage, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		// 游戏内部数据
		sweetheartData := doc.SweetheartData
		betMul := bet / internal.BaseBet
		for _, roundInfo := range sweetheartData.GetPlate() {
			for _, award2 := range roundInfo.Award {
				award2.Win = award2.GetWin() * betMul
			}
			if roundInfo.GetWin() != 0 {
				roundInfo.Win = roundInfo.GetWin() * betMul
			}
			if roundInfo.GetBonusWin() != 0 {
				roundInfo.BonusWin = roundInfo.GetBonusWin() * betMul
			}
		}
		...
		jiliUtData.Data, _ = proto.Marshal(sweetheartData)
		ret = jiliUtData
		...
		for _, pinfos := range doc.LogPlateInfos {
			for _, pinfo := range pinfos {
				...
				for _, v := range pinfo.BonusArray {
					ut.FloatStrPtrMul(v, betMul)
				}
			}
		}
