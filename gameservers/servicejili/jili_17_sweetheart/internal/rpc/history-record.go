...
func init() {
	jiliut.RegRpc(fmt.Sprintf("/history/%s/get-history-record", `sb`), get_history_record)

	jiliut.RegRpc(fmt.Sprintf("/history/%s/get-single-round-log-summary/", `sb`), get_single_round_log_summary)

	jiliut.RegRpc(fmt.Sprintf("/history/%s/get-log-plate-info/", `sb`), get_log_plate_info)
}
