...
var winReg3 = regexp.MustCompile(`^(Respin Win: )([0-9.+-]+)$`)
...
func spin(pid int64, data []byte, ps *nats.Msg) (ret protoreflect.ProtoMessage, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		ut.FloatPtrMul(&scData.TotalWin, betMul)
		for _, roundInfo := range scData.GetRoundQueue() {
		...
		jiliUtData.Data, _ = proto.Marshal(scData)
		...
		for _, sm := range doc.SingleRoundLogSummaries {
			for i, desc := range sm.Desc {
				...
				rets3 := winReg3.FindStringSubmatch(desc)
				if len(rets3) == 3 {
					winstr := rets3[2]
					ut.FloatStrPtrMul(&winstr, betMul)
					ret := rets3[1] + winstr
