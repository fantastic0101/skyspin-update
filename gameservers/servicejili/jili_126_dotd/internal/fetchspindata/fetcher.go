...
import (
	...
	"serve/servicejili/jili_126_dotd/internal/message"
...
func newFetcher(username string, coll *mongo.Collection, buy bool) *Fetcher {
	...
	launchUrl := lo.Must(jiliut.LaunchGame(username, "126", "en-US"))
	...
	f.gameReq = &jiliUtMessage.Server_Request{
		...
		Req: jiliut.ProtoEncode(&jiliUtMessage.Server_SpinReq{
			...
			Bet:           proto.Float64(1),
			MallBet:       lo.Ternary(buy, proto.Float64(48.5), nil),
		}),
	}
...
func (f *Fetcher) run() {
	...
	for ; ; time.Sleep(time.Second) {
		...
		var dotdData message.Dotd_SpinAck
		lo.Must0(proto.Unmarshal(spinResponse.Data, &dotdData))
		spinResponse.Data = []byte{}

		if dotdData.GetAckType() != 0 {
			slog.Error("pull spin data error", "acktype", dotdData.GetAckType(), "user", f.username, "buy", f.buy)
			break
		}
		...
		doc := &models.RawSpin{
			...
			HasGame:  len(dotdData.GetRoundQueue()) > 1,
			DotdData: &dotdData,
			UtData:   &spinResponse,
			Selected: true,
		}
