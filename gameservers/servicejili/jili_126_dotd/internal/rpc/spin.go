...
import (
	...
	"serve/servicejili/jili_126_dotd/internal/gamedata"
...
var winReg2 = regexp.MustCompile(`^(Respin Win: )([0-9.+-]+)$`)
...
func spin(pid int64, data []byte, ps *nats.Msg) (ret protoreflect.ProtoMessage, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		// 游戏内部数据
		dotdData := doc.DotdData
		for _, roundInfo := range dotdData.GetRoundQueue() {
			ut.FloatPtrMul(roundInfo.RoundWin, bet)
			for _, awardData := range roundInfo.AwardDataVec {
				ut.FloatPtrMul(awardData.Win, bet)
			}
		}
		ut.FloatPtrMul(dotdData.TotalWin, bet)
		ut.FloatPtrMul(dotdData.FreeTotalWin, bet)
		// 通用数据
		jiliUtData := doc.UtData
		ut.FloatPtrMul(jiliUtData.TotalWin, bet)

		//lo.Must0(ut.FloatEQ(spinAck.GetTotalWin(), totalWin))
		totalWin := dotdData.GetTotalWin()
		...
		dotdData.NowMoney = proto.Float64(ut.Gold2Money(balanceGold))
		jiliUtData.PostMoney = dotdData.NowMoney
		jiliUtData.Data, _ = proto.Marshal(dotdData)
		...
		his.PostMoney = ut.Ftoa(dotdData.GetNowMoney())
