...
func newFetcher(username string, coll *mongo.Collection, buy bool) *Fetcher {
	...
	launchUrl := lo.Must(jiliut.LaunchGame(username, "35", "en-US"))
...
func (f *Fetcher) run() {
	...
	for ; ; time.Sleep(time.Second) {
		...
		var ols2Data message.Ols2_SpinAck
		lo.Must0(proto.Unmarshal(spinResponse.Data, &ols2Data))
		spinResponse.Data = []byte{}

		if ols2Data.GetAckType() != 0 {
			slog.Error("pull spin data error", "user", f.username, "buy", f.buy)
			break
		}

		totalWin := ols2Data.GetTotalWin()
