...
func spin(ps *nats.Msg) (ret []byte, err error) {
	var gameReq message.Me_GameReqData
	...
	var spinReq message.Me_SpinReq
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		for i := range spinAck.BonusTotalWin {
			spinAck.BonusTotalWin[i] *= baseBet
		}
		...
		var spinAll = message.Me_SpinAllData{
			Info: []*message.Me_SpinAck{
				spinAck,
			}}
		encode, _ := proto.Marshal(&spinAll)

		var resData = message.Me_ResData{
			Type:  proto.Int32(AckType["spin"]),
			Token: proto.String(token),
			Data: []*message.Me_InfoData{
				{
					Encode: encode,
				},
			},
