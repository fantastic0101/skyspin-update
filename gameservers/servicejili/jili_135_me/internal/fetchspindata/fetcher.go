...
import (
	...
	"serve/servicejili/jili_135_me/internal/gendata"
...
type Fetcher struct {
	...
	launchUrl    *url.URL
	spinUrl      *url.URL
	gameReq      *message.Me_GameReqData
	historyToken string
}
...
func newFetcher(username string, coll *mongo.Collection, ty int) *Fetcher {
	...
	var loginDataReq = message.Me_LoginDataReq{
	...
	var gameloginResData message.Me_ResData
	...
	f.gameReq = &message.Me_GameReqData{
		Token: proto.String(token),
		Aid:   proto.Int32(tokenST.Profile.Aid),
		Apiid: proto.Int32(tokenST.Profile.ApiId),
		// Encode: lo.Must(proto.Marshal(&spinReq)),
		Encode: jiliut.ProtoEncode(&message.Me_SpinReq{
			Bet:      proto.Float64(1),
			Currency: proto.Int32(0),
			Free:     proto.Int32(0),
			VipLv:    proto.Int32(0),
			Extra:    lo.Ternary(ty == internal.GameTypeExtra, proto.Int32(1), proto.Int32(0)),
			MallBet:  lo.Ternary(ty == internal.GameTypeBuy, proto.Float64(58.5), nil),
			MallID:   lo.Ternary(ty == internal.GameTypeBuy, proto.Int32(50), nil),
		}),
	}
	return f
}
...
func (f *Fetcher) run() {
	...
	for ; ; time.Sleep(time.Second) {
		var spinResData message.Me_ResData
		...
		var spinAll message.Me_SpinAllData
		...
		if spinAck.GetAckType() != 0 {
			slog.Error("pull spin data error", "acktype", spinAck.GetAckType(), "user", f.username, "ty", f.ty)
			break
		}
		...
		doc := &models.RawSpin{
			...
			Type:     f.ty,
			HasGame:  len(spinAck.GetRoundQueue()) > 1 || f.ty == internal.GameTypeBuy,
			Data:     spinAck,
			Selected: true,
		}
