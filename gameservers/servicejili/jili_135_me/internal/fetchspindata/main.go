...
func main() {
	...
	for i := 0; i < 5; i++ {
		go func(id int) {
			var num int
			for {
				newFetcher(fmt.Sprintf("%d_wc123456_%d", internal.GameNo, id), coll, internal.GameTypeNormal).run()
		...
		go func(id int) {
			var num int
			for {
				newFetcher(fmt.Sprintf("%d_wcextra_123456_%d", internal.GameNo, id), coll, internal.GameTypeExtra).run()
