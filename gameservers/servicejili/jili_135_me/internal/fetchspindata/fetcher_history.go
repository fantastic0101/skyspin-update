...
func (f *Fetcher) fetchHisotry(roundIndex string, doc *models.RawSpin) {
	...
	platcomm.Retry(20, 3*time.Second, func() bool {
		lo.Must0(f.invokeHistory("/history/me/get-history-record", historyRecodeQuery.Encode(), &records))
	...
	// https://history-api.jlfafafa3.com/history/me/get-single-round-log-summary/en-US/1718339075129795002

	var summaries []*models.SingleRoundLogSummary
	lo.Must0(f.invokeHistory("/history/me/get-single-round-log-summary/en-US/"+roundIndex, "", &summaries))
	...
	for _, summary := range summaries {
		...
		// https://history-api.jlfafafa3.com/history/me/get-log-plate-info/1718339075129795002/1718339075129795002

		path := fmt.Sprintf("/history/me/get-log-plate-info/%s/%s", roundIndex, summary.LogIndex)
