...
func newFetcher(username string, coll *mongo.Collection, buy bool) *Fetcher {
	...
	launchUrl := lo.Must(jiliut.LaunchGame(username, "103", "en-US"))
	...
	f.gameReq = &jiliUtMessage.Server_Request{
		...
		Req: jiliut.ProtoEncode(&jiliUtMessage.Server_SpinReq{
			...
			Bet:           proto.Float64(1),
			MallBet:       lo.Ternary(buy, proto.Float64(42.5), nil),
		}),
	}
...
func (f *Fetcher) run() {
	...
	for ; ; time.Sleep(time.Second) {
		...
		var mw2Data message.Mw2_SpinAck
		lo.Must0(proto.Unmarshal(spinResponse.Data, &mw2Data))
		spinResponse.Data = []byte{}

		if mw2Data.GetAckType() != 0 {
			slog.Error("pull spin data error", "acktype", mw2Data.GetAckType(), "user", f.username, "buy", f.buy)
			break
		}
		...
		doc := &models.RawSpin{
			...
			HasGame:  len(mw2Data.GetRoundQueue()) > 1,
			Mw2Data:  &mw2Data,
			UtData:   &spinResponse,
			Selected: true,
		}
