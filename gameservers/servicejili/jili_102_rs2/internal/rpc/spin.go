...
func spin(ps *nats.Msg) (ret []byte, err error) {
	...
	var spinReq message.Rs2_SpinReq
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		for _, round := range spinAck.RoundQueue {
			for _, stagedata := range round.ComboStageData {
				ut.FloatPtrMul(stagedata.ComboStageWin, bet)
				ut.FloatPtrDiv(stagedata.ComboStageWin, internal.BaseBet)
				for _, awarddatavec := range stagedata.AwardDataVec {
		...
		for _, bounsInfo := range spinAck.BonusInfo {
			ut.FloatPtrMul(bounsInfo.Win, bet)
			ut.FloatPtrDiv(bounsInfo.Win, internal.BaseBet)
		}
