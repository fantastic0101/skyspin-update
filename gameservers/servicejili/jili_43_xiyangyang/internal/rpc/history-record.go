...
func init() {
	jiliut.RegRpc(fmt.Sprintf("/history/%s/get-history-record", `xyy`), get_history_record)

	jiliut.RegRpc(fmt.Sprintf("/history/%s/get-single-round-log-summary/", `xyy`), get_single_round_log_summary)

	jiliut.RegRpc(fmt.Sprintf("/history/%s/get-log-plate-info/", `xyy`), get_log_plate_info)
}
