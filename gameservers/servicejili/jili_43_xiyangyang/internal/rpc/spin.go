...
var winReg2 = regexp.MustCompile(`^(Special Bonus: )([0-9.+-]+)$`)
...
func spin(pid int64, data []byte, ps *nats.Msg) (ret protoreflect.ProtoMessage, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		// 游戏内部数据
		xiyangyangData := doc.XiyangyangData
		for i := range xiyangyangData.GetPlate() {
			temp := xiyangyangData.GetPlate()[i]
			if temp.Win != 0 {
				ut.FloatPtrMul(&temp.Win, bet)
				ut.FloatPtrDiv(&temp.Win, internal.BaseBet)
			}
		}
		...
		jiliUtData.Data, _ = proto.Marshal(xiyangyangData)
		ret = jiliUtData
