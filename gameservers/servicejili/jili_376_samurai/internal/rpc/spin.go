...
import (
	...
	"serve/comm/ut"
	"serve/servicejili/jili_376_samurai/internal"
	"serve/servicejili/jili_376_samurai/internal/models"
...
func spin(pid int64, data []byte, ps *nats.Msg) (ret protoreflect.ProtoMessage, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		// 游戏内部数据
		samuraiData := doc.SamuraiData
		// 通用数据
		jiliUtData := doc.UtData
		betMul := bet / jiliUtData.BaseBet

		for _, roundInfo := range samuraiData.GetRoundQueue() {
			...
			for i, displayWin := range roundInfo.DisplayWin {
				if displayWin > 0 {
					roundInfo.DisplayWin[i] = ut.Round6(displayWin * betMul)
				}
			}
			for _, displayAwardVec := range roundInfo.GetDisplayAward() {
				if displayAwardVec == nil {
					continue
				}
				for _, displayAward := range displayAwardVec.AwardDataVec {
					ut.FloatPtrMul(&displayAward.Win, betMul)
				}
			}
		}
		ut.FloatPtrMul(&samuraiData.TotalWin, betMul)
		ut.FloatPtrMul(&samuraiData.FreeTotalWin, betMul)
		ut.FloatPtrMul(&jiliUtData.TotalWin, betMul)

		//lo.Must0(ut.FloatEQ(spinAck.GetTotalWin(), totalWin))
		totalWin := samuraiData.GetTotalWin()
		...
		jiliUtData.Data, _ = proto.Marshal(samuraiData)
