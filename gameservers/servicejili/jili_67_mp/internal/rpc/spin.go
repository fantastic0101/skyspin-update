...
func spin(ps *nats.Msg) (ret []byte, err error) {
	...
	//}
	spinReq := map[string]float64{}
	err = json.Unmarshal(ps.Data, &spinReq)
	if err != nil {
		return
	}
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		info := doc.Data["info"].(map[string]interface{})
		info["ShowIndex"] = fmt.Sprintf("%s-%s-%s", showIndex[0:5], showIndex[5:11], showIndex[11:20])
		info["TotalWin"] = info["TotalWin"].(float64) * bet
		...
		info["NowMoney"] = ut.Gold2Money(balanceGold)
		...
		his.PostMoney = ut.Ftoa(info["NowMoney"].(float64))
