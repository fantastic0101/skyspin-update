...
import (
	...
	"serve/servicejili/jili_58_gq/internal/gendata"
...
type Fetcher struct {
	...
	launchUrl    *url.URL
	spinUrl      *url.URL
	gameReq      *message.Gq_GameReqData
	historyToken string
}
...
func newFetcher(username string, coll *mongo.Collection, buy bool) *Fetcher {
	...
	var loginDataReq = message.Gq_LoginDataReq{
	...
	var gameloginResData message.Gq_ResData
	lo.Must0(jiliut.PostProto(gameloginUrl.String(), &loginDataReq, &gameloginResData))

	// https://uat-wbslot-fd.jlfafafa1.com/Gq/game/spin?token=
	f.spinUrl = &url.URL{
		Scheme: "https",
		Host:   ut.ReverseString(query.Get("gs")),
	...
	f.gameReq = &message.Gq_GameReqData{
		Token: proto.String(token),
		Aid:   proto.Int32(tokenST.Profile.Aid),
		Apiid: proto.Int32(tokenST.Profile.ApiId),
		// Encode: lo.Must(proto.Marshal(&spinReq)),
		Encode: jiliut.ProtoEncode(&message.Gq_SpinReq{
			Bet:     proto.Float64(1),
			MallBet: lo.Ternary(buy, proto.Float64(38), nil),
			MallID:  lo.Ternary(buy, proto.Int32(50), nil),
		}),
	}
	return f
}
...
func (f *Fetcher) run() {
	...
	for ; ; time.Sleep(time.Second) {
		var spinResData message.Gq_ResData
		...
		var spinAll message.Gq_SpinAllData
		...
		if spinAck.GetAckType() != 0 {
			slog.Error("pull spin data error", "acktype", spinAck.GetAckType(), "user", f.username, "buy", f.buy)
			break
		}
		...
		doc := &models.RawSpin{
			...
			Times:    totalWin / internal.BaseBet, // bet is 1.0 for Gq
			Type:     lo.Ternary(f.buy, gendata.GameTypeGame, gendata.GameTypeNormal),
			HasGame:  len(spinAck.FreeInfo) > 0,
			Data:     spinAck,
			Selected: true,
		}
