...
import (
	...
	"serve/comm/define"
	"serve/comm/lazy"
	"serve/servicejili/jili_58_gq/internal/gamedata"
...
func spin(ps *nats.Msg) (ret []byte, err error) {
	...
	var spinReq message.Gq_SpinReq
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		// 处理数据
		spinAck := doc.Data
		for _, round := range spinAck.MainPlate {
			ut.FloatPtrMul(round.SingleWin, bet)
			ut.FloatPtrDiv(round.SingleWin, internal.BaseBet)
		}

		for _, round := range spinAck.FreeInfo {
			for _, round2 := range round.FreeQueue {
				for _, round3 := range round2.FreePlate {
					ut.FloatPtrMul(round3.SingleWin, bet)
					ut.FloatPtrDiv(round3.SingleWin, internal.BaseBet)
				}
			}
			ut.FloatPtrMul(round.FreeTotalWin, bet)
			ut.FloatPtrDiv(round.FreeTotalWin, internal.BaseBet)
		}
