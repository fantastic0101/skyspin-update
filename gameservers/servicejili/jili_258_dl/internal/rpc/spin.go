...
import (
	...
	"serve/servicejili/jili_258_dl/internal/gamedata"
	"serve/servicejili/jili_258_dl/internal/models"
...
func spin(pid int64, data []byte, ps *nats.Msg) (ret protoreflect.ProtoMessage, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		// 游戏内部数据
		zeusData := doc.ZeusData

		for _, game := range zeusData.MainGame {
			ut.FloatPtrMul(game.BonusWin, bet)
			ut.FloatPtrMul(game.SingleWin, bet)
			JPWin := make([]float64, len(game.JPWin))
			copy(JPWin, game.JPWin)
			for i := range JPWin {
				JPWin[i] = JPWin[i] * bet
			}
			game.JPWin = JPWin
			for _, item := range game.AwardDataVec {
				ut.FloatPtrMul(item.Win, bet)
			}
		}

		for _, free := range zeusData.FreeGame {
			ut.FloatPtrMul(free.FreeTotalWin, bet)
			for _, queue := range free.FreeQueue {
				ut.FloatPtrMul(queue.BonusWin, bet)
				ut.FloatPtrMul(queue.SingleWin, bet)
				JPWin := make([]float64, len(queue.JPWin))
				copy(JPWin, queue.JPWin)
				for i := range JPWin {
					JPWin[i] = JPWin[i] * bet
				}
				queue.JPWin = JPWin
				for _, item := range queue.AwardDataVec {
					ut.FloatPtrMul(item.Win, bet)
				}
			}
		}
		ut.FloatPtrMul(zeusData.TotalWin, bet)
		// 通用数据
		jiliUtData := doc.UtData
		ut.FloatPtrMul(jiliUtData.TotalWin, bet)
		if jiliUtData.TotalWin != nil {
		...
		zeusData.NowMoney = proto.Float64(ut.Gold2Money(balanceGold))
		jiliUtData.PostMoney = zeusData.NowMoney
		jiliUtData.PostMoney = proto.Float64(ut.Gold2Money(balanceGold))
		jiliUtData.Data, _ = proto.Marshal(zeusData)
