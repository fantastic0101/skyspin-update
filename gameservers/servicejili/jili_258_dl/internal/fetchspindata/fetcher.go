...
import (
	...
	"serve/servicejili/jili_258_dl/internal/message"
...
func newFetcher(username string, coll *mongo.Collection, buy bool) *Fetcher {
	...
	f.gameReq = &jiliUtMessage.Server_Request{
		...
		Req: jiliut.ProtoEncode(&jiliUtMessage.Server_SpinReq{
			...
			MallBet: lo.Ternary(buy, proto.Float64(37.5), nil),
		}),
	}
	return f
}
...
func (f *Fetcher) run() bool {
	...
	for ; ; time.Sleep(time.Second) {
		...
		var zeusData message.Dl_SpinAck
		...
		doc := &models.RawSpin{
			...
			HasGame:  len(zeusData.FreeGame) > 0,
			ZeusData: &zeusData,
			UtData:   &spinResponse,
			Selected: true,
		}
