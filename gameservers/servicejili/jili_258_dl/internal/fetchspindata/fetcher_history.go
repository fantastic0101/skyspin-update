...
func (f *Fetcher) fetchHisotry(roundIndex string, doc *models.RawSpin) {
	...
	platcomm.Retry(20, 3*time.Second, func() bool {
		lo.Must0(f.invokeHistory("/history/dl/get-history-record", historyRecodeQuery.Encode(), &records))
		n := len(records)
		lo.Must0(n < 2)
		return n == 0
	})
	...
	lo.Must0(f.invokeHistory("/history/dl/get-single-round-log-summary/en-US/"+roundIndex, "", &summaries))
	...
	for _, summary := range summaries {
		...
		path := fmt.Sprintf("/history/dl/get-log-plate-info/%s/%s", roundIndex, summary.LogIndex)
