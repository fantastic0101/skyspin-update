...
import (
	...
	"serve/servicejili/jili_209_ap/internal/gendata"
...
func nextPlayResp(player *models.Player, bet int64, selfPoolGold int64, isBuy bool, App *redisx.AppStore) (playResp *models.RawSpin, hitBigAward, forcedKill, buyKill bool, err error) {
	...
	if false {
		...
		t := lo.Ternary(isBuy, gendata.GameTypeGame, gendata.GameTypeNormal)
	...
	if isBuy {
		if selfPoolGold < 0 && rand.IntN(1000) < gamedata.GetSettings().BuyMinAwardPercent {
