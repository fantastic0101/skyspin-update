...
import (
	...
	"regexp"
	"serve/comm/redisx"
	"serve/servicejili/jili_209_ap/internal/message"
	"strconv"
	"time"
	...
	"serve/comm/ut"
	"serve/servicejili/jili_209_ap/internal"
	"serve/servicejili/jili_209_ap/internal/gamedata"
	"serve/servicejili/jili_209_ap/internal/models"
...
func spin(pid int64, data []byte, ps *nats.Msg) (ret protoreflect.ProtoMessage, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		// teemo for test
		// doc, hitBigAward, forcedKill, buyKill, err = findDoc("6696190974f4f5a7e397f69b")
		...
		// 游戏内部数据
		apData := doc.ApData
		for _, roundInfo := range apData.GetRoundQueue() {
		...
		apData.NowMoney = proto.Float64(ut.Gold2Money(balanceGold))
		jiliUtData.PostMoney = apData.NowMoney
		jiliUtData.Data, _ = proto.Marshal(apData)
		...
		his.PostMoney = ut.Ftoa(apData.GetNowMoney())
		...
		for i, sm := range doc.SingleRoundLogSummaries {
			roundInfos := apData.RoundQueue[i]
...
func getSymbolLength(stageInfo *message.Ap_ComboStageInfo) (symbolLength string) {
