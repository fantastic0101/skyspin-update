...
func newFetcher(username string, coll *mongo.Collection, buy bool) *Fetcher {
	...
	launchUrl := lo.Must(jiliut.LaunchGame(username, "209", "en-US"))
	...
	f.gameReq = &jiliUtMessage.Server_Request{
		...
		Req: jiliut.ProtoEncode(&jiliUtMessage.Server_SpinReq{
			...
			Bet:           proto.Float64(1),
			MallBet:       lo.Ternary(buy, proto.Float64(44.5), nil),
		}),
	}
...
func (f *Fetcher) run() {
	...
	for ; ; time.Sleep(time.Second) {
		...
		var apData message.Ap_SpinAck
		lo.Must0(proto.Unmarshal(spinResponse.Data, &apData))
		spinResponse.Data = []byte{}

		if apData.GetAckType() != 0 {
			slog.Error("pull spin data error", "acktype", apData.GetAckType(), "user", f.username, "buy", f.buy)
			break
		}
		...
		doc := &models.RawSpin{
			...
			HasGame:  len(apData.GetRoundQueue()) > 1,
			ApData:   &apData,
			UtData:   &spinResponse,
			Selected: true,
		}
