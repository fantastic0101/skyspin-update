...
import (
	...
	"serve/servicejili/jili_230_cts/internal/gendata"
...
type Fetcher struct {
	...
	launchUrl    *url.URL
	spinUrl      *url.URL
	gameReq      *message.Cts_GameReqData
	historyToken string
}
...
func newFetcher(username string, coll *mongo.Collection, ty int) *Fetcher {
	...
	var loginDataReq = message.Cts_LoginDataReq{
	...
	var gameloginResData message.Cts_ResData
...
func (f *Fetcher) run() {
	...
	for ; ; time.Sleep(time.Second) {
		var spinResData message.Cts_ResData
