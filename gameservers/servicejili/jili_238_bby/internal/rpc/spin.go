...
import (
	...
	"serve/servicejili/jili_238_bby/internal/gendata"
...
func spin(ps *nats.Msg) (ret []byte, err error) {
	...
	var spinReq message.Bb_SpinReq
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		var spinAck *message.Bb_SpinAck
		...
		// 处理赢分数据
		for _, awardData := range spinAck.MainGame {
			for _, awarddatavec := range awardData.AwardDataVec {
				ut.FloatPtrMul(awarddatavec.Win, bet)
			}
			ut.FloatPtrMul(awardData.SingleWin, bet)
			ut.FloatPtrMul(awardData.BonusWin, bet)
		}
		for _, awardData2 := range spinAck.BonusGame {
			for _, awarddatavec := range awardData2.BonusQueue {
				ut.FloatPtrMul(awarddatavec.SingleWin, bet)
				ut.FloatPtrMul(awarddatavec.BonusWin, bet)
			}
			ut.FloatPtrMul(awardData2.BonusTotalWin, bet)
		...
		zi := int32(0)
		zui := uint32(0)
		var spinAll = message.Bb_SpinAllData{
			Info: []*message.Bb_SpinAck{
				spinAck,
			},
			ServiceInfo: []*message.Bb_ServiceData{
				{
					MissionReward: &zui,
					JpType:        &zi,
					JpWin:         &zero,
		...
		for _, pinfos := range doc.LogPlateInfos {
			for _, pinfo := range pinfos {
				...
				for i := range pinfo.BonusArray {
					ut.FloatStrPtrMul(&pinfo.BonusArray[i], bet)
				}
			}
		}
