...
type Fetcher struct {
	...
	launchUrl    *url.URL
	spinUrl      *url.URL
	gameReq      *message.Bb_GameReqData
	historyToken string
}
...
func newFetcher(username string, coll *mongo.Collection, buy bool) *Fetcher {
	...
	var loginDataReq = message.Bb_LoginDataReq{
	...
	var gameloginResData message.Bb_ResData
...
func (f *Fetcher) run() {
	...
	for {
		...
		var spinResData message.Bb_ResData
		...
		doc := &models.RawSpin{
			...
			HasGame:  len(spinAck.BonusGame) > 0,
			Data:     spinAck,
			Selected: true,
		}
