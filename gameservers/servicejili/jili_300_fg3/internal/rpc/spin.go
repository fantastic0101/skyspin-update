...
func spin(pid int64, data []byte, ps *nats.Msg) (ret protoreflect.ProtoMessage, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		// 游戏内部数据
		fgData := doc.FgData
		for _, awardData := range fgData.GetAwardDataVec() {
			ut.FloatPtrMul(awardData.Win, bet)
		}
		ut.FloatPtrMul(fgData.TotalWin, bet)
		// 通用数据
		...
		fgData.NowMoney = proto.Float64(ut.Gold2Money(balanceGold))
		jiliUtData.PostMoney = fgData.NowMoney
		jiliUtData.Data, _ = proto.Marshal(fgData)
		...
		his.PostMoney = ut.Ftoa(fgData.GetNowMoney())
