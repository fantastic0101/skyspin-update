...
import (
	...
	"serve/servicejili/jili_109_fg/internal/gendata"
	"serve/servicejili/jili_109_fg/internal/message"
	"serve/servicejili/jili_109_fg/internal/models"
...
func newFetcher(username string, coll *mongo.Collection, ty int) *Fetcher {
	...
	launchUrl := lo.Must(jiliut.LaunchGame(username, "109", "en-US"))
...
func (f *Fetcher) run() {
	...
	for ; ; time.Sleep(time.Second) {
		...
		var fgData message.Fg_SpinAck
		lo.Must0(proto.Unmarshal(spinResponse.Data, &fgData))
		spinResponse.Data = []byte{}

		if fgData.GetAckType() != 0 {
			slog.Error("pull spin data error", "acktype", fgData.GetAckType(), "user", f.username, "type", f.ty)
			break
		}

		totalWin := fgData.GetTotalWin()
