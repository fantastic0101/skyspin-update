...
func spin(ps *nats.Msg) (ret []byte, err error) {
	...
	var spinReq message.Tks_SpinReq
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		var nextIds []primitive.ObjectID
		var balance float64
		var balanceGold int64
		var spinAck *message.Tks_SpinAck
		selfPoolGold, _ := slotsmongo.GetSelfSlotsPool(plr.PID)
		if len(plr.NextIds) > 0 {
			if !plr.Kill { // 赤壁之战模式，不能修改投注金额
				bet = plr.LastBet
			}
			gold = ut.Money2Gold(bet)
			costGold = ut.Money2Gold(bet)
			...
			doc, err = gendata.FindFromSimulate(plr.NextIds[0])
			if err != nil {
			...
			plr.NextIds = plr.NextIds[1:]
			spinAck = doc.Data
			if plr.Kill {
				spinAck.NowMode = proto.Bool(false)
				spinAck.IsHighRateMode = proto.Bool(false)
			} else {
				spinAck.NowMode = proto.Bool(true)
				spinAck.IsHighRateMode = proto.Bool(true)
			}
			if len(plr.NextIds) <= 0 {
				spinAck.IsHighRateMode = proto.Bool(false)
			}
			...
			doc, nextIds, hitBigAward, forcedKill, err = nextPlayResp(plr, gold, selfPoolGold, App)
			if err != nil {
				fmt.Println(err)
			...
			spinAck = doc.Data
			if len(nextIds) > 0 {
				// 判断最后一盘是否超过最大赢分，如果超过，解除赤壁之战模式
				winMaxLimit := slotspool.GetMaxWin(plr.AppID, int64(float64(plr.GetWinLose())/10000), curItem.Multi)
				lastSpin, err := gendata.FindFromSimulate(nextIds[len(nextIds)-1])
				if err != nil {
					return err
				}
				lastDoc := lastSpin.Data
				ut.FloatPtrMul(lastDoc.TotalWin, bet)
				totalWin := lastDoc.GetTotalWin()
				...
				plr.NextIds = []primitive.ObjectID{}
				spinAck.NowMode = proto.Bool(false)
				spinAck.IsHighRateMode = proto.Bool(false)
			}
		}
		...
		// 处理赢分数据
		for _, awardData := range spinAck.AwardData {
			ut.FloatPtrMul(awardData.AwardPrize, bet)
		}
		ut.FloatPtrMul(spinAck.MGWin, bet)
		ut.FloatPtrMul(spinAck.TotalWin, bet)
		totalWin := spinAck.GetTotalWin()
		for _, freeGame := range spinAck.FreeGamePackage {
			for _, plateInfo := range freeGame.FreeGamePlateInfo {
				for _, awardData := range plateInfo.AwardData {
					ut.FloatPtrMul(awardData.AwardPrize, bet)
				}
			}
			ut.FloatPtrMul(freeGame.TotalPrize, bet)
		}
