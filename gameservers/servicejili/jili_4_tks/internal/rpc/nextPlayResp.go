...
func nextPlayResp(player *models.Player, bet int64, selfPoolGold int64, App *redisx.AppStore) (playResp *models.RawSpin, nextIds []primitive.ObjectID, hitBigAward, forcedKill bool, err error) {
	...
	if false {
		...
		//objectId, _ := primitive.ObjectIDFromHex("666be42bf254b86efa189259")
		match := bson.M{
			//"_id": objectId,
		}
		match["type"] = 0
		...
		for i := 0; i < len(nextIdStrs); i++ {
			id, _ := primitive.ObjectIDFromHex(nextIdStrs[i])
			nextIds = append(nextIds, id)
		}
		return spindoc, nextIds, false, false, err
	}
	...
	if player.PersonWinMaxMult != 0 && player.PersonWinMaxScore != 0 {
		for {
			playResp, nextIds, err = gendata.GCombineDataMng.Next(fmt.Sprintf("%d", bet), App.GamePatten)
