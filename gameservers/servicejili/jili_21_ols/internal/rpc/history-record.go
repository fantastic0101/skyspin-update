...
func init() {
	jiliut.RegRpc(fmt.Sprintf("/history/%s/get-history-record", "ols"), get_history_record)

	jiliut.RegRpc(fmt.Sprintf("/history/%s/get-single-round-log-summary/", "ols"), get_single_round_log_summary)

	jiliut.RegRpc(fmt.Sprintf("/history/%s/get-log-plate-info/", "ols"), get_log_plate_info)
}
