...
type Fetcher struct {
	...
	launchUrl    *url.URL
	spinUrl      *url.URL
	gameReq      *message.Luckytree_GameReqData
	historyToken string
}
...
func newFetcher(username string, coll *mongo.Collection, buy bool) *Fetcher {
	...
	var loginDataReq = message.Luckytree_LoginDataReq{
	...
	var gameloginResData message.Luckytree_ResData
...
func (f *Fetcher) run() {
	...
	for {
		...
		var spinResData message.Luckytree_ResData
