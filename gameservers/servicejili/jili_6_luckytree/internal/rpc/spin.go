...
func spin(ps *nats.Msg) (ret []byte, err error) {
	var gameReq message.Luckytree_GameReqData
	...
	var spinReq message.Luckytree_SpinReq
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		spinAck := doc.Data
		for _, pd := range spinAck.PlateData {
			ut.FloatPtrMul(pd.WinMoney, bet)
			totalWin += pd.GetWinMoney()
			for _, award := range pd.AwardVec {
				ut.FloatPtrMul(award.Win, bet)
			}
		}
		for _, bgd := range spinAck.BonusGameData {
			ut.FloatPtrMul(bgd.TotalWin, bet)
			totalWin += bgd.GetTotalWin()
			for i := range bgd.HitCountWin {
				ut.FloatPtrMul(&bgd.HitCountWin[i], bet)
			}
		}
		for _, fgd := range spinAck.FreeGameData {
			ut.FloatPtrMul(fgd.TotalWin, bet)
			totalWin += fgd.GetTotalWin()
		}
		...
		spinAck.ShowIndex = proto.String(fmt.Sprintf("%s-%s-%s", showIndex[0:5], showIndex[5:11], showIndex[11:20]))
		var spinAll = message.Luckytree_SpinAllData{
			Info: []*message.Luckytree_SpinAck{
				spinAck,
			}}
		encode, _ := proto.Marshal(&spinAll)

		var resData = message.Luckytree_ResData{
			Type:  proto.Int32(AckType["spin"]),
			Token: proto.String(token),
			Data: []*message.Luckytree_InfoData{
				{
					Encode: encode,
				},
			},
		...
		// {
		//     "Type": "",
		//     "RoundIndex": "1718339075034995002",
		//     "NetValue": "-2",
		//     "Bet": "2",
		//     "Win": "0",
		//     "CreateTime": 1718440229400,
		//     "PreMoney": "982964.27",
		//     "PostMoney": "982962.27",
		//     "ItemName": null,
		//     "CurrencyNo": 0,
		...
		// ut.FloatStrPtrMul(&doc.HistoryRecord.NetValue, bet)
