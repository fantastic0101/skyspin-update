...
func init() {
	jiliut.RegRpc(fmt.Sprintf("/history/%s/get-history-record", "ft"), get_history_record)

	jiliut.RegRpc(fmt.Sprintf("/history/%s/get-single-round-log-summary/", "ft"), get_single_round_log_summary)

	jiliut.RegRpc(fmt.Sprintf("/history/%s/get-log-plate-info/", "ft"), get_log_plate_info)
}
