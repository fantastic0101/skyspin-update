...
import (
	...
	"serve/comm/db"
	"serve/servicejili/jili_91_ifff/internal/models"
...
type CombineDataMng struct {
	...
	bucketExtra      map[int][]primitive.ObjectID
}
...
func (mng *CombineDataMng) Next(bet string, WitchBucket int) (playResp *models.RawSpin, err error) {
	data := mng.get(bet)
...
func (mng *CombineDataMng) GetBigReward(witchBucket int) (playResp *models.RawSpin, err error) {
	ids := []int{}
	ids = append(ids, GetBucketIds(5, 10)...) //todo 记得还原
...
func (mng *CombineDataMng) GetBigReward2_5(witchBucket int) (playResp *models.RawSpin, err error) {
	ids := []int{}
	if len(ids) == 0 {
		ids = append(ids, GetBucketIds(2, 5)...) //todo 记得还原
		ids = lo.Filter(ids, func(item int, index int) bool {
			return len(mng.buckets[witchBucket][item]) != 0
		})
	}
	if len(ids) == 0 {
		ids = append(ids, GetBucketIds(1, 6)...) //todo 记得还原
		ids = lo.Filter(ids, func(item int, index int) bool {
			return len(mng.buckets[witchBucket][item]) != 0
		})
	}
	if len(ids) == 0 {
		ids = append(ids, GetBucketIds(0, 10)...) //todo 记得还原
		ids = lo.Filter(ids, func(item int, index int) bool {
			return len(mng.buckets[witchBucket][item]) != 0
		})
	}
...
func (mng *CombineDataMng) SampleForceSimulate(witchBucket int) (playResp *models.RawSpin, err error) {
