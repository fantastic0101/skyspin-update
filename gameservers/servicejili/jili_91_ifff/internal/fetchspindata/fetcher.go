...
import (
	...
	tmp "serve/servicejili/jili_91_ifff/internal/message"
...
func newFetcher(username string, coll *mongo.Collection) *Fetcher {
	...
	launchUrl := lo.Must(jiliut.LaunchGame(username, "91", "en-US"))
...
func (f *Fetcher) run() bool {
	...
	for ; ; time.Sleep(time.Second) {
		...
		var ifffData tmp.Ifff_SpinAck
		lo.Must0(proto.Unmarshal(spinResponse.Data, &ifffData))
		spinResponse.Data = []byte{}

		if ifffData.GetAckType() != 0 {
			slog.Error("pull spin data error", "acktype", ifffData.GetAckType(), "user", f.username, "buy", f.buy)
			break
		}
