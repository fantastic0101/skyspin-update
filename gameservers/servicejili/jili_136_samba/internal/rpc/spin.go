...
import (
	...
	"serve/comm/define"
	"serve/comm/lazy"
	"serve/servicejili/jili_136_samba/internal/gamedata"
...
func spin(ps *nats.Msg) (ret []byte, err error) {
	var gameReq message.Samba_GameReqData
	...
	var spinReq message.Samba_SpinReq
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		ut.FloatPtrMul(spinAck.FreeTotalWin, baseBet)
		...
		var spinAll = message.Samba_SpinAllData{
			Info: []*message.Samba_SpinAck{
				spinAck,
			}}
		encode, _ := proto.Marshal(&spinAll)

		var resData = message.Samba_ResData{
			Type:  proto.Int32(AckType["spin"]),
			Token: proto.String(token),
			Data: []*message.Samba_InfoData{
				{
					Encode: encode,
				},
			},
		...
		for i, v := range spinAck.RoundQueue { //9
			float := strconv.FormatFloat(*v.Mult, 'g', -1, 64)
