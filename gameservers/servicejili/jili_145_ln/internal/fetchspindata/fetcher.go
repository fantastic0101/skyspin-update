...
import (
	...
	"serve/servicejili/jili_145_ln/internal/gendata"
...
type Fetcher struct {
	...
	launchUrl    *url.URL
	spinUrl      *url.URL
	gameReq      *message.Ln_GameReqData
	historyToken string
}
...
func newFetcher(username string, coll *mongo.Collection, ty int) *Fetcher {
	...
	var loginDataReq = message.Ln_LoginDataReq{
	...
	var gameloginResData message.Ln_ResData
...
func (f *Fetcher) run() {
	...
	for {
		...
		var spinResData message.Ln_ResData
		...
		var spinAll message.Ln_SpinAllData
