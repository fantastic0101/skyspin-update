...
import (
	...
	"serve/servicejili/jili_183_gj/internal/models"
...
var winReg3 = regexp.MustCompile(`^(ALL WIN :)([0-9.+-]+)$`)
...
func spin(pid int64, data []byte, ps *nats.Msg) (ret protoreflect.ProtoMessage, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		// 游戏内部数据
		GjData := doc.GjData
		for _, roundInfo := range GjData.RoundQueue {
			ut.FloatPtrMul(roundInfo.WheelWin, bet)
		...
		ut.FloatPtrMul(GjData.TotalWin, bet)
		...
		jiliUtData.Data, _ = proto.Marshal(GjData)
		...
		//his.PostMoney = ut.Ftoa(GjData.GetNowMoney())
			...
			for i, desc := range sm.Desc {
				...
				if len(rets) == 3 {
					...
					if len(rets) == 3 {
						...
						rets = winReg3.FindStringSubmatch(desc)
						if len(rets) == 3 {
							winstr := rets[2]
							ut.FloatStrPtrMul(&winstr, bet)
