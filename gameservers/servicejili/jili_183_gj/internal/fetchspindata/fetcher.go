...
import (
	...
	"serve/servicejili/jili_183_gj/internal/message"
...
func newFetcher(username string, coll *mongo.Collection, ty int) *Fetcher {
	...
	launchUrl := lo.Must(jiliut.LaunchGame(username, "183", "en-US"))
...
func (f *Fetcher) run() {
	...
	for ; ; time.Sleep(time.Second) {
		...
		var gjData message.Gj_SpinAck
		lo.Must0(proto.Unmarshal(spinResponse.Data, &gjData))
		spinResponse.Data = []byte{}

		if gjData.GetAckType() != 0 {
			slog.Error("pull spin data error", "acktype", gjData.GetAckType(), "user", f.username)
			break
		}
