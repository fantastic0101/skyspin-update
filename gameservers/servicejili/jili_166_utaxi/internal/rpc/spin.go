...
func spin(ps *nats.Msg) (ret []byte, err error) {
	var gameReq message.Utaxi_GameReqData
	...
	var spinReq message.Utaxi_SpinReq
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		fmt.Println("logid is:", showIndex)
		...
		spinAck := doc.Data
		for _, rq := range spinAck.RoundQueue {
			for _, ad := range rq.AwardDataVec {
				ut.FloatPtrMul(ad.Win, bet)
			}
			ut.FloatPtrMul(rq.RoundWin, bet)
			totalWin += rq.GetRoundWin()
		}
		ut.FloatPtrMul(spinAck.TotalWin, bet)
		ut.FloatPtrMul(spinAck.FreeTotalWin, bet)
		...
		var spinAll = message.Utaxi_SpinAllData{
			Info: []*message.Utaxi_SpinAck{
				spinAck,
			}}
...
// func Generate24lengthID(t time.Time) string {
// 	timestamp := t.UnixNano()
// 	timestampStr := strconv.FormatInt(timestamp, 10)

// 	for len(timestampStr) < 24 {
// 		timestampStr = timestampStr + fmt.Sprintf("%v", rand.Intn(10))
// 	}
// 	return timestampStr
// }

// func generateUniqueID(t time.Time) string {
