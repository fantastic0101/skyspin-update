...
func init() {
	// jiliut.RegRpc(fmt.Sprintf("/history/%s/get-history-record", "utaxi"), get_history_record)

	// jiliut.RegRpc(fmt.Sprintf("/history/%s/get-single-round-log-summary/", "utaxi"), get_single_round_log_summary)

	// jiliut.RegRpc(fmt.Sprintf("/history/%s/get-log-plate-info/", "utaxi"), get_log_plate_info)
}
...
func get_history_record_old(ps *nats.Msg) (ret []byte, err error) {
	...
	*/
	...
	// )
	// if roundIndex != "" {
	// 	id, er := primitive.ObjectIDFromHex(roundIndex)
	// 	if er != nil {
	// 		err = er
	// 		return
	// 	}
	...
	// 	doc.HistoryRecord.RoundIndex = "1718867695491146166"
	...
	ret = []byte(`{
		...
		"Data": [
			{
				"Type": "",
				"RoundIndex": "17195039057008761600999",
				"NetValue": "-1",
				"Bet": "1",
				"Win": "0",
				"CreateTime": 1719538752800,
				"PreMoney": "997307.29",
				"PostMoney": "997306.29",
				"ItemName": null,
				"CurrencyNo": 0,
...
func get_single_round_log_summary_old(ps *nats.Msg) (ret []byte, err error) {
	// ul, err := url.Parse(string(ps.Data))
	// if err != nil {
	// 	return
	// }

	// id, err := primitive.ObjectIDFromHex(path.Base(ul.Path))
	...
	// err = coll.FindOne(context.TODO(), db.ID(id), opts).Decode(&doc)
	...
	ret = []byte(`{
		...
		"Data": [
			{
				"LogIndex": "17195039057008761600999",
				"Title": [
					"一般游戏"
				],
				"Desc": [],
...
func get_log_plate_info_old(ps *nats.Msg) (ret []byte, err error) {
	...
	// id := lo.Must(primitive.ObjectIDFromHex(lo.Must(lo.Nth(args, -2))))
	...
	// platinfo := doc.LogPlateInfos[id2]
	// for _, item := range platinfo {
	// 	for i := 0; i < 15; i++ {
	// 		// item.PPX = 19
	// 		item.BonusArray = append(item.BonusArray, "0")
	// 	}

	// }

	// ret = jiliut.MarshalJsonReturn(platinfo)
	ret = []byte(`{
		"Code": 0,
		"Message": "成功",
		"Data": [
