...
type Fetcher struct {
	...
	launchUrl    *url.URL
	spinUrl      *url.URL
	gameReq      *message.Utaxi_GameReqData
	historyToken string
}
...
func newFetcher(username string, coll *mongo.Collection, buy bool) *Fetcher {
	...
	var loginDataReq = message.Utaxi_LoginDataReq{
	...
	var gameloginResData message.Utaxi_ResData
...
func (f *Fetcher) run() {
	...
	for {
		...
		var spinResData message.Utaxi_ResData
