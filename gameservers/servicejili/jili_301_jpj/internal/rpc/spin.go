...
func spin(pid int64, data []byte, ps *nats.Msg) (ret protoreflect.ProtoMessage, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		for _, roundInfo := range jpjData.GetAwardDataVec() {
			ut.FloatPtrMul(&roundInfo.Win, bet)
		}
		ut.FloatPtrMul(&jpjData.JPWin, bet)
		ut.FloatPtrMul(&jpjData.TotalWin, bet)

		// 通用数据
		jiliUtData := doc.UtData
		serviceInfo := jiliUtData.GetService()
		...
		jpjData.NowMoney = ut.Gold2Money(balanceGold)
		jiliUtData.PostMoney = jpjData.NowMoney
		jiliUtData.Data, _ = proto.Marshal(jpjData)
		...
		his.PostMoney = ut.Ftoa(jpjData.GetNowMoney())
