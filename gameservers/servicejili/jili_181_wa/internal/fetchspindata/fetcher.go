...
import (
	...
	"serve/servicejili/jili_181_wa/internal/message"
...
func newFetcher(username string, coll *mongo.Collection, buy bool) *Fetcher {
	...
	launchUrl := lo.Must(jiliut.LaunchGame(username, "181", "en-US"))
	...
	f.gameReq = &jiliUtMessage.Server_Request{
		...
		Req: jiliut.ProtoEncode(&jiliUtMessage.Server_SpinReq{
			...
			Bet:           proto.Float64(1),
			MallBet:       lo.Ternary(buy, proto.Float64(38.5), nil),
		}),
	}
...
func (f *Fetcher) run() {
	...
	for ; ; time.Sleep(time.Second) {
		...
		var waData message.Wa_SpinAck
		lo.Must0(proto.Unmarshal(spinResponse.Data, &waData))
		spinResponse.Data = []byte{}

		if waData.GetAckType() != 0 {
			slog.Error("pull spin data error", "acktype", waData.GetAckType(), "user", f.username, "buy", f.buy)
			break
		}

		totalWin := waData.GetTotalWin()
		...
		doc := &models.RawSpin{
			...
			HasGame:  len(waData.GetRoundQueue()) > 1, //单数据多个pan代表进入小游戏
			WaData:   &waData,
			UtData:   &spinResponse,
			Selected: true,
		}
