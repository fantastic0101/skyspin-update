...
func spin(pid int64, data []byte, ps *nats.Msg) (ret protoreflect.ProtoMessage, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		// 游戏内部数据
		waData := doc.WaData
		for _, roundInfo := range waData.GetRoundQueue() {
		...
		ut.FloatPtrMul(waData.TotalWin, bet)
		ut.FloatPtrMul(waData.FreeTotalWin, bet)
		// 通用数据
		...
		waData.NowMoney = proto.Float64(ut.Gold2Money(balanceGold))
		jiliUtData.PostMoney = waData.NowMoney
		jiliUtData.Data, _ = proto.Marshal(waData)
		...
		his.PostMoney = ut.Ftoa(waData.GetNowMoney())
