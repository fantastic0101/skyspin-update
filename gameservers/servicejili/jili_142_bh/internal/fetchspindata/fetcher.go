...
import (
	...
	"serve/servicejili/jili_142_bh/internal/gendata"
...
type Fetcher struct {
	...
	launchUrl    *url.URL
	spinUrl      *url.URL
	gameReq      *message.Bh_GameReqData
	historyToken string
}
...
func newFetcher(username string, coll *mongo.Collection, buy bool) *Fetcher {
	...
	var loginDataReq = message.Bh_LoginDataReq{
	...
	var gameloginResData message.Bh_ResData
	...
	f.gameReq = &message.Bh_GameReqData{
		Token: proto.String(token),
		Aid:   proto.Int32(tokenST.Profile.Aid),
		Apiid: proto.Int32(tokenST.Profile.ApiId),
		// Encode: lo.Must(proto.Marshal(&spinReq)),
		Encode: jiliut.ProtoEncode(&message.Bh_SpinReq{
			Bet:     proto.Float64(1),
			MallBet: lo.Ternary(buy, proto.Float64(internal.One_BuyMul), nil),
			MallID:  lo.Ternary(buy, proto.Int32(internal.BuyMul), nil),
		}),
	}
	return f
}
...
func (f *Fetcher) run() {
	...
	for {
		...
		var spinResData message.Bh_ResData
		...
		doc := &models.RawSpin{
			...
			HasGame:  len(spinAck.FreeGame) > 0,
			Data:     spinAck,
			Selected: true,
		}
