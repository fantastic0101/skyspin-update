...
func main() {
	...
	for i := 0; i < 20; i++ {
		go func(id int) {
			...
			var num int
			for {
				newFetcher(fmt.Sprintf("%d_teemo002_%d_%d", internal.GameNo, id, num), coll, false).run()
		...
		go func(id int) {
			var num int
			for {
				newFetcher(fmt.Sprintf("%d_teemobuy_%d_%d", internal.GameNo, id, num), coll, true).run()
				fmt.Println("拉取数据失败了，重试中。。。")
				time.Sleep(5 * time.Second)
				num++
			}
...
//
//	// var jili = jili.JILI()
//	launchUrl := lo.Must(jiliut.LaunchGame(username, "142", "en-US"))
//
//	ul := lo.Must(url.Parse(launchUrl))
//
//	query := ul.Query()
