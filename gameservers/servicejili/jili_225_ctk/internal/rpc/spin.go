...
func spin(ps *nats.Msg) (ret []byte, err error) {
	var gameReq message.Ctk_GameReqData
	...
	var spinReq message.Ctk_SpinReq
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		for _, round := range spinAck.RoundQueue {
			for _, combostageData := range round.ComboStageData {
				ut.FloatPtrMul(combostageData.ComboStageWin, baseBet)
			}
