...
import (
	...
	"serve/servicejili/jili_225_ctk/internal/gendata"
...
type Fetcher struct {
	...
	launchUrl    *url.URL
	spinUrl      *url.URL
	gameReq      *message.Ctk_GameReqData
	historyToken string
}
...
func newFetcher(username string, coll *mongo.Collection, ty int) *Fetcher {
	...
	var loginDataReq = message.Ctk_LoginDataReq{
	...
	var gameloginResData message.Ctk_ResData
...
func (f *Fetcher) run() {
	...
	for ; ; time.Sleep(time.Second) {
		var spinResData message.Ctk_ResData
		...
		var spinAll message.Ctk_SpinAllData
		...
		doc := &models.RawSpin{
			...
			Type:     f.ty,
			HasGame:  len(spinAck.GetRoundQueue()) > 1,
			Data:     spinAck,
			Selected: true,
		}
