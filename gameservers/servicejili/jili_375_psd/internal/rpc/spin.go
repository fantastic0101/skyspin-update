...
import (
	...
	"serve/servicejili/jili_375_psd/internal/models"
...
func spin(pid int64, data []byte, ps *nats.Msg) (ret protoreflect.ProtoMessage, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		for _, roundInfo := range psdData.GetRoundQueue() {
			ut.FloatPtrMul(&roundInfo.RoundWin, betMul)

			for _, comboStage := range roundInfo.GetComboStageData() {
				ut.FloatPtrMul(&comboStage.ComboStageWin, betMul)
				for _, awardInfo := range comboStage.GetAwardDataVec() {
					ut.FloatPtrMul(&awardInfo.Win, betMul)
				}
			}
		}
		ut.FloatPtrMul(&psdData.TotalWin, betMul)
		ut.FloatPtrMul(&psdData.FreeTotalWin, betMul)
		ut.FloatPtrMul(&jiliUtData.TotalWin, betMul)

		//lo.Must0(ut.FloatEQ(spinAck.GetTotalWin(), totalWin))
		totalWin := psdData.GetTotalWin()
		...
		jiliUtData.Data, _ = proto.Marshal(psdData)
