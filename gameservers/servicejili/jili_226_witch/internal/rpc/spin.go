...
func spin(pid int64, data []byte, ps *nats.Msg) (ret protoreflect.ProtoMessage, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		// 游戏内部数据
		witchData := doc.WitchData
		// 通用数据
		jiliUtData := doc.UtData
		betMul := bet / jiliUtData.BaseBet

		for _, roundInfo := range witchData.GetMainGame().GetPlateQueue() {
			ut.FloatPtrMul(&roundInfo.SingleWin, betMul)

			for _, awardInfo := range roundInfo.GetAwardDataVec() {
				ut.FloatPtrMul(&awardInfo.Win, betMul)
			}
		}
		for _, roundInfo := range witchData.GetFreeGame().GetFreeQueue() {
			ut.FloatPtrMul(&roundInfo.RoundWin, betMul)
			for _, plate := range roundInfo.GetPlateQueue() {
				ut.FloatPtrMul(&plate.SingleWin, betMul)
				for _, awardInfo := range plate.GetAwardDataVec() {
					ut.FloatPtrMul(&awardInfo.Win, betMul)
				}
			}
		}
		ut.FloatPtrMul(&witchData.MainGame.RoundWin, betMul)
		if len(witchData.GetFreeGame().GetFreeQueue()) != 0 {
			ut.FloatPtrMul(&witchData.FreeGame.FreeTotalWin, betMul)
		}
		ut.FloatPtrMul(&witchData.TotalWin, betMul)
		ut.FloatPtrMul(&witchData.MainGame.ScatterWin, betMul)
		ut.FloatPtrMul(&jiliUtData.TotalWin, betMul)

		//lo.Must0(ut.FloatEQ(spinAck.GetTotalWin(), totalWin))
		totalWin := witchData.GetTotalWin()
		...
		jiliUtData.Data, _ = proto.Marshal(witchData)
