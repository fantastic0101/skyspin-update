...
import (
	...
	"serve/servicejili/jili_239_dbg/internal/gendata"
...
type Fetcher struct {
	...
	launchUrl    *url.URL
	spinUrl      *url.URL
	gameReq      *message.Dbg_GameReqData
	historyToken string
}
...
func newFetcher(username string, coll *mongo.Collection, buy bool) *Fetcher {
	...
	var loginDataReq = message.Dbg_LoginDataReq{
	...
	var gameloginResData message.Dbg_ResData
...
func (f *Fetcher) run() {
	...
	for {
		...
		var spinResData message.Dbg_ResData
		...
		var spinAll message.Dbg_SpinAllData
