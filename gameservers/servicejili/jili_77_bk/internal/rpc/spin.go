...
import (
	...
	"serve/servicejili/jili_77_bk/internal/gamedata"
...
func spin(pid int64, data []byte, ps *nats.Msg) (ret protoreflect.ProtoMessage, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		// 游戏内部数据
		bkData := doc.BkData
		for _, mainInfo := range bkData.MainInfo {
			ut.FloatPtrMul(mainInfo.MainWin, bet)
			ut.FloatPtrMul(mainInfo.ScatterWin, bet)
			for _, plateQueue := range mainInfo.PlateQueue {
				ut.FloatPtrMul(plateQueue.SingleWin, bet)
				for _, awardDataList := range plateQueue.AwardDataList {
					ut.FloatPtrMul(awardDataList.Money, bet)
				}
			}
		}

		for _, freeInfo := range bkData.FreeInfo {
			ut.FloatPtrMul(freeInfo.FreeTotalWin, bet)

			for _, freeQueue := range freeInfo.FreeQueue {
				ut.FloatPtrMul(freeQueue.FreeWin, bet)
				for _, freePlateQueue := range freeQueue.FreePlateQueue {
					ut.FloatPtrMul(freePlateQueue.SingleWin, bet)
					for _, awardDataList := range freePlateQueue.AwardDataList {
						ut.FloatPtrMul(awardDataList.Money, bet)
					}
				}
			}
		...
		ut.FloatPtrMul(bkData.TotalWin, bet)
		// ut.FloatPtrMul(bkData.FreeTotalWin, bet)
		// 通用数据
		...
		bkData.NowMoney = proto.Float64(ut.Gold2Money(balanceGold))
		jiliUtData.PostMoney = bkData.NowMoney
		jiliUtData.Data, _ = proto.Marshal(bkData)
		...
		his.PostMoney = ut.Ftoa(bkData.GetNowMoney())
