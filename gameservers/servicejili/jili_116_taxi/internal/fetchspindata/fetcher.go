...
type Fetcher struct {
	...
	launchUrl    *url.URL
	spinUrl      *url.URL
	gameReq      *message.Taxi_GameReqData
	historyToken string
}
...
func newFetcher(username string, coll *mongo.Collection) *Fetcher {
	...
	var loginDataReq = message.Taxi_LoginDataReq{
	...
	var gameloginResData message.Taxi_ResData
...
func (f *Fetcher) run() {
	...
	for ; ; time.Sleep(time.Second) {
		var spinResData message.Taxi_ResData
		...
		doc := &models.RawSpin{
			...
			HasGame:  len(spinAck.GetRoundQueue()) > 0,
			Data:     spinAck,
			Selected: true,
		}
