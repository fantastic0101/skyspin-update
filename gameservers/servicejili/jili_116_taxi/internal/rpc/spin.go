...
import (
	...
	"serve/servicejili/jili_116_taxi/internal/models"
...
var winReg = regexp.MustCompile(`^(\w+ game win: )([0-9.+-]+)$`) //"Game win : 2"
var winReg2 = regexp.MustCompile(`^(Bonus win : )([0-9.+-]+)$`)  //"Bonus win : 2"

func spin(ps *nats.Msg) (ret []byte, err error) {
	var gameReq message.Taxi_GameReqData
	...
	var spinReq message.Taxi_SpinReq
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		// uid := fmt.Sprintf("%v00000", ut.Generate_jili_19lengthID(time.Now()))
		// fmt.Println("uid is:", uid)
		logid := primitive.NewObjectIDFromTimestamp(now)
		fmt.Println("logid is:", logid)
		...
		for _, pd := range spinAck.RoundQueue {
			ut.FloatPtrMul(pd.RoundWin, bet)
			ut.FloatPtrDiv(pd.RoundWin, internal.BaseBet)
			for _, ad := range pd.AwardDataVec {
				ut.FloatPtrMul(ad.Win, bet)
				ut.FloatPtrDiv(ad.Win, internal.BaseBet)
			}
		}
		...
		//			if poolCost > 0 {
		//				toSelfAwardPool = -costGold * int64(poolCost)
		//			}
		//		}
		...
		var spinAll = message.Taxi_SpinAllData{
			Info: []*message.Taxi_SpinAck{
				spinAck,
			}}
		encode, _ := proto.Marshal(&spinAll)

		var resData = message.Taxi_ResData{
			Type:  proto.Int32(AckType["spin"]),
			Token: proto.String(token),
			Data: []*message.Taxi_InfoData{
				{
					Encode: encode,
				},
			},
