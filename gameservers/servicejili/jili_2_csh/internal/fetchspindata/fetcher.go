...
import (
	...
	"serve/servicejili/jili_2_csh/internal/gendata"
...
type Fetcher struct {
	...
	launchUrl    *url.URL
	spinUrl      *url.URL
	gameReq      *message.Csh_GameReqData
	historyToken string
}
...
func newFetcher(username string, coll *mongo.Collection, buy bool) *Fetcher {
	...
	launchUrl := lo.Must(jiliut.LaunchGame(username, "2", "en-US"))
	...
	var loginDataReq = message.Csh_LoginDataReq{
	...
	var gameloginResData message.Csh_ResData
	...
	f.gameReq = &message.Csh_GameReqData{
		Token: proto.String(token),
		Aid:   proto.Int32(tokenST.Profile.Aid),
		Apiid: proto.Int32(tokenST.Profile.ApiId),
		// Encode: lo.Must(proto.Marshal(&spinReq)),
		Encode: jiliut.ProtoEncode(&message.Csh_SpinReq{
			Bet:     proto.Float64(1),
			MallBet: lo.Ternary(buy, proto.Float64(47.8), nil),
...
func (f *Fetcher) run() {
	...
	for ; ; time.Sleep(time.Second) {
		fmt.Println("拉取数据。。。")
		var spinResData message.Csh_ResData
