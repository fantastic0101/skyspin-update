...
import (
	...
	tmp "serve/servicejili/jili_47_bfs/internal/message"
...
func newFetcher(username string, coll *mongo.Collection, buy bool) *Fetcher {
	...
	launchUrl := lo.Must(jiliut.LaunchGame(username, "47", "en-US"))
	...
	f.gameReq = &jiliUtMessage.Server_Request{
		...
		Req: jiliut.ProtoEncode(&jiliUtMessage.Server_SpinReq{
			...
			MallBet:        lo.Ternary(buy, proto.Float64(36), nil),
		}),
	}
...
func (f *Fetcher) run() bool {
	...
	for ; ; time.Sleep(time.Second) {
		...
		doc := &models.RawSpin{
			...
			HasGame:  len(bkData.GetAckQueue()) != 1,
			BkData:   &bkData,
			UtData:   &spinResponse,
			Selected: true,
		}
