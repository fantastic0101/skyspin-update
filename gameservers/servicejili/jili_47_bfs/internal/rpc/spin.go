...
import (
	...
	"serve/comm/ut"
	"serve/servicejili/jili_47_bfs/internal"
	"serve/servicejili/jili_47_bfs/internal/gamedata"
...
func spin(pid int64, data []byte, ps *nats.Msg) (ret protoreflect.ProtoMessage, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		// 游戏内部数据
		bkData := doc.BkData
		for _, ackQueue := range bkData.GetAckQueue() {
			ut.FloatPtrMul(ackQueue.Win, bet)
			for _, awardData := range ackQueue.AwardDataVec {
				ut.FloatPtrMul(awardData.Win, bet)
			}
			for _, mergedAdv := range ackQueue.MergedADV {
				ut.FloatPtrMul(mergedAdv.Win, bet)
			}
		}
