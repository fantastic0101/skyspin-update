...
import (
	...
	"math"
	"serve/comm/redisx"
	"serve/servicejili/jili_372_dts/internal"
	"serve/servicepp/ppcomm"
	"sync"

	"serve/comm/db"
	"serve/servicejili/jili_372_dts/internal/models"
...
type CombineDataMng struct {
	...
	combineDatasbuy map[string]*CombineDataBuy

	bucketExtras      map[int][][]primitive.ObjectID
	combineExtraDatas map[string]*CombineData
	// 购买游戏2
	buyPlusBucketMap    [][]primitive.ObjectID
	buyPlusBuckets      []primitive.ObjectID
	combinePlusDatasbuy map[string]*CombineDataBuy
	// 购买游戏3
	buyPPlusBucketMap    [][]primitive.ObjectID
	buyPPlusBuckets      []primitive.ObjectID
	combinePPlusDatasbuy map[string]*CombineDataBuy
}
...
func LoadCombineData() (err error) {
	...
	var (
		...
		buyPlusBucketMap  = make([][]primitive.ObjectID, len(GBuckets.bounds))
		buyPlusIds        []primitive.ObjectID
		buyPPlusBucketMap = make([][]primitive.ObjectID, len(GBuckets.bounds))
		buyPPlusIds       []primitive.ObjectID
	)
	for _, doc := range docs {
		...
		if doc.Type == internal.GameTypeGamePlus {
			buyPlusBucketMap[doc.BucketID] = append(buyPlusBucketMap[doc.BucketID], doc.ID)
			buyPlusIds = append(buyPlusIds, doc.ID)
		}
		if doc.Type == internal.GameTypeGamePPlus {
			buyPPlusBucketMap[doc.BucketID] = append(buyPPlusBucketMap[doc.BucketID], doc.ID)
			buyPPlusIds = append(buyPPlusIds, doc.ID)
		}
	}
	...
	GCombineDataMng = &CombineDataMng{
		...
		buckets:              buckets,
		buyBuckets:           buyIds,
		buyPlusBucketMap:     buyPlusBucketMap,
		buyPlusBuckets:       buyPlusIds,
		buyPPlusBucketMap:    buyPPlusBucketMap,
		buyPPlusBuckets:      buyPPlusIds,
		combinePlusDatasbuy:  map[string]*CombineDataBuy{},
		combinePPlusDatasbuy: map[string]*CombineDataBuy{},
...
func (mng *CombineDataMng) Next(bet string, ty, WitchBucket int) (playResp *models.RawSpin, err error) {
	...
	//id, _ := primitive.ObjectIDFromHex("67f63de935e138d346c1f97c")
...
func (mng *CombineDataMng) NextBuy(bet string, ty int) (playResp *models.RawSpin, err error) {
	data := mng.combineDatasbuy[bet]
	if ty == internal.GameTypeGame {
		...
		if data == nil {
			...
			mng.combineDatasbuy[bet] = data
		}
		mng.mtx.Unlock()
	} else if ty == internal.GameTypeGamePlus {
		mng.mtx.Lock()
		data = mng.combinePlusDatasbuy[bet]
		if data == nil {
			data = &CombineDataBuy{
				rawSeries: mng.buyPlusBuckets,
			}
			mng.combinePlusDatasbuy[bet] = data
		}
		mng.mtx.Unlock()
	} else if ty == internal.GameTypeGamePPlus {
		mng.mtx.Lock()
		data = mng.combinePPlusDatasbuy[bet]
		if data == nil {
			data = &CombineDataBuy{
				rawSeries: mng.buyPPlusBuckets,
			}
			mng.combinePPlusDatasbuy[bet] = data
		}
		mng.mtx.Unlock()
	}
...
func (mng *CombineDataMng) GetBuyMinData(ty int) (playResp *models.RawSpin, err error) {
	bucketId := GetBuyMinBucketId()
	data := mng.buyBucketMap[bucketId]
	if ty == internal.GameTypeGamePlus {
		data = mng.buyPlusBucketMap[bucketId]
	} else if ty == internal.GameTypeGamePPlus {
		data = mng.buyPPlusBucketMap[bucketId]
	}
	id := lo.Sample(data)
