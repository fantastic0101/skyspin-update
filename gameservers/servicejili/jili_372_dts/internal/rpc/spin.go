...
func spin(pid int64, data []byte, ps *nats.Msg) (ret protoreflect.ProtoMessage, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		for _, roundInfo := range dtsData.GetRoundQueue() {
		...
		ut.FloatPtrMul(&dtsData.TotalWin, betMul)
		ut.FloatPtrMul(&dtsData.FreeTotalWin, betMul)
		ut.FloatPtrMul(&jiliUtData.TotalWin, betMul)

		//lo.Must0(ut.FloatEQ(spinAck.GetTotalWin(), totalWin))
		totalWin := dtsData.GetTotalWin()
		...
		jiliUtData.Data, _ = proto.Marshal(dtsData)
