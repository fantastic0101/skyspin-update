...
import (
	...
	"serve/servicejili/jili_372_dts/internal/gendata"
...
func nextPlayResp(player *models.Player, bet int64, selfPoolGold int64, ty int, App *redisx.AppStore) (playResp *models.RawSpin, hitBigAward, forcedKill, buyKill bool, err error) {
	...
	if ty != internal.GameTypeNormal && ty != internal.GameTypeExtra {
		...
		if rand.IntN(1000) < minAwardPercent {
			playResp, err = gendata.GCombineDataMng.GetBuyMinData(ty)
			buyKill = true
		} else {
			playResp, err = gendata.GCombineDataMng.NextBuy(fmt.Sprintf("%d", bet), ty)
		}
		return
	}
