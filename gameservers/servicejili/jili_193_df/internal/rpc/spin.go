...
import (
	...
	"serve/comm/define"
	"serve/comm/lazy"
	"serve/servicejili/jili_193_df/internal/gamedata"
...
func init() {
	...
	reqMuxs[AckType.Spin] = spins
}

var winReg = regexp.MustCompile(`^(\w+ game win: )([0-9.+-]+)$`)
var winReg2 = regexp.MustCompile(`^(Lucky Wheel Win : )([0-9.+-]+)$`)

func spin(pid int64, data []byte) (ret protoreflect.ProtoMessage, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		dfData.NowMoney = proto.Float64(ut.Gold2Money(balanceGold))
		jiliUtData.PostMoney = dfData.NowMoney
		jiliUtData.Data, _ = proto.Marshal(dfData)
		...
		his.PostMoney = ut.Ftoa(dfData.GetNowMoney())
...
func spins(pid int64, data []byte, ps *nats.Msg) (ret protoreflect.ProtoMessage, err error) {
	var spinReq jiliUtMessage.Server_SpinReq
	err = proto.Unmarshal(data, &spinReq)
	if err != nil {
		return
