...
func init() {
	jiliut.RegRpc(fmt.Sprintf("/history/%s/get-history-record", internal.GameShortName), get_history_record)

	jiliut.RegRpc(fmt.Sprintf("/history/%s/get-single-round-log-summary/", internal.GameShortName), get_single_round_log_summary)

	jiliut.RegRpc(fmt.Sprintf("/history/%s/get-log-plate-info/", internal.GameShortName), get_log_plate_info)
}
...
func get_log_plate_info(ps *nats.Msg) (ret []byte, err error) {
	...
	// "SymbolLength": "11111112141211141111111111",
	// "Mult": "1",
	// "TopStr": null,
	// "MysterySymbol": null
	plateInfos := doc.LogPlateInfos[id2]
	// for _, item := range plateInfos {
	// 	//item.SymbolLength = "11111112141211141111111111"
	// 	//item.Mult = "1"
	// }
	ret = jiliut.MarshalJsonReturn(plateInfos)
