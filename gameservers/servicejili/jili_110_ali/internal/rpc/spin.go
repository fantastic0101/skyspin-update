...
import (
	...
	"serve/servicejili/jili_110_ali/internal/gamedata"
...
func spin(pid int64, data []byte, ps *nats.Msg) (ret protoreflect.ProtoMessage, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		// doc, hitBigAward, forcedKill, buyKill, err = nextPlayResp(plr, gold, selfPoolGold, isBuy, ty, App.GamePatten, App.MaxMultiple)
		if err != nil {
		...
		for _, roundInfo := range zeusData.GetRoundQueue() {
		...
		for i, sm := range doc.SingleRoundLogSummaries {
			roundInfos := zeusData.RoundQueue[i]
...
func getSymbolLength(stageInfo *message.Ali_ComboStageInfo) (symbolLength string) {
