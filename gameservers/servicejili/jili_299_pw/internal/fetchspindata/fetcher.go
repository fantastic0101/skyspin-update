...
import (
	...
	"serve/servicejili/jili_299_pw/internal/gendata"
	"serve/servicejili/jili_299_pw/internal/message"
	"serve/servicejili/jili_299_pw/internal/models"
...
func newFetcher(username string, coll *mongo.Collection, ty int) *Fetcher {
	...
	launchUrl := lo.Must(jiliut.LaunchGame(username, "299", "en-US"))
...
func (f *Fetcher) run() bool {
	...
	for ; ; time.Sleep(time.Second) {
		...
		var pwData message.Pw_SpinAck
		lo.Must0(proto.Unmarshal(spinResponse.Data, &pwData))
		spinResponse.Data = []byte{}

		if pwData.GetAckType() != 0 {
			slog.Error("pull spin data error", "acktype", pwData.GetAckType(), "user", f.username)
			break
		}

		totalWin := pwData.GetTotalWin()
