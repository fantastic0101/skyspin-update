...
func spin(pid int64, data []byte, ps *nats.Msg) (ret protoreflect.ProtoMessage, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		// 游戏内部数据
		PwData := doc.PWData
		for _, roundInfo := range PwData.AwardDataVec {
			ut.FloatPtrMul(roundInfo.Win, bet)

		}
		ut.FloatPtrMul(PwData.TotalWin, bet)
		...
		jiliUtData.Data, _ = proto.Marshal(PwData)
		...
		//his.PostMoney = ut.Ftoa(PwData.GetNowMoney())
