...
import (
	...
	"serve/servicejili/jili_37_sweetheart5/internal/gendata"
	"serve/servicejili/jili_37_sweetheart5/internal/message"
...
func newFetcher(username string, coll *mongo.Collection) *Fetcher {
	...
	launchUrl := lo.Must(jiliut.LaunchGame(username, "37", "en-US"))
...
func (f *Fetcher) run() {
	...
	for ; ; time.Sleep(time.Second) {
		...
		var sweetheart5Data message.Custom_AllPlate
		lo.Must0(proto.Unmarshal(spinResponse.Data, &sweetheart5Data))
		spinResponse.Data = []byte{}

		if len(sweetheart5Data.Plate) <= 0 {
		...
		for _, plate := range sweetheart5Data.Plate {
		...
		doc := &models.RawSpin{
			...
			Times:           totalWin, // bet is 1.0 for csh
			Type:            gendata.GameTypeNormal,
			HasGame:         len(sweetheart5Data.Plate) > 1,
			Sweetheart5Data: &sweetheart5Data,
			UtData:          &spinResponse,
			Selected:        true,
		}
