...
func init() {
	jiliut.RegRpc(fmt.Sprintf("/history/%s/get-history-record", "nc"), get_history_record)

	jiliut.RegRpc(fmt.Sprintf("/history/%s/get-single-round-log-summary/", "nc"), get_single_round_log_summary)

	jiliut.RegRpc(fmt.Sprintf("/history/%s/get-log-plate-info/", "nc"), get_log_plate_info)
}
