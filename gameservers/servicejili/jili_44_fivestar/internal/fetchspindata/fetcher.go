...
import (
	...
	"serve/servicejili/jili_44_fivestar/internal/gendata"
...
type Fetcher struct {
	...
	launchUrl    *url.URL
	spinUrl      *url.URL
	gameReq      *message.Server_Request
	historyToken string
	token        string
}
...
func newFetcher(username string, coll *mongo.Collection, ty int) *Fetcher {
	...
	launchUrl := lo.Must(jiliut.LaunchGame(username, "44", "en-US"))
	...
	var loginDataReq = message.Server_InfoReq{
		Os:       proto.String("Windows"),
		Language: proto.String("en-US"),
		Browser: []*message.Server_Browser{
			{
	...
	var gameloginResData message.Server_InfoReq
	...
	f.gameReq = &message.Server_Request{
		Ack: proto.Int32(0),
		//jiliUtMessage
		Req: jiliut.ProtoEncode(&message.Server_SpinReq{

			//	BuyCustomType: proto.Int32(0),
			Bet: proto.Int64(1000),
			//MallBet: lo.Ternary(buy, proto.Int64(43.5), nil),
		}),
	}
...
func (f *Fetcher) run() bool {
	...
	for ; ; time.Sleep(time.Second) {
		fmt.Println("拉取数据。。。")
		var response message.Server_Response
		...
		var spinResponse message.Server_SpinResponse
		lo.Must0(proto.Unmarshal(response.Response, &spinResponse))

		var fivestarData message.Custom_AllPlate
		lo.Must0(proto.Unmarshal(spinResponse.AllPlate, &fivestarData))
		spinResponse.AllPlate = []byte{}

		if len(fivestarData.Plate) <= 0 {
			slog.Error("pull spin data error", " plate length is 0", " err")
			break
		}

		//totalWin := fivestarData.GetTotalWin()

		var totalWin int64
		for _, plate := range fivestarData.Plate {
