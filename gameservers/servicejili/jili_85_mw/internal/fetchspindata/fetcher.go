...
import (
	...
	"serve/servicejili/jili_85_mw/internal/gendata"
...
type Fetcher struct {
	...
	launchUrl    *url.URL
	spinUrl      *url.URL
	gameReq      *message.Mw_GameReqData
	historyToken string
}
...
func newFetcher(username string, coll *mongo.Collection, buy bool) *Fetcher {
	...
	var loginDataReq = message.Mw_LoginDataReq{
	...
	var gameloginResData message.Mw_ResData
	...
	f.gameReq = &message.Mw_GameReqData{
		Token: proto.String(token),
		Aid:   proto.Int32(tokenST.Profile.Aid),
		Apiid: proto.Int32(tokenST.Profile.ApiId),
		// Encode: lo.Must(proto.Marshal(&spinReq)),
		Encode: jiliut.ProtoEncode(&message.Mw_SpinReq{
			Bet:     proto.Float64(1),
			MallBet: lo.Ternary(buy, proto.Float64(41.5), nil),
...
func (f *Fetcher) run() {
	...
	for ; ; time.Sleep(time.Second) {
		var spinResData message.Mw_ResData
		...
		doc := &models.RawSpin{
			...
			HasGame:  spinAck.GetFreeTotalWin() > float64(0),
			Data:     spinAck,
			Selected: true,
		}
