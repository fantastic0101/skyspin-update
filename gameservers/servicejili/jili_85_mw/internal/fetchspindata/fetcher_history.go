...
func (f *Fetcher) fetchHisotry(roundIndex string, doc *models.RawSpin) {
	...
	historyRecodeQuery := lo.Must(url.ParseQuery("EndRowIndex=1&LangId=zh-CN&LogIndexAsRoundIndex=false&RoundIndex=1718339075110445002&StartRowIndex=1"))
	...
	platcomm.Retry(20, 3*time.Second, func() bool {
		lo.Must0(f.invokeHistory("/history/mw/get-history-record", historyRecodeQuery.Encode(), &records))
	...
	lo.Must0(f.invokeHistory("/history/mw/get-single-round-log-summary/en-US/"+roundIndex, "", &summaries))
	...
	for _, summary := range summaries {
		...
		path := fmt.Sprintf("/history/mw/get-log-plate-info/%s/%s", roundIndex, summary.LogIndex)
