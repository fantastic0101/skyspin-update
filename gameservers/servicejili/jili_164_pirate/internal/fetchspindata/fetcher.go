...
import (
	...
	"serve/servicejili/jili_164_pirate/internal"
	"serve/servicejili/jili_164_pirate/internal/gendata"
	"serve/servicejili/jili_164_pirate/internal/message"
	"serve/servicejili/jili_164_pirate/internal/models"
...
func newFetcher(username string, coll *mongo.Collection, ty int) *Fetcher {
	...
	launchUrl := lo.Must(jiliut.LaunchGame(username, "108", "en-US"))
	...
	f.gameReq = &jiliUtMessage.Server_Request{
		...
		Req: jiliut.ProtoEncode(&jiliUtMessage.Server_SpinReq{
			...
			MallBet: lo.Ternary(ty == internal.GameTypeGame, proto.Float64(33.5), nil),
		}),
	}
...
func (f *Fetcher) run() bool {
	...
	for ; ; time.Sleep(time.Second) {
		...
		var mw3Data message.Mw3_SpinAck
		lo.Must0(proto.Unmarshal(spinResponse.Data, &mw3Data))
		spinResponse.Data = []byte{}

		if mw3Data.GetAckType() != 0 {
			slog.Error("pull spin data error", "acktype", mw3Data.GetAckType(), "user", f.username)
			break
		}
		...
		doc := &models.RawSpin{
			...
			Type:     f.ty,
			HasGame:  len(mw3Data.GetRoundQueue()) > 1,
			Mw3Data:  &mw3Data,
			UtData:   &spinResponse,
			Selected: true,
		}
		...
		if rand.Int()%100 > 70 {
			...
			} else if f.ty == internal.GameTypeGame {
				if cnt >= 30000 {
					wg.Done()
					return true
				}
