...
func (f *Fetcher) fetchHisotry(roundIndex string, doc *models.RawSpin) {
	...
	platcomm.Retry(20, 3*time.Second, func() bool {
		lo.Must0(f.invokeHistory("/history/mw3/get-history-record", historyRecodeQuery.Encode(), &records))
	...
	lo.Must0(f.invokeHistory("/history/mw3/get-single-round-log-summary/en-US/"+roundIndex, "", &summaries))
	...
	for _, summary := range summaries {
		...
		path := fmt.Sprintf("/history/mw3/get-log-plate-info/%s/%s", roundIndex, summary.LogIndex)
