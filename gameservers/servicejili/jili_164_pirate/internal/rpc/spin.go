...
func spin(pid int64, data []byte, ps *nats.Msg) (ret protoreflect.ProtoMessage, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		if isBuy {
			...
			mul = 77.5
			costGold = int64(float64(costGold) * mul)
		}
		...
		pirateData.TotalWin = totalWin
		for _, roundInfo := range pirateData.GetRoundQueue() {
			ut.FloatPtrMul(&roundInfo.RoundWin, betMul)
			for _, comboStageInfo := range roundInfo.ComboStageData {
				ut.FloatPtrMul(&comboStageInfo.ComboStageWin, betMul)
				for _, awardData := range comboStageInfo.AwardDataVec {
					ut.FloatPtrMul(&awardData.Win, betMul)
				}
			}
		}
		...
		jiliUtData.Data, _ = proto.Marshal(pirateData)
