...
type Fetcher struct {
	...
	launchUrl    *url.URL
	spinUrl      *url.URL
	gameReq      *message.Fb_GameReqData
	historyToken string
}
...
func newFetcher(username string, coll *mongo.Collection, ty int) *Fetcher {
	...
	var loginDataReq = message.Fb_LoginDataReq{
	...
	var gameloginResData message.Fb_ResData
...
func (f *Fetcher) run() bool {
	...
	for {
		...
		var spinResData message.Fb_ResData
		...
		var spinAll message.Fb_SpinAllData
