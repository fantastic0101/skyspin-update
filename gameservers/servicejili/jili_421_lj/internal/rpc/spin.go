...
import (
	...
	"serve/comm/define"
	"serve/comm/lazy"
	"serve/servicejili/jili_421_lj/internal/gamedata"
...
func spin(pid int64, data []byte, ps *nats.Msg) (ret protoreflect.ProtoMessage, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		mallBet := 0.0
		//if strings.HasPrefix(lazy.ServiceName, "jili") {
		//	mallBet = spinReq.GetMallBet()
		//} else if len(&spinReq.Mall) > 0 {
		...
		for _, roundInfo := range ljData.GetAwardDataVec() {
			ut.FloatPtrMul(&roundInfo.Win, betMul)
		}
		for _, wildOdd := range ljData.GetWildOdd() {
			ut.FloatPtrMul(&wildOdd, betMul)
		}
		ut.FloatPtrMul(&ljData.WildWin, betMul)
		ut.FloatPtrMul(&ljData.TotalWin, betMul)
		...
		jiliUtData.Data, _ = proto.Marshal(ljData)
