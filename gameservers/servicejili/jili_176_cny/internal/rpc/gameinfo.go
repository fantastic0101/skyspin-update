...
var BetGrade = []float64{1, 0.6, 1.2, 1.8, 2.4, 3, 6, 12, 18, 24, 30, 60, 120, 180, 300, 600}
...
func gameinfo(ps *nats.Msg) (ret []byte, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		// aid := int32(pid)
		// var ack = message.Cny_LoginDataAck{
		// 	Aid: &aid,
		// }
		...
		var ack = message.Cny_GameInfoAck{
			...
			Prefer:    []*message.Cny_PreferRoundShow{},
			ShowExtra: proto.Bool(true),
			Test:      proto.Bool(true),
			WalletInfo: []*message.Cny_WalletInfo{
				{
