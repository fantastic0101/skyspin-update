...
import (
	...
	"serve/servicejili/jili_115_aa/internal/gendata"
...
type Fetcher struct {
	...
	launchUrl    *url.URL
	spinUrl      *url.URL
	gameReq      *message.Aa_GameReqData
	historyToken string
}
...
func newFetcher(username string, coll *mongo.Collection, buy bool) *Fetcher {
	...
	var loginDataReq = message.Aa_LoginDataReq{
	...
	var gameloginResData message.Aa_ResData
...
func (f *Fetcher) run() {
	...
	for {
		...
		var spinResData message.Aa_ResData
		...
		var spinAll message.Aa_SpinAllData
