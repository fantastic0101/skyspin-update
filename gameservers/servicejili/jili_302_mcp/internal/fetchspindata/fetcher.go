...
import (
	...
	"serve/servicejili/jili_302_mcp/internal/gendata"
	"serve/servicejili/jili_302_mcp/internal/message"
...
func newFetcher(username string, coll *mongo.Collection) *Fetcher {
	...
	launchUrl := lo.Must(jiliut.LaunchGame(username, "21", "en-US"))
...
func (f *Fetcher) run() {
	...
	for ; ; time.Sleep(time.Second) {
		...
		var olsData message.Ols_SpinAck
		lo.Must0(proto.Unmarshal(spinResponse.Data, &olsData))
		spinResponse.Data = []byte{}

		//todo
		// totalWin := zeusData.GetWin()
		totalWin := olsData.GetMoney()
