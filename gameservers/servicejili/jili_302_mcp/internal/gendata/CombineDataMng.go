...
import (
	...
	"math"
	"serve/comm/redisx"
	"serve/servicejili/jili_302_mcp/internal"
...
func LoadCombineData() (err error) {
	...
	type bucketGroups struct {
		...
		normal2 [][]primitive.ObjectID
		normal3 [][]primitive.ObjectID
		normal4 [][]primitive.ObjectID
		normal5 [][]primitive.ObjectID
		normal6 [][]primitive.ObjectID
		normal7 [][]primitive.ObjectID
	}

	bucketConfig := map[int]*bucketGroups{
		ppcomm.BucketHeartBeat:      {makeSlice(), makeSlice(), makeSlice(), makeSlice(), makeSlice(), makeSlice(), makeSlice()},
		ppcomm.BucketWave:           {makeSlice(), makeSlice(), makeSlice(), makeSlice(), makeSlice(), makeSlice(), makeSlice()},
		ppcomm.BucketGov:            {makeSlice(), makeSlice(), makeSlice(), makeSlice(), makeSlice(), makeSlice(), makeSlice()},
		ppcomm.BucketMix:            {makeSlice(), makeSlice(), makeSlice(), makeSlice(), makeSlice(), makeSlice(), makeSlice()},
		ppcomm.BucketStable:         {makeSlice(), makeSlice(), makeSlice(), makeSlice(), makeSlice(), makeSlice(), makeSlice()},
		ppcomm.BucketHighAward:      {makeSlice(), makeSlice(), makeSlice(), makeSlice(), makeSlice(), makeSlice(), makeSlice()},
		ppcomm.BucketSuperHighAward: {makeSlice(), makeSlice(), makeSlice(), makeSlice(), makeSlice(), makeSlice(), makeSlice()},
	}
	//var (
	...
	for _, doc := range docs {
		...
		handleBucket := func(bucketType int, config *bucketGroups, isOff bool) {
			if isOff {
				if doc.Type == internal.GameTypeBet1 {
					config.normal[doc.BucketID] = append(config.normal[doc.BucketID], doc.ID)
				}
				if doc.Type == internal.GameTypeBet5 {
					config.normal2[doc.BucketID] = append(config.normal2[doc.BucketID], doc.ID)
				}
				if doc.Type == internal.GameTypeBet10 {
					config.normal3[doc.BucketID] = append(config.normal3[doc.BucketID], doc.ID)
				}
				if doc.Type == internal.GameTypeBet50 {
					config.normal4[doc.BucketID] = append(config.normal4[doc.BucketID], doc.ID)
				}
				if doc.Type == internal.GameTypeBet100 {
					config.normal5[doc.BucketID] = append(config.normal5[doc.BucketID], doc.ID)
				}
				if doc.Type == internal.GameTypeBet500 {
					config.normal6[doc.BucketID] = append(config.normal6[doc.BucketID], doc.ID)
				}
				if doc.Type == internal.GameTypeBet1000 {
					config.normal7[doc.BucketID] = append(config.normal7[doc.BucketID], doc.ID)
				}
			}
		}
	...
	buckets := make([]map[int][][]primitive.ObjectID, 7)
	buckets1 := make(map[int][][]primitive.ObjectID, len(bucketConfig))
	buckets2 := make(map[int][][]primitive.ObjectID, len(bucketConfig))
	buckets3 := make(map[int][][]primitive.ObjectID, len(bucketConfig))
	buckets4 := make(map[int][][]primitive.ObjectID, len(bucketConfig))
	buckets5 := make(map[int][][]primitive.ObjectID, len(bucketConfig))
	buckets6 := make(map[int][][]primitive.ObjectID, len(bucketConfig))
	buckets7 := make(map[int][][]primitive.ObjectID, len(bucketConfig))
	for k, v := range bucketConfig {
		buckets1[k] = v.normal
		buckets2[k] = v.normal2
		buckets3[k] = v.normal3
		buckets4[k] = v.normal4
		buckets5[k] = v.normal5
		buckets6[k] = v.normal6
		buckets7[k] = v.normal7
	}
	buckets[internal.GameTypeBet1] = buckets1
	buckets[internal.GameTypeBet5] = buckets2
	buckets[internal.GameTypeBet10] = buckets3
	buckets[internal.GameTypeBet50] = buckets4
	buckets[internal.GameTypeBet100] = buckets5
	buckets[internal.GameTypeBet500] = buckets6
	buckets[internal.GameTypeBet1000] = buckets7
...
func (mng *CombineDataMng) Next(bet string, ty, WitchBucket int) (playResp *models.RawSpin, err error) {
	...
	//id, _ = primitive.ObjectIDFromHex(`67f3befd36cf57c2a2632f1e`)
...
func (mng *CombineDataMng) GetBigReward(ty, witchBucket int) (playResp *models.RawSpin, err error) {
	...
	ids = lo.Filter(ids, func(item int, index int) bool {
		return len(mng.buckets[ty][witchBucket][item]) != 0
	})
	lo.Must0(len(ids) != 0)
	bucketid := lo.Sample(ids)
	data := mng.buckets[ty][witchBucket][bucketid]
	id = lo.Sample(data)
...
func (mng *CombineDataMng) SampleForceSimulate(ty, witchBucket int) (playResp *models.RawSpin, err error) { //todo
	var id primitive.ObjectID
	if ty == internal.GameTypeBet1 {
		data := mng.buckets[internal.GameTypeBet1][witchBucket][0]
		id = lo.Sample(data)
	}
	if ty == internal.GameTypeBet5 {
		data := mng.buckets[internal.GameTypeBet5][witchBucket][8]
		id = lo.Sample(data)
	}
	if ty == internal.GameTypeBet10 {
		data := mng.buckets[internal.GameTypeBet10][witchBucket][16]
		id = lo.Sample(data)
	}
	if ty == internal.GameTypeBet50 {
		data := mng.buckets[internal.GameTypeBet50][witchBucket][24]
		id = lo.Sample(data)
	}
	if ty == internal.GameTypeBet100 {
		data := mng.buckets[internal.GameTypeBet100][witchBucket][32]
		id = lo.Sample(data)
	}
	if ty == internal.GameTypeBet500 {
		data := mng.buckets[internal.GameTypeBet500][witchBucket][40]
		id = lo.Sample(data)
	}
	if ty == internal.GameTypeBet1000 {
		data := mng.buckets[internal.GameTypeBet1000][witchBucket][48]
		id = lo.Sample(data)
	}
	return findFromSimulate(id)
}

func (mng *CombineDataMng) get(bet string, ty int) (data *CombineData) {
	mng.mtx.Lock()
...
func (mng *CombineDataMng) ControlNextData(nextData *redisx.NextMulti, gameType int, witchBucket int) (playResp *models.RawSpin, err error) {
	...
	bucketid := lo.Sample(ids)
	data := mng.buckets[gameType][witchBucket][bucketid]
