...
func init() {
	jiliut.RegRpc(fmt.Sprintf("/history/%s/get-history-record", "mcp"), get_history_record)

	jiliut.RegRpc(fmt.Sprintf("/history/%s/get-single-round-log-summary/", "mcp"), get_single_round_log_summary)

	jiliut.RegRpc(fmt.Sprintf("/history/%s/get-log-plate-info/", "mcp"), get_log_plate_info)
}
...
func get_single_round_log_summary(ps *nats.Msg) (ret []byte, err error) {
	...
	//res := doc.SingleRoundLogSummaries
	//for _, val := range res {
	//	val.StageIndex = "0"
	//	val.Plate = make([]*struct{}, 0)
	//}
...
func get_log_plate_info(ps *nats.Msg) (ret []byte, err error) {
	...
	//res := doc.LogPlateInfos[id2]
	//res[0].JPWinList = make([]*struct{}, 0)
	//res[0].MysterySymbol = nil
	//res[0].GoldCount = nil
	//res[0].Title = nil
