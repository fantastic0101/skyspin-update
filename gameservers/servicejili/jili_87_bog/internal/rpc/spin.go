...
func spin(pid int64, data []byte, ps *nats.Msg) (ret protoreflect.ProtoMessage, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		ut.FloatPtrMul(&bogData.TotalWin, betMul)
		ut.FloatPtrMul(&bogData.FreeTotalWin, betMul)
		for _, roundInfo := range bogData.GetRoundQueue() {
			...
			for _, temp := range roundInfo.FreeExpandInfo {
				ut.FloatPtrMul(&temp.Win, betMul)
			}
		}
		...
		jiliUtData.Data, _ = proto.Marshal(bogData)
