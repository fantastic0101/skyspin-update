...
import (
	...
	"serve/servicejili/jili_5_chilli/internal/gamedata"
	"serve/servicejili/jili_5_chilli/internal/gendata"
...
func nextPlayResp(player *models.Player, bet int64, selfPoolGold int64, App *redisx.AppStore) (playResp *models.RawSpin, hitBigAward, forcedKill bool, err error) {
	...
	if false {
		...
		t := gendata.GameTypeNormal

		//spid, _ := primitive.ObjectIDFromHex("667916eaec22674487ef1316")
		cur := lo.Must(coll.Aggregate(context.TODO(), mongo.Pipeline{
			db.D("$match", db.D("type", t)),
			//db.D("$match", db.D("type", t, "_id", spid)),
		...
		spindoc := spindocs[0]
		fmt.Println("id is:", spindoc.ID.Hex())
		return spindoc, false, false, err
	}

	//限制
	if player.RestrictionsStatus == 1 {
		if player.Win >= player.RestrictionsMaxWin*10000 || player.Multi >= player.RestrictionsMaxMulti {
			playResp, err = gendata.GCombineDataMng.SampleForceSimulate(App.GamePatten)
			forcedKill = true
			return
		}
	}
	...
	if hitPermillageArr(gamedata.Settings.HitBigAwardPercent, player.SpinCountOfThisEnter) {
		if bet*40 < selfPoolGold {
			avg := player.CaclAvgBet()
			if avg*40 < selfPoolGold {
				playResp, err = gendata.GCombineDataMng.GetBigReward(App.GamePatten)
				hitBigAward = true
				return
			}
