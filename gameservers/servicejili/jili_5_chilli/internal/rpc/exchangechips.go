...
func init() {
	jiliut.RegRpc("/chilli/account/exchangechips", exchangechips)
}

// /csh/account/exchangechips
func exchangechips(ps *nats.Msg) (ret []byte, err error) {
	var req message.Chilli_ExchangeReq
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		var ack = message.Chilli_ExchangeAck{
			Stamp: proto.String(strconv.Itoa(int(stamp))),
			Wallet: []*message.Chilli_Wallet{
				{
