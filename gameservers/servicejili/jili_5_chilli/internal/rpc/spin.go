...
func spin(ps *nats.Msg) (ret []byte, err error) {
	...
	var spinReq message.Chilli_SpinReq
	...
	err = db.CallWithPlayer(pid, func(plr *models.Player) error {
		...
		for _, singlePlate := range spinAck.PlateData {
			for _, laInfo := range singlePlate.LineAwardInfo {
				ut.FloatPtrMul(laInfo.WinMoney, bet)
				totalWin += laInfo.GetWinMoney()
				for _, v := range laInfo.LineAwardDataVec {
					ut.FloatPtrMul(v.Win, bet)
				}
			}
			for _, saInfo := range singlePlate.ScatterAwardInfo {
				ut.FloatPtrMul(saInfo.Money, bet)
				totalWin += saInfo.GetMoney()
			}
		}

		for _, afp := range spinAck.AllFreePlateDataPerRoundVec {
			for _, fpa := range afp.FreePlateDataVec {
				for _, laInfo := range fpa.LineAwardInfo {
					ut.FloatPtrMul(laInfo.WinMoney, bet)
					totalWin += laInfo.GetWinMoney()
					for _, v := range laInfo.LineAwardDataVec {
						ut.FloatPtrMul(v.Win, bet)
				...
				for _, saInfo := range fpa.ScatterAwardInfo {
					ut.FloatPtrMul(saInfo.Money, bet)
					totalWin += saInfo.GetMoney()
				}
			}
		...
		var spinAll = message.Chilli_SpinAllData{
			Info: []*message.Chilli_SpinAck{
				spinAck,
			}}
