...
func init() {
	jiliut.RegRpc(fmt.Sprintf("/history/%s/get-history-record", "hc"), get_history_record)

	jiliut.RegRpc(fmt.Sprintf("/history/%s/get-single-round-log-summary/", "hc"), get_single_round_log_summary)

	jiliut.RegRpc(fmt.Sprintf("/history/%s/get-log-plate-info/", "hc"), get_log_plate_info)
}
