...
type Fetcher struct {
	...
	launchUrl    *url.URL
	spinUrl      *url.URL
	gameReq      *message.Chilli_GameReqData
	historyToken string
}
...
func newFetcher(username string, coll *mongo.Collection, buy bool) *Fetcher {
	...
	var loginDataReq = message.Chilli_LoginDataReq{
	...
	var gameloginResData message.Chilli_ResData
...
func (f *Fetcher) run() {
	...
	for {
		...
		var spinResData message.Chilli_ResData
		...
		doc := &models.RawSpin{
			...
			//HasGame:  len(spinAck.FreeGamePackage) > 1,
			HasGame:  len(spinAck.AllFreePlateDataPerRoundVec) > 1,
			Data:     spinAck,
			Selected: true,
		}
