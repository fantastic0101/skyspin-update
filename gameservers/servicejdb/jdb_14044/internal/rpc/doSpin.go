...
import (
	...
	"serve/servicejdb/jdb_14044/internal/gendata"
...
func doSpin(msg *nats.Msg) (ret []byte, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *jdbcomm.Player) error {
		...
		bets, _ := internal.RestoreBet(extraBetType, bet)

		if !ut.FloatInArr(ut.FloatArrMul(App.Cs, curItem.Multi*internal.Line), bets) {
