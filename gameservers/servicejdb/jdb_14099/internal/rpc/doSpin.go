...
import (
	...
	"serve/comm/ut"
	"serve/servicejdb/jdb_14099/internal"
	"serve/servicejdb/jdb_14099/internal/gendata"
...
func doSpin(msg *nats.Msg) (ret []byte, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *jdbcomm.Player) error {
		...
		case "ExtraBet_01":
			Multi = curItem.Multi * internal.Line * 1.25
		}
		// 参数处理
		...
		if isBuy {
			...
			i := internal.FeatureType(buyFeatureType)
			mul = internal.GetFreeMultiply()[i]
			costGold *= mul
		}
		...
		doc, hitBigAward, forcedKill, buyKill, err = jdbcomm.NextPlayResp(&jdbcomm.NextPlayRespParam{
			...
			NextSuperBuyFunc:       gendata.GCombineDataMng.NextSuperBuy,          //购买游戏轮的次获取，包括首轮和后续轮次
