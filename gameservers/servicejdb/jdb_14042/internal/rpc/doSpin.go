...
import (
	...
	"serve/comm/ut"
	"serve/servicejdb/jdb_14042/internal"
	"serve/servicejdb/jdb_14042/internal/gendata"
...
func doSpin(msg *nats.Msg) (ret []byte, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *jdbcomm.Player) error {
		...
		switch extraBetType {
		case "NoExtraBet":
			Multi = curItem.Multi * internal.Line
		case "ExtraBet_01":
			Multi = curItem.Multi * internal.Line * internal.Double
		}
		// 参数处理
		...
		if !ut.FloatInArr(ut.FloatArrMul(App.Cs, Multi), bet) {
		...
		mul := int64(1)
		if isBuy && internal.BuyBetMulti != 0 {

			mul = int64(internal.GetFreeMultiply()[buyFeatureType])
			costGold *= mul
		}

		originBet := bet
		if isDouble {
			originBet = bet / internal.Double
		}
		...
		allWin := ut.JdbBet2Money(ut.GetFloat(doc.Times) * originBet)
		...
		data := doc.Deal2(originBet, ut.Gold2Money(balanceGold))
		balanceGold, err = slotsmongo.ModifyGold(&slotsmongo.ModifyGoldPs{
			Pid:     plr.PID,
		...
		if isBuy {
			...
			if isDouble {
				toSelfAwardPool += int64(float32(bet) * internal.Double)
			}
		}
