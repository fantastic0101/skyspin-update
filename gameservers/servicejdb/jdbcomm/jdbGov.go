...
type SpinFunc func() (SFSObject, int)
...
type Game struct {
	...
	Spin        SpinFunc `json:"spin"`
	WeightMap   map[int]SpinFunc
	CurrentType int
}
				...
				if ok {
					...
					if action == 13 {
						...
						if c == "h5.spinResponse" {
							...
							fmt.Println("success -------->", s.DBName, objectId.Hex(), "type", s.CurrentType)
...
// 根据权重随机选一个下注
func (s *Game) RoundSpin() SFSObject {
	so := SFSObject{}
	allWeight := 0
	for k, _ := range s.WeightMap {
		allWeight += k
	}
	randNum := rand.Int() % allWeight
	for k, v := range s.WeightMap {
		randNum -= k
		if randNum < 0 {
			so, s.CurrentType = v()
			break
