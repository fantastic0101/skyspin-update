...
import (
	...
	"serve/comm/ut"
	"serve/servicejdb/jdb_14098/internal"
	"serve/servicejdb/jdb_14098/internal/gendata"
...
func doSpin(msg *nats.Msg) (ret []byte, err error) {
	...
	err = db.CallWithPlayer(pid, func(plr *jdbcomm.Player) error {
		...
		case "ExtraBet_01":
			Multi = curItem.Multi * internal.Line * 1.2
		}
		// 参数处理
		...
		if isBuy {
			...
			mul = internal.GetFreeMultiply()[buyFeatureType]
			costGold *= mul
		}
		...
		data := doc.Deal2(bet, ut.Gold2Money(balanceGold))
		balanceGold, err = slotsmongo.ModifyGold(&slotsmongo.ModifyGoldPs{
			Pid:     plr.PID,
