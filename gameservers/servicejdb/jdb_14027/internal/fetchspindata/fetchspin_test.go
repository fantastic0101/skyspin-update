...
import (
	...
	"serve/comm/db"
	"serve/comm/robot"
	"serve/servicejdb/jdb_14027/internal"
	"serve/servicejdb/jdb_14027/internal/gendata"
)
...
func normal() {
	fetcher := jdbcomm.NewFetcher(internal.GameID, "testusertjh_01")
	r := &robot.Robot{}
	ppR := jdbcomm.NewPPRobot(r, fetcher, &jdbcomm.FetchDataParam{
...
func game() {
	fetcher := jdbcomm.NewFetcher(internal.GameID, "testusertjhgame_01")
	r := &robot.Robot{}
	ppR := jdbcomm.NewPPRobot(r, fetcher, &jdbcomm.FetchDataParam{
		Game: internal.GameID,
		ArrC: map[float64]int{ // 游戏下注挡位，k：下注值，v:权重
