<template>
   ...
   <span style="font-size: 18px;color: #4d93fb; font-weight: bolder">RTP</span>



  <!--  系统语言  -->
    <Dialog v-model="dialogVisible" :title="systemText.RTPSetting['title']" width="50%" max-width="450px" @mask-close="maskClose">
        <div class="dialog-list">
            ...
            <el-form ref="form" :model="RTPForm" label-width="70px" label-position="top" style="width: 100%;display: flex;justify-content: center">
                <el-form-item style="width: 100%; max-width: 300px">
                    ...
                    <template #label>
                        <div class="setRtp__label">{{systemText.RTPSetting['label']}}:</div>
                    </template>

                    <el-select style="width: 100%; max-width: 300px" v-model="RTPForm.RTP" popper-class="RTPSelect">
                        <template v-for="item in RTPList">
                            <el-option :label="item + '%' + `${Number(item) == 96 ? `(${systemText.RTPSetting['comm']})` : ''}`" :value="Number(item)" v-if="appId == 'faketrans' || (appId != 'faketrans' && item <= 120)"/>
                        </template>
                    </el-select>
                </el-form-item>
            </el-form>
        ...
        <div class="dialog--foot">
            <el-button type="default" @click="closeDialog">{{systemText.RTPSetting['cancel']}}</el-button>
            <el-button type="success" @click="saveRTP">{{systemText.RTPSetting['sure']}}</el-button>
        </div>
