...
type betlistRet struct {
	Title       []string
	List        [][]any
	NextStartID primitive.ObjectID
}
...
func v1_record_bet_list(app *operator.MemApp, ps betlistPs, ret *betlistRet) (err error) {
	...
	if true {
		now := time.Now()
		if d := now.Sub(app.LastPullHisTime); d < 9*time.Second {
			// err = errors.New("Too frequently")
			// ret.List = [][]any{}
			// return
			slog.Info("Too frequently!!", "appid", app.AppID, "delta", d)
		}
	...
	for i, betlog := range betlogs {
		...
		row[titlekeys["GameType"]] = betlog.GameType
		row[titlekeys["BonusCode"]] = betlog.BonusCode
...
func v1_record_bet_list_csv(app *operator.MemApp, ps betlistPs, ret *betlistCSVRet) (err error) {
	...
	for i, betlog := range betlogs {
		...
		row[titlekeys["ID"]] = betlog.ID.Hex()
		...
		row[titlekeys["RoundID"]] = betlog.RoundID
		row[titlekeys["GameType"]] = strconv.Itoa(betlog.GameType)
		row[titlekeys["BonusCode"]] = betlog.BonusCode
