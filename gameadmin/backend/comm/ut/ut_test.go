...
func TestReverseProxy(t *testing.T) {
	m, _ := NewReverseProxyUrlMap(map[string]string{
		"static-pg.rpgamestest.com": "http://localhost/1",
		"static-pg.":                "http://localhost/1-1",
		"static-":                   "http://localhost/1-0",
		"static-pg.xxx.":            "http://localhost/1-2",
		"m-pg.rpgamestest.com":      "http://localhost/2",
		"public-pg.rpgamestest.com": "http://localhost/3",
	})

	assert.Equal(t, "/1", m.Get("static-pg.rpgamestest.com").Path)
	assert.Equal(t, "/1-2", m.Get("static-pg.xxx.com").Path)
	assert.Equal(t, "/1-1", m.Get("static-pg.xx.com").Path)
	assert.Equal(t, "/1-0", m.Get("static-pgg.yy.com").Path)
}
