...
func PostJsonRaw(ctx context.Context, url string, body any, fn1 func(req *http.Request)) (respBody []byte, err error) {
	cli := getHttpClient()

	lg := slog.With("url", url)
	defer func() {
		lg.Info("PostJsonRaw", "respBody", string(respBody), "err", err)
	}()
	...
	lg = lg.With("reqBody", string(bodyBytes))
