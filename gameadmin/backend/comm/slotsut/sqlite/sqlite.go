...
func (this *DB) GetAllTableNames() (names []string, err error) {
	rows, err := this.Query(fmt.Sprintf("select name from sqlite_master where type='table';"))
	if err != nil {
		fmt.Println("error:", err)
		return nil, err
	}
	names = ResultPan(rows)
	return
}
...
func (this *DB) GetCountFromTableByID(tablename string, id int) (count int, err error) {
	err = this.QueryRow(fmt.Sprintf("select count from %v where id = %v;", tablename, id)).Scan(&count)
	return
}
