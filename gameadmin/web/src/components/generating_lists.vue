...
<template>
    <div>
        <el-space wrap class="small-font">
            ...
            {{ $t("总回报率") }}：{{ uiData.value['outtotalbeishubi'] == '' ? "" : parseFloat(uiData.value['outtotalbeishubi']).toFixed(4) }}%
            {{ $t("购买游戏比例") }}: {{ uiData.value['gameTotalBeiShuBi'] == '' ? "" :
            parseFloat(uiData.value['gameTotalBeiShuBi']).toFixed(4) }}%
        </el-space>
        <el-table :data="uiData.tableData" style="width: 100%" v-loading="loading">
            <el-table-column :label="($t('区间'))">
            ...
            <el-table-column :label="$t('库存参考')">
                <template #default="scope">
                    <div :key="index" style="line-height: 40px;" v-for="(item, index) in scope.row">
                        {{ item.ExpectCount + "(" + item.CurrentCount + ")" }}</div>
                </template>
            </el-table-column>
            <el-table-column :label="$t('剩余个数')">
                <template #default="scope">
                    <div :key="index" style="line-height: 40px;" v-for="(item, index) in scope.row">{{
            ...
            <el-table-column label="中奖倍数" width="280px">
                <template #default="scope">
                    <div :key="index" style="line-height: 40px;" v-for="(item, index) in scope.row">
                        {{ item.DstTimes.toFixed(4)-item.PoolCost.toFixed(4) }},原始值：{{ item.DstTimes1.toFixed(4) }}</div>
                </template>
            </el-table-column>
...
onMounted(() => {
    let hash = {
        ...
        "slotsgen_majiang": "MaJiang",
        "slotsgen_tuzi": "TuZi",
    }
    let url = window.location.href;
    let subs = url.split("/");
    let name = subs[subs.length - 1];
    uiData.gameName = hash[name] || "";
    refreshData()
})
    ...
    for (let i = 0; i < tableData.length; i++) {
        ...
        for (let j = 0; j < tt.length; j++) {
            ...
            if (o.buyGame) {
                ...
                o.beishubi = Number.parseFloat(((o.DstTimes-o.PoolCost) / commonTotalCount).toFixed(6)) * 100
            }
...
const ExtractAction = async (index) => {
    ...
    const dataIndex = uiData.tableData[index];
    const args = dataIndex.map((data) => ({
        index: data.Index,
        count: ut.isNull(data.DstCount) ? 0 : data.DstCount,
    }));

    if (args.some((data) => data.count <= 0 && data.count > 0)) {
        return tip.e(`区间(${dataIndex[0].MinRate},${dataIndex[0].MaxRate}]，没有生成的，不可配置`);
    }

    loading.value = true;
    const [resultData, err] = await callFunc("ExtractAction", args);
    loading.value = false;
