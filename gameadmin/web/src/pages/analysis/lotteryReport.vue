<template>
    <div class='gameList'>
        ...
        <el-table :data="OpenTime.List" class="elTable" :header-cell-style="{ background: '#F5F7FA', color: '#333333' }"
            ...
            <el-table-column prop="SoldCount" :label="$t('销售张数')"/>
            <el-table-column prop="SoldGold" :label="$t('销售金额')"/>
            <el-table-column prop="PeriodSoldCnt" :label="$t('总销售张数')"/>
            <el-table-column prop="PeriodSoldGold" :label="$t('总销售金额')"/>
            <el-table-column prop="PeriodProfits" :label="$t('开奖金额')"/>
...
async function callFunc(args: any) {
    ...
    return Client.send(`mq/lotter/admin/reportOfDay`, args)
}
...
const queryList = async () => {
    ...
    OpenTime.List = data.Result?.map(v => {
        return {
            ...
            PeriodSoldGold:ut.toNumberWithComma(v.PeriodSoldGold).trim(),
            PeriodProfits:ut.toNumberWithComma(v.PeriodProfits).trim(),
