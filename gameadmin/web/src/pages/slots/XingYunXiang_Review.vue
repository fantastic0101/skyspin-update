<template>
    <div class="boxcolor flex_hbox">
        <div class="pan flex_grow">
            ...
            <el-carousel :autoplay="false" height="900px" trigger="click" v-if="data.Turns" ref="myCarousel" indicator-position="none"  @change="carouselChange">
                                ...
                                <div class="text-center">
                                    <div :key="'line' + i" class="line" v-for="(p2, i) in pan.HitLines">
                                        <el-space wrap>
                                            ...
                                            <div>{{ $t('金额')}}: <span class="color">{{ (p2.G)/10000 }}</span></div>
                                            <div>
                                                <el-popover placement="right" :width="400" trigger="hover">
                                                    <template #reference>
                                                        {{ $t('详情') }}
                                                        <span class="table-icon"><el-icon><InfoFilled /></el-icon></span>
                                                    </template>
                                                    <div class="info">
                                                        <div>
                                                            <p>
                                                                {{ $t('金额')}} {{ p2.Formula }}
<!--                                                                {{ (p2.G * props.Bet)/20 }}= ( ({{ p2.Counts?.join("*") }})* {{ p2.Rate }}* {{props.Bet}} /20 ) 【固定除以20】-->
                                                            </p>
                                                            <p>
                                                                【详情】 各列数量：{{ p2.Counts?.join("*") }}，{{ p2.Counts?.length }}列({{ p2.Rate }}分)，
                                                                倍率({{pan.Multi }})。
                                                            </p>
                                                        </div>
                                                    </div>
...
let myCarouselSelectCommand = reactive(null)
let updateData = () => {
    let turns = []
