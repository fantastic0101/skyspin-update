    ...
    <div class="boxcolor flex_hbox">
        <div class="pan flex_grow">
            ...
            <template v-if="data.Turns">
                ...
                <span class="text-center">当前位置: N0.{{data.myCarouselSelectCommand}}/{{data.Turns.length}}</span>
            </template>
<!--            <common-top :props="props" :data="data"></common-top>-->
            <el-carousel :autoplay="false" max-height="900px" trigger="click" v-if="data.Turns" ref="myCarousel" indicator-position="none"  @change="carouselChange">
                <el-carousel-item :key="'p' + panid" v-for="(pan, panid) in data.Turns">
                    <div class="text-center">
                        Bet Size： <span>{{ pan.gd.cs }}</span>
                        <el-divider direction="vertical" />
                        Bet Level： <span>{{ pan.gd.ml }}</span>
                    </div>
                    <div class="flex_vbox">
<!--                        <SlotPan :images="data.images" :Pan="pan.Pan" :highlight="(x, y) => hightLight(panid, x, y)" />-->
                                                    ...
                                                    <div class="info">
                                                        <el-space wrap>
                                                            ...
                                                            <p>
                                                                ...
                                                                {{ `${$t('炸弹倍率之和')}×${$t('投注金额')}×${$t('赢分金额')}=${$t('此次总得分')}` }}
                                                            </p>
                                                        </el-space>
                                                    </div>
                                                </el-popover>
                    ...
                    <div class="omf">
                        <template v-for="item in pan.gd.omf">
                            <div class="multiplier-value-container">
                                <div class="multiplier mtp_x"></div>
                                <div class="multiplier" :class="'mtp_'+item"></div>
                            </div>
                        </template>
                    </div>
                </el-carousel-item>
...
import SlotPan from "./SlotPan_Bonaza.vue";
import CommonTop from './pg-slots-common-top.vue'
...
let updateData = () => {
    ...
    if (props.PlayResp && props.PlayResp.bd) {
        props.PlayResp.bd.forEach(v => {
            /*if (isStartGame) {
                v.free = true
            }*/
...
.omf{
    ...
    font-size: 2rem;
    margin: 0px -49px;
    .multiplier-value-container{
        display: flex;
        flex-direction: row;
        justify-content: center;
        align-items: center;
        .multiplier {
            background-image: url('src/assets/slots/UltimateStriker/num.png');
