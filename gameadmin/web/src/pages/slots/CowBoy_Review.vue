    ...
    <div class="boxcolor flex_hbox">
        <div class="pan flex_grow">
            <template v-if="data.Turns">
                ...
                <span class="text-center">{{ $t('当前位置') }}: N0.{{data.myCarouselSelectCommand}}</span>
            </template>
            <el-carousel :autoplay="false" max-height="900px" trigger="click" v-if="data.Turns" ref="myCarousel" indicator-position="none"  @change="carouselChange">
                <el-carousel-item :key="'p' + panid" v-for="(pan, panid) in data.Turns">
                    <div class="flex_vbox">
                        <SlotPan :images="data.images" :Pan="pan.Pan" :hitLines="data.Turns.hitLines"/>
                                ...
                                <div class="text-center">
                                    <div :key="'line' + i" class="line" v-for="(p2, i) in pan.HitLines" >
                                        <el-space wrap>
                                            ...
                                            <div @click="sendHitLines(p2,panid)">
                                                    ...
                                                    <div class="info">
                                                        <el-space wrap>
                                                            ...
                                                            <p>
                                                                ...
                                                                {{ `${$t('下注额度')}×${$t('百搭总倍数')}×${$t('（3个、4个或者5个相同的元素对应的倍数）')}=${$t('当前连线得分')}` }}
                                                            </p>
...
import SlotPan from "./SlotPan_CowBoy.vue";
import { onMounted, onUpdated, reactive, ref } from "vue";
import { dyImport, getImages } from "./allimage";
import rulesPng from "@/assets/CowBoyMap/rules.png";
interface Props {
    PlayResp: any  // 盘面
    DiaLogGame: any  // 盘面
}
...
function getElementsFromArray(a, b) {
    let newArr = a.flat();
    let arrs = [];
    b.forEach(index => {
        if (index >= 0 && index < newArr.length) {
            arrs.push([...newArr][index]);
        }
    });
    return arrs
}
const sendHitLines = (i,index) => {
    data.Turns.hitLines = getElementsFromArray(data.Turns[index].Pan.Data, i.Line)
}
let updateData = () => {
    let turns = []
