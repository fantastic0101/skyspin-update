            ...
            >
                <template v-if="typeof imgId === 'object'">
                    <template v-if="images[imgId.nums]">
                        ...
                        <span v-if="(imgId.coin/(newPanListDi*10))>=0 && (imgId.coin/(newPanListDi*10))<5" style="color: yellow">{{imgId.coin/10000}}</span>
                        <span v-if="(imgId.coin/(newPanListDi*10))>=5 && (imgId.coin/(newPanListDi*10))<50" style="color: red">{{imgId.coin/10000}}</span>
                        <span v-if="(imgId.coin/(newPanListDi*10))>=50" style="color: pink">{{imgId.coin/10000}}</span>
...
const newPanListCoin = props.Pan.Coin
const newPanListDi = props.Di
const c = () => {
    for (let i = 0; i < newPanListData.length; i++) {
        if (newPanListData[i].length === newPanListCoin[i].length) {
            for (let j = 0; j < newPanListData[i].length; j++) {
                if (newPanListData[i][j] > 0 && newPanListCoin[i][j] > 0) {
                    newPanListData[i][j] = {
                        nums: newPanListData[i][j],
                        coin: newPanListCoin[i][j],
                    };
                }
            }
        }
