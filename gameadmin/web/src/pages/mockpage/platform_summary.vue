...
<template>
    <div >
        <AnalysisPlatform :uiData='uiData' @query-list="btnQuery" :hidden-time="true" :hase-all="true"></AnalysisPlatform>
        ...
        <div class="page_table_context">
            ...
            >
                ...
                <template #handleTools="scope">
                    <el-space>
                        <upload-excel @uploadFile="resolveData" file-name="平台汇总"/>
                ...
                <template #playersWinLose="scope">
                    <el-text :type="scope.scope.playersWinLose >= 0 ? 'success' : 'danger'" size="small">
                        ...
                        {{ ut.toNumberWithComma(scope.scope.playersWinLose * 1e4) }}
                    </el-text>
                </template>
                <template #companyWinsLoses="scope">
                    <el-text :type="scope.scope.companyWinsLoses >= 0 ? 'success' : 'danger'" size="small">
                        {{ ut.toNumberWithComma(scope.scope.companyWinsLoses * 1e4) }}
                    </el-text>
                </template>
...
const generatorExcel = () => {
    let generatorHeader = [...tableHeader].filter(item => item.label != '序号')


    excel.DataGeneratorExcel(generatorHeader, uiData.tableDataSlice, `平台汇总`)
}
