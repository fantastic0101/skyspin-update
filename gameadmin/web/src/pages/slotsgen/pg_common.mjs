...
import { onMounted, ref, reactive, h, onActivated, onUpdated, watch } from 'vue';
...
function setup(props) {
    ...
    async function callFunc(api, args) {
        return Client.send(`mq/${gid}/AdminSlotsRpc/${api}`, args)
    }
    ...
    return () =>  
        h('div', { style:{'text-align': 'center'}}, [
            r('el-button', {onClick: refreshData, type: 'primary'}, ()=>'刷新 ' + gid),
            h('br'),
            `Normal RTP: ${fmtfloat(totalTimes[0] / totalCount[0])}, Count: ${totalCount[0]}`,
            h('br'),
            `Buy RTP: ${fmtfloat(totalTimes[1] / (totalCount[1] * bets[1]))}, Count: ${totalCount[1]}`,
            r('el-table', { data: tableData.value, style: { width: '100%' } }, () => [
                ...
                r('el-table-column', { label: '组合占用' }, ({ row }) => row.map(item => h('div', item.UseCombineCount))),
