...
async function callFunc(api, args) {
    return Client.send(`mq/pg_39/AdminSlotsRpc/${api}`, args)
}
...
function setup() {
    ...
    async function refreshData() {
        ...
        for (const vv of data.RateStatus) {
            gameTotalCount += vv.reduce((sum, item) => sum + item.DstCount, 0)
            totalbeishu += vv.reduce((sum, item) => sum + item.DstTimes, 0)
        }
        console.log('gameTotalcount', gameTotalCount)
    ...
    return () =>  
        h('div', { style:{'text-align': 'center'}}, [
            ...
            h('br'),
            "普通转动回报: " + fmtfloat(totalbeishu / gameTotalCount),
            r('el-table', { data: tableData.value, style: { width: '100%' } }, () => [
                ...
                r('el-table-column', { label: '组合用掉个数' }, ({ row }) => row.map(item => h('div', item.UseCombineCount))),
                ...
                r('el-table-column', { label: '分布比例' }, ({ row }) => [
                    h('div', { style: { display: 'flex', 'justify-content': 'space-around', 'align-items': 'flex-end', 'line-height': '40px' } }, [
                        h('div', null, row.map(item=>h('div', fmtfloat( item.DstCount / gameTotalCount)))),
                        h('div', {style:{'padding-left': '20px'}},  fmtfloat( row.reduce((sum, item) => sum + item.DstCount, 0)/ gameTotalCount )),
                    ]),
                ]),
                r('el-table-column', { label: '中奖倍数',  }, ({ row }) => row.map(item => h('div', { style: {'line-height': '40px'}}, item.DstTimes))),
                r('el-table-column', { label: '中奖倍数-奖池',  }, ({ row }) => row.map(item => h('div', { style: {'line-height': '40px'}}, item.DstTimes-item.PoolCost*item.DstCount))),

                r('el-table-column', { label: '回报分布' }, ({ row }) => [
                    h('div', { style: { display: 'flex', 'justify-content': 'space-around', 'align-items': 'flex-end', 'line-height': '40px' } }, [
                        h('div', null, row.map(item=>h('div', fmtfloat( item.DstTimes / gameTotalCount)))),
                        h('div', {style:{'padding-left': '20px'}},  fmtfloat( row.reduce((sum, item) => sum + item.DstTimes, 0)/ gameTotalCount )),
                    ]),
                ]),
