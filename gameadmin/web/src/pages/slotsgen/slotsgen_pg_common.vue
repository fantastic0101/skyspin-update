<template>
    ...
    <div style="text-align: center; margin-top: 1rem">
        <el-collapse>
            <el-collapse-item title="pg游戏列表" name="1">
        ...
        <div style="margin-top: 1rem">
            ...
            <el-row :gutter="20" style="margin-top: 1rem">
                ...
                <el-col :span="20">
                    <el-space v-if="typeFor.length === 0">
                        <el-statistic title="下注倍数" :value="checkedModel.BuyBetMulti"/>
                        <el-divider direction="vertical"/>
                        <el-statistic title="Buy RTP"
                                      :value="(uiData.totalCount[1] * bets[1])?ut.fmtPercent(uiData.totalTimes[1] / (uiData.totalCount[1] * bets[1])): $t('暂无')"/>
                        <el-divider direction="vertical"/>
                        <el-statistic title="Count" :value="uiData.totalCount[1]"/>
                    </el-space>
                    <el-space v-else>
                        <template v-for="(item,index) in typeFor">
                            <el-statistic title="下注倍数" :value="item+1"/>
                            <el-divider direction="vertical"/>
                            <el-statistic title="Buy RTP"
                                          :value="(uiData.totalCount[index+1] * (item+1))?
                                          ut.fmtPercent(
                                              uiData.totalTimes[index+1]
                                              /
                                              (uiData.totalCount[index+1] * (item+1))
                                              )
                                          : $t('暂无')"/>
                            <el-divider direction="vertical"/>
                            <el-statistic title="Count" :value="uiData.totalCount[index+1]"/>
                        </template>
                    </el-space>
                </el-col>
            </el-row>
...
onMounted(async () => {
    ...
    checkedList.value = datas.List.map(l => {
        if (l.ID.split('_').length > 1 && l.ID.startsWith('pg')) {
            return {
                ...l,
                label: l.Name,
                value: l.ID
    ...
    restaurants.value = checkedList.value
})
...
const refreshData = async () => {
    ...
    if (isDisType.value) {
        typeFor.value = []
        // buy.value = checkedModel.value.Type + 1
        const uniqueTypes = new Set();
        data.RateStatus.forEach((list,index) =>{
            list.forEach((op) => {
    ...
    uiData.totalCount = Object.values(totalCountByType);
    uiData.totalTimes = Object.values(totalTimesByType);
