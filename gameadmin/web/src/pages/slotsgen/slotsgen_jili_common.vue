<template>
    ...
    <div style="text-align: center; margin-top: 1rem">
        <el-collapse>
            <el-collapse-item title="jili游戏列表" name="1">
...
onMounted(async () => {
    try {
        ...
        const filteredList = datas.List.reduce((acc, l) => {
            if (typeof l.ID === 'string' && l.ID.startsWith('jili')) {
                acc.push({
                    ...l,
                    label: l.Name,
                    value: l.ID
...
const refreshData = async () => {
    ...
    for (const opType of typeFor.value) {
        if (uiData?.gameName === "jili_144_cai") {
            if (opType === 1) typeMulti.value.push(1);
            if (opType === 2) typeMulti.value.push(50);
        } else {
            if (opType === 1) typeMulti.value.push(50);
            if (opType === 2) typeMulti.value.push(1.5);
        }
