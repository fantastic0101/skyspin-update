            ...
            <div style="min-height: 440px" v-loading="loading" element-loading-background="rgba(0, 0, 0, 0)">
                ...
                <div style="width: 100%;display: flex;margin-top: 15px" v-if="!currentAllFilter.length">
                    <div class="game-type-title class-game-border all-title"
                         v-if="!searchName && sortValue != 2" id="classificationContent">{{ filterGameType }} &nbsp;
                        <span style="font-weight: normal;color: #999">GAMES</span>
                    </div>
                ...
                <div>
                    ...
                    <template v-else>
                        <template v-for="(item, index) in AllGameList">
                            <div :key="index" v-if="item instanceof Array && item.length" :id="`${index}-container`">

                                <div class="game-type-title manufacturer-title" :id="`${index}-title`">{{ index }}
                                    &nbsp;GAMES
                                </div>
...
const generatorMap = () => {
    ...
    for (let index in Manufacturer.value) {
        ...
        if (Manufacturer.value[index].label == "SPRIBE") {
            continue
        }
...
.sortSelect .is-focused {
    box-shadow: 0 0 0 1px #EB334D inset !important;
